...
- **D-04:** The gate signal is the **per-cell test outcome** and is **distinct from the judge rubric score**. Planner/researcher must confirm where the test outcome is persisted — whether Phase 78's runner already records a `test_exit`/gate field on the Run, or whether Phase 79 must (a) read it from an existing field or (b) add gate capture. **This is the single most important research question for the phase** — if the gate signal is not yet persisted on the Run, closing CMP-01 requires a small Phase 78 runner addition (coordinate with the Phase 78 close-out).
