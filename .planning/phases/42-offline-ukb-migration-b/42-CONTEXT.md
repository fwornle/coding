...
- **A's parallel-path asymmetry (cross this bridge later, 2026-05-23):** Phase 41 left A in the co-exist pattern per ROADMAP design — A's ObservationWriter/Consolidator/observations-api still own A's write and read paths; the km-core adapter is operator-callable only (no daemon). Once Phase 42 closes, B will be 100% on km-core while A will still run two parallel paths. The user reviewed this asymmetry on 2026-05-23 and **explicitly deferred the decision** of whether to also migrate A's read path or writer (would be a Phase 41.1). Three options were on the table: (a) accept co-exist, (b) read-side cutover only (dashboard + prompt-hook read from km-core), (c) full writer + reader cutover. Revisit before Phase 44 (REST API) since the unified read surface forces the choice.
