...
- **R-2 Express as peerDependency in km-core.** All three systems use Express today (grep-confirmed at `scripts/observations-api-server.mjs`, OKM `src/api/routes.ts:450+`, `integrations/mcp-server-semantic-analysis/src/sse-server.ts`). Zero migration tax. Factory signature returns `express.Router`. Documented breaking-change-on-major if the framework ever changes.
...
- **S-2 Hard reset on restore.** `POST /api/v1/snapshots/:id/restore` performs: `git checkout tag/<snapshot> -- .data/exports/` + `rm -rf .data/leveldb/` (or per-system equivalent) + restart server. Server rebuilds LevelDB from JSONs on first read via the Phase 37 D-22 LevelDB-empty → JSON-fallback semantics. **Destructive by design** — any uncommitted live data since the snapshot is lost. Caller is responsible for taking a safety snapshot first if needed.
- **S-3 OKB-baseline guard whitelists snapshot prefixes.** The git hook that enforces Phase 37 D-21's two-commit pattern is extended to recognize `chore(snapshot): …` and `chore(restore): …` as exempt (they don't require a sibling `feat:` commit). Normal two-commit pattern for code/data changes continues unchanged. Single concrete edit: add the two prefixes to the hook's whitelist.
