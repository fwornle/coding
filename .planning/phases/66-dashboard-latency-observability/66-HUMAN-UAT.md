...
- truth: "The dashboard's claude-code latency column shows the warm-pool speedup as a median dropping to ≤3s (green) — PERF-03 / SC-1"
  ...
  chosen_fix_direction: "SPAWN/QUEUE OVERHEAD (operator decision, 2026-06-21). The proxy must record the per-call overhead component the worker pool actually affects — the time from request-received to worker-ready/first-token (warm reuse ≈ 0; cold spawn ≈ 10–14s), EXCLUDING model generation time. Surface a per-model median over THAT overhead (not total latency_ms) and re-point the dashboard tile + Token Usage column to it. The existing ≤3s green / amber / red envelope then becomes meaningful (it matches PERF-01's warm-overhead bar), and a pool regression (cold-spawn storm / pool disabled) shows up as overhead climbing toward ~14s. Implementation spans BOTH repos: (1) rapid-llm-proxy — capture an overhead/queue-to-ready timestamp in the worker-pool dispatch path and log it as a new token-usage field (66-01-class change to token-usage.ts + worker-pool.mjs + the logTokenCall call site in server.mjs), plus a per-model median query; (2) system-health-dashboard — point llm-latency-tile.tsx + token-usage.tsx at the new overhead field, relabel ('overhead'/'spawn latency'), keep the ≤3s/amber/red envelope. Haiku (direct path, no pool) has ~no pool-overhead — keep it as a non-graded reference or omit from the overhead view."
