...
**One-liner:** Stamps `metadata.project` at the canonical-mapper primary site and the km-core-adapter defence-in-depth dual site for every wave-analysis writer call, gated by Plan 01's `isProject()` typeguard — execution paused at Task 4 (human-verify checkpoint) with a runtime-resolution blocker requiring operator decision.

## What Shipped

Two source files and three wave-agent call sites carry the new `metadata.project` stamp end-to-end through the wave-analysis writer pipeline:
...
### Task 1: Stamp metadata.project at canonical-mapper.ts + wire wave1/2/3 callers
...
### Task 2: Defence-in-depth dual-stamp in km-core-adapter.ts + verify persistence-agent + code-graph-rag no-op
...
### Task 3: Build submodule + container restart
...
### Task 4: HUMAN-UAT — PAUSED (checkpoint reached)
...
### Out-of-Scope Discovery: km-core resolution inside Docker container

- **Found during:** Task 3 post-restart in-container verification.
- **Observation:** Inside the running container, `import('@fwornle/km-core')` resolves to `~/Agentic/km-core/dist/` (mounted via docker-compose.yml:96 `${HOME}/Agentic/km-core:/coding/node_modules/@fwornle/km-core:ro`). That host clone last committed 2026-06-04 (`c7bc236`) and does NOT carry Plan 01/02 (project module + coding.lower.json).
- **Runtime impact:** A test `docker exec coding-services node --input-type=module -e 'import("@fwornle/km-core").then(km => console.log(typeof km.isProject))'` returns `isProject: undefined PROJECTS: undefined`. The next `ukb full` run that hits the new `isProject(options.project)` call site WILL throw `TypeError: isProject is not a function`.
- **This is the primary blocker the Task 4 checkpoint surfaces to the operator** — see "Checkpoint Status" below for the resolution options.
- **Not auto-fixable** because the choice between (a) sync `~/Agentic/km-core` with `lib/km-core` (network + git push/pull across two clones of the same remote) and (b) change `docker-compose.yml` to mount `lib/km-core` directly (one-line architectural change affecting ALL container services that import km-core) is a Rule 4 architectural decision requiring operator input.

## Checkpoint Status

**Task 4 status:** PAUSED — human-verify with a blocking resolution issue.

The source-side + dist-side stamping is complete and correct. The runtime gap is in km-core resolution inside the container: the bind-mount source (`~/Agentic/km-core`) is days behind the active dev tree (`lib/km-core`) that holds Plan 57-01's `isProject` export.

The operator must choose one of:

1. **Sync `~/Agentic/km-core` with `lib/km-core`** — both repos point at `git@github.com:fwornle/km-core.git`. Push Plan 01/02 commits from `lib/km-core` to the remote, then `cd ~/Agentic/km-core && git pull && npm run build`. Lowest architectural impact; preserves current docker-compose mount.
2. **Change docker-compose.yml km-core mount to `lib/km-core`** — replace line 96 `${HOME}/Agentic/km-core:/coding/node_modules/@fwornle/km-core:ro` with `${CODING_REPO:-.}/lib/km-core:/coding/node_modules/@fwornle/km-core:ro`. Container then resolves km-core through the active dev tree; matches the outer-repo node_modules pattern. Mid-impact architectural change affecting ALL container services that import km-core (all of integrations/mcp-server-semantic-analysis).
3. **Defer Plan 57-03 verification + back out the Plan 01 import** — make Phase 57 stamping work only in tests/dev outside the container until the km-core mount choice is made in a separate phase. Highest churn (back out work already shipped).

Recommendation: Option 1 (sync), then proceed with Task 4's `ukb full` verification.
...
| Final docs + state | outer (coding) | main | (pending until Task 4 resolves) | (deferred — checkpoint in progress) |
...
When Task 4 unblocks and Plan 03 closes:

- **Plan 04** (`coding.lower.json` classifier injection) — independent of writer-stamping; can land in parallel.
- **Plan 05** (`scripts/backfill-project-tag.mjs`) — backfills the existing 1262 nodes with `metadata.project`. Will use the same `isProject()` typeguard for closed-set validation.
...
2. **`~/Agentic/km-core` is days behind `lib/km-core`:** Last commit `c7bc236` (2026-06-04) vs `lib/km-core` at `f87bd1a` (Plan 57-02, 2026-06-14). The Docker container imports km-core from the stale clone. This is the Task 4 blocker.
...
*Plan paused 2026-06-14T14:40Z, ~7 min wall-clock execution of Tasks 1-3. Task 4 awaits operator decision on km-core container resolution.*
