...
- [x] **PERF-03:** The dashboard's claude-code latency column shows the speedup — median claude-code/sonnet latency drops from ~14s to ≤3s within 24h of rollout. (COMPLETE Phase 66 — both SC-1 and SC-2 closed. 66-04 re-pointed both :3032 surfaces to the worker-pool SPAWN OVERHEAD (overhead_ms / p50_overhead_ms): SC-1 (warm→green) live-confirmed — warm sonnet overhead 1–14ms renders the GREEN "OK" badge `rgb(21,128,61)` on the "LLM Pool Overhead" tile AND a green badge on the Token Usage by-model "Spawn Overhead" column; Median Latency retained as forensic context. SC-2 (regression→red) closed by 66-05 via the opt-in `LLM_PROXY_WORKER_SPAWN_DELAY_MS` test seam (worker-pool.mjs defers the prompt write so the injected delay lands in the overhead window): with the env at 6000ms the sonnet overhead median climbed to 6.0s and BOTH surfaces flipped RED — tile "Regressed" badge bg `rgb(239,68,68)`, table "Spawn Overhead" badge text `rgb(185,28,28)` on `rgb(254,242,242)`; clearing the env returned both to GREEN (tile "OK" `rgb(21,128,61)`, table 1.1s green). Live-proven 2026-06-21 on :3032 via gsd-browser computed-rgb read-back. Evidence: 66-05-tile-cold-red.png, 66-05-table-overhead-red.png, 66-05-tile-restored-green.png, 66-05-table-restored-green.png. 5/5 must-haves PASS.)
...
| PERF-03 | Phase 66 | Complete — SC-1 + SC-2 both live-proven (2026-06-21). 66-04 re-pointed both `:3032` surfaces from total latency to worker-pool SPAWN OVERHEAD (overhead_ms / p50_overhead_ms); SC-1 (warm→green) confirmed: warm sonnet overhead 1–14ms → GREEN "OK" `rgb(21,128,61)` on the "LLM Pool Overhead" tile + green badge on the Token Usage "Spawn Overhead" column; Median Latency retained as forensic context; haiku non-graded reference. 66-05 closed SC-2 (regression→red) via the opt-in `LLM_PROXY_WORKER_SPAWN_DELAY_MS` worker-pool test seam (defers the prompt write so the injected delay lands in the overhead window, NOT generation; no-op when unset; 65/65 unit tests, tsc clean): env=6000ms drove the sonnet overhead median to 6.0s and BOTH surfaces flipped RED — tile "Regressed" bg `rgb(239,68,68)`, table "Spawn Overhead" `rgb(185,28,28)` on `rgb(254,242,242)`; clearing it returned both to GREEN and the pool to healthy (warm 14ms, /health 200). Computed-rgb verified via gsd-browser. 5/5 must-haves PASS. Evidence: 66-05-{tile-cold-red,table-overhead-red,tile-restored-green,table-restored-green}.png. Proxy commits: test `aa474a3`, feat `a4ce41d`. |
