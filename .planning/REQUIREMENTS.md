...
- [~] **PERF-03:** The dashboard's claude-code latency column shows the speedup — median claude-code/sonnet latency drops from ~14s to ≤3s within 24h of rollout. (Implemented & verified Phase 66 — both surfaces render the per-model median from fallback-path telemetry; live ≤3s/green-badge confirmation pending in 66-HUMAN-UAT.md.)
...
| PERF-03 | Phase 66 | Implemented & verified (2026-06-21 — Phase 66 delivered both dashboard surfaces: `:3032` LlmLatencyTile + Token Usage Median column, sourced from proxy `p50_latency_ms` fallback-path telemetry; 6/6 must-haves verified, code review 0 critical. Live ≤3s/green-badge confirmation + pool-disabled regression signal pending — tracked in 66-HUMAN-UAT.md) |
