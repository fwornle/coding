...
- [~] **PERF-03:** The dashboard's claude-code latency column shows the speedup — median claude-code/sonnet latency drops from ~14s to ≤3s within 24h of rollout. (Implemented & LIVE-VERIFIED Phase 66 — 66-04 re-pointed both surfaces to the worker-pool SPAWN OVERHEAD (overhead_ms / p50_overhead_ms); SC-1 confirmed live: warm sonnet overhead 1–7ms renders the GREEN "OK" badge `rgb(21,128,61)` on the :3032 "LLM Pool Overhead" tile AND a green 7ms badge on the Token Usage by-model "Spawn Overhead" column, while the retained Median Latency column shows the 165.1s total as forensic context. SC-2 (pool-disabled→red regression signal) NOT reachable on this Apple Silicon host: genuine cold-spawn overhead tops out at ~2.5s (< the 5s red / 3s amber thresholds) and the pool-disable switch logs NULL overhead which the tile filters out — deferred with a recommended `LLM_PROXY_WORKER_SPAWN_DELAY_MS` test-seam follow-up, see 66 deferred-items.md. Stays [~] until SC-2 red is demonstrated on a slow-boot host or via the seam.)
...
| PERF-03 | Phase 66 | Implemented & LIVE-VERIFIED for SC-1; SC-2 deferred (2026-06-21 — 66-04 re-pointed both surfaces from total latency to worker-pool SPAWN OVERHEAD (overhead_ms / p50_overhead_ms). SC-1 confirmed live: warm sonnet overhead 1–7ms → GREEN "OK" badge `rgb(21,128,61)` on the `:3032` "LLM Pool Overhead" tile + green 7ms on the Token Usage "Spawn Overhead" column; Median Latency 165.1s retained as forensic context; haiku non-graded reference; quiet families "no recent pool calls". SC-2 (pool-disabled→red) NOT reachable on this host — cold-spawn overhead ≤2.5s (< 5s red / 3s amber); disable switch logs NULL (filtered). Deferred to 66 deferred-items.md with `LLM_PROXY_WORKER_SPAWN_DELAY_MS` test-seam follow-up. 4/5 must-haves PASS, 1 (SC-2 red) deferred.) |
