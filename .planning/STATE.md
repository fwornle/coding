...
- Phase 51 added 2026-05-23: GSD wave-execution spawns sub-agents as separate `claude` CLI invocations in `/private/tmp/<uuid>/` worktrees; their transcripts (58 confirmed today during the Phase 42 wave) live under `/Users/Q284340/.claude/projects/-private-tmp/` and the per-project ETM does not watch that path → entire categories of coding work generate zero observations. Two-path fix: ship Path B (post-hoc sweep via existing `scripts/convert-transcripts.js`) first as the cheap stopgap, Path A (spawn-time ETM registration) as the follow-on. Strong plumbing overlap with Phase 50. See `.planning/phases/51-…/51-CONTEXT.md`.
