...
- [ ] 42.2-01-PLAN.md — Forensics pass over canonical-mapper emit paths + LLMService dispatch sites + 802-entity team-field audit; outputs .planning/forensics/report-42.2-00-canonical-emit.md with binary RE-MIGRATION REQUIRED/NOT REQUIRED verdict. BLOCKS Plan 02.
...
- [ ] 42.2-04-PLAN.md — Legacy persistence trio retirement (depends on 42.2-02 + 42.2-03): delete persistence-agent.ts + graph-database-adapter.ts + GraphDatabaseService.js; rewire ~30 consumer call sites in coordinator.ts (saveSuccessfulWorkflowCompletion / linkInsightDocuments / removeEntity / JSON export / registered-agent path per 42-07-SUMMARY surprise #1), tools.ts (3 tool-handler call sites), and content-validation-agent.ts (collapse setGraphDB+setPersistenceAgent into setKmCoreAdapter); full submodule npm test pass.
