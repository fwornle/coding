...
**Plans:** 2 plans
  - [ ] 66-01-PLAN.md — Add MEDIAN(latency_ms) per-model query to the proxy token-usage SQL layer; expose p50_latency_ms on getSummary by_model rows (rapid-llm-proxy)
  - [ ] 66-02-PLAN.md — Surface per-model median on both dashboard surfaces: :3032 headline tile + Token Usage by-model column with green ≤3s / red-toward-14s regression badge (coding)
**UI hint:** yes
