...
**Requirements**: No REQUIREMENTS.md IDs mapped; acceptance surface is CONTEXT decisions D-01..D-12 (D-01 AMENDED + D-12 ADDED 2026-07-08 post-research). All 12 covered across 6 plans.
**Plans**: 6 plans
- [ ] 85-01-PLAN.md — Progress emitter (atomic never-throw) + rerun_of/base_variant Run metadata + CLI flags incl. capture_raw_bodies (D-03/04/05/06/07/12) [Wave 1]
- [ ] 85-02-PLAN.md — Host-side run-launch primitives: detached fixed-argv spawn + process-group cancel + pid liveness (D-01/08) [Wave 1]
- [ ] 85-03-PLAN.md — Host executor seam on health-coordinator :3034 (delegates spawn/cancel; clears stale span) (D-01/02/08) [Wave 2]
- [ ] 85-04-PLAN.md — vkb-server API routes: run (dual-source 409 → delegate), run-status (verbatim), run-cancel, specs (matrix preview) (D-01/02/04/08/09) [Wave 3]
...
| 85. Experiment Control Center | v7.5 | 0/6 | Planned | - |
