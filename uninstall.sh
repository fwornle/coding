...
# Clean up the retired Mastracode AGENT.
#
# NOT the same thing as the @mastra/opencode block above, despite the shared
# vendor name: that is an observational-memory PLUGIN for OpenCode and is still
# in use. This block is about `mastracode`, the standalone coding-agent CLI that
# `coding --mastra` used to launch, replaced by pi (`coding --pi`).
#
# The reason this exists at all: config/agents/mastra.sh generated
# ~/.mastracode/hooks.json wiring six lifecycle events to a transcript-writer
# script inside this repo. That hooks file OUTLIVES the uninstall — every later
# `mastracode` launch keeps appending to .observations/transcripts/ and keeps
# invoking a script path that no longer exists. Removing it is the whole point.
echo -e "\n${BLUE}🗑️  Removing retired Mastracode agent integration...${NC}"

# The hooks file is OURS — generated by the launcher, so we delete it outright.
if [[ -f "$HOME/.mastracode/hooks.json" ]]; then
    rm -f "$HOME/.mastracode/hooks.json"
    echo "  Removed ~/.mastracode/hooks.json (lifecycle transcript hooks)"
    # Only if we left it empty — never clobber anything the user put there.
    rmdir "$HOME/.mastracode" 2>/dev/null && echo "  Removed empty ~/.mastracode/"
else
    echo "  ~/.mastracode/hooks.json not found -- skipping"
fi

# Everything below is the USER's, not ours: OAuth credentials, and transcripts
# that are observation source data. Report, do not delete.
_mastra_support="$HOME/Library/Application Support/mastracode"
[[ ! -d "$_mastra_support" ]] && _mastra_support="$HOME/.local/share/mastracode"
if [[ -d "$_mastra_support" ]]; then
    echo -e "  ${GREEN}Preserved $_mastra_support${NC}"
    echo "    (holds mastracode OAuth credentials + the rapid-proxy-mastra customProvider)"
    echo "    Remove by hand if you no longer want mastracode at all:"
    echo "      rm -rf \"$_mastra_support\""
fi

_mastra_transcript="$CODING_REPO/.observations/transcripts/mastra-transcript.jsonl"
if [[ -f "$_mastra_transcript" ]]; then
    echo -e "  ${GREEN}Preserved mastra-transcript.jsonl${NC} ($(wc -l < "$_mastra_transcript" | tr -d ' ') lines of observation source data)"
fi

if command -v mastracode >/dev/null 2>&1; then
    echo "  mastracode CLI still installed at $(command -v mastracode)"
    echo "    Remove it with: npm uninstall -g mastracode"
fi
