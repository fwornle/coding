...
- **`measurement-start.mjs`** opens the span and writes `.data/active-measurement.json`. On top of that span, every cell also binds its tokens **per-request** so capture is robust even when the cell isn't the only thing routing through the proxy (Phase 84):
    ...
    - **pi** routes via its own `models.json` custom provider (`rapid-proxy-pi`), which carries an
      `x-task-id: $TASK_ID` header; **copilot** uses the BYOK task-scoped seam.
...
**Parallel runs share their background.** Sandboxed cells normally suppress this panel (a throwaway worktree has no honest background of its own). But when a matrix runs in [parallel](experiment-skill.md#serial-vs-parallel-execution) mode (`execution_mode: parallel`), the cells overlap in time, so the same background traffic ran concurrently with **all** of them. The panel is therefore shown on every parallel cell's timeline — recorded once, overlaid on each — with an explicit disclaimer that the activity is **shared between the cells and cannot be attributed to any single cell**.

![Parallel cell timeline — the shared "Concurrent background activity" panel with the parallel disclaimer](../images/measurement-parallel-ambient-note.png)
...
| pi | `--append-system-prompt "<knowledge>"` (launch argv, same channel as claude) |
