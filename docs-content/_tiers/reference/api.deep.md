...
Only **Graphify** is still an MCP server. The semantic-analysis and constraint-monitor tools
were retired as MCP servers in favour of CLIs — the tools themselves still exist and do the
same work, but the agent reaches them by running a command rather than over MCP. That change
removed roughly 14 KB of tool schema from every context window.
