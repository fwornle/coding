...
The legacy shell script is gone, and so is the MCP server that briefly replaced it — the
semantic-analysis MCP tools were retired in favour of a CLI, which the agent runs on your
behalf. Asking for a "ukb" pass in chat still works; it resolves to:
...
- `ukb` as a shell command in any form — the binary was removed, and so was the MCP
