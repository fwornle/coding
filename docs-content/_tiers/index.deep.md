...
The pass is asynchronous — the command returns a workflow id and leaves the run going, so
watch it on the dashboard or poll `semantic workflow status` rather than waiting on it.
