...
- **km-core node_modules patch (snapshot-restore on restart)**: `node_modules/@fwornle/km-core/dist/store/persistence.js` `hydrate()` is locally patched (2026-06-11) to prefer the JSON export over the LevelDB `graph:state` cache when the JSON has more nodes. **Why**: `persistGraph` only fires on clean `close()`; an obs-api crash during shutdown (we hit a `libc++abi mutex lock failed` on SIGTERM) leaves LevelDB frozen at the prior clean state — *days* behind the JSON exports written by the exporter's 5s-debounced scheduleExport. Without the patch, `launchctl kickstart com.coding.obs-api` resurrects the stale state (a 2026-06-05 snapshot came back over a 2026-06-11 backfill). **Re-apply after `npm install`** until upstream km-core fixes persistGraph to debounce-on-every-mutation. No patch-package wired yet; do it manually.
