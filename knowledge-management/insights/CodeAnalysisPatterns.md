...
- **Parent Component (CodingPatterns)** – `CodingPatterns` consumes the analysis results produced by **CodeAnalysisPatterns** to surface higher‑level pattern detections, refactoring suggestions, and quality dashboards. The parent invokes the batch pipeline via a simple façade exposed in `integrations/semantic-analysis/src/agents/ontology-classification-agent.ts`.
