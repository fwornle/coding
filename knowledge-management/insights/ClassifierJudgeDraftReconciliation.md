...
[Code References] integrations/system-health-dashboard/src/components/llm-routing/use-classifier-judge.ts:1-24 — header comment documenting the 2026-09-02 dead-backend incident and the rationale for a second hook; integrations/system-health-dashboard/src/components/llm-routing/use-classifier-judge.ts:33-40 — `JudgeBackend` interface separating `enabled` (config) from `reachable` (runtime, nullable); integrations/system-health-dashboard/src/components/llm-routing/use-classifier-judge.ts:150-158 — `effectiveEnabled()` and `merged` memo overlaying draft edits onto server state without touching `reachable`; integrations/system-health-dashboard/src/components/llm-routing/use-classifier-judge.ts:161-192 — `save()` builds a diff-only PATCH body from `draftEnabled`/`draftRubric`/`draftStrategy`; integrations/system-health-dashboard/src/components/llm-routing/use-classifier-judge.ts:120-131 — polling `useEffect` with comment on why it does not pause during an open edit; integrations/system-health-dashboard/src/components/llm-routing/offload-decision.tsx:1-23 — file-level comment on config-vs-recorded modes and the proxy-mirror-must-be-checked design; integrations/system-health-dashboard/src/components/llm-routing/offload-decision.tsx:188-197 — `resolveKey` memo excluding `hours` from invalidation; integrations/system-health-dashboard/src/components/llm-routing/offload-decision.tsx:238-244 — comment on why `cls`/judge state is excluded from `configRungs`; lib/statusline/model-limits.cjs:186-236 — `catalogueContextWindow()` and `limits()` precedence chain and mtime+size cache identity; integrations/system-health-dashboard/src/components/cost/cost-model.ts:45-58 — `budgetForMonth()` explicit-null-is-honored overlay lookup; integrations/system-health-dashboard/src/store/provider.tsx:12-33 — `ReduxProvider` global error handler and dev-only `window.__REDUX_STORE__` exposure
