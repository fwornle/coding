...
At the top level, BrowserAccess resides under the **KnowledgeManagement** parent component, inheriting the same data‑consistency guarantees provided by the GraphDatabaseAdapter (implemented in `integrations/semantic-analysis/src/storage/graph-database-adapter.ts`).  The component does not embed its own database logic; instead it **delegates persistence** to the **GraphDatabaseModule**, which itself is a thin wrapper around the GraphDatabaseAdapter.  This separation of concerns keeps BrowserAccess focused on session handling and UI exposure while reusing the robust graph‑storage infrastructure already present in the system.
