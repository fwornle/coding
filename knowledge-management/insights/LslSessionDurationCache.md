...
[Architecture Notes] Agent launch scripts (copilot.sh, opencode.sh, pi.sh) are sourced by a common orchestrator (scripts/launch-agent-common.sh) rather than invoked as standalone executables, enabling a uniform hook-based lifecycle across differing backends.; Configuration for opencode is built at launch time via string-spliced JSON fragments into OPENCODE_CONFIG_CONTENT rather than a static config file, tightly coupling shell scripting to JSON structural correctness (guarded by _oc_splice_config).; pi's extension and prompt installation (pi.sh's _pi_install_extensions, _pi_write_append_system) uses a marker-comment convention to distinguish tool-managed files from user-authored ones, avoiding destructive overwrites in global-scope installs.; Routing decisions are centralized in llm-routing.yaml and enforced at the proxy layer; per-agent config scripts (opencode.sh, pi.sh) are intentionally limited to translating agent-native controls into that shared vocabulary rather than making routing decisions themselves.; batch-provenance.mjs is a pure, dependency-free helper module (no imports) exporting selectBatchesForReport() and a private toEpoch() utility, suggesting it's designed for isolated unit testing separate from the dashboard server it's consumed by.
