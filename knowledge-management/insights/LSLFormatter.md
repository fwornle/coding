...
4. The resulting formatted payload is emitted to downstream components such as *OntologyClassificationAgent* (`integrations/semantic-analysis/src/agents/ontology-classification-agent.ts`) or persisted by the *Logger*.  
...
### Siblings
- [OntologyManager](./OntologyManager.md) -- The OntologyManager uses the OntologyClassificationAgent, located in integrations/semantic-analysis/src/agents/ontology-classification-agent.ts, to classify observations against the ontology system.
