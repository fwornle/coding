...
DMRConfigLoader performs multi-path search for the DMR YAML config file, checking several candidate locations to support both local dev and containerized deployments as documented in integrations/semantic-analysis/docs/configuration.md

# DMRConfigLoader — Technical Insight Document

## What It Is

DMRConfigLoader is a SubComponent within the `LLMAbstraction` layer of the `integrations/semantic-analysis` integration. Its responsibility is to locate, parse, and resolve the YAML configuration file that describes how the system should connect to Docker Model Runner (DMR) — a local inference backend used as one of the alternatives to public cloud LLM providers. Documentation for the loader's behavior and the configuration file format lives at `integrations/semantic-analysis/docs/configuration.md`, and its role in the broader mode-resolution flow is described in `integrations/semantic-analysis/README.md`.
...
The loader's outputs — resolved endpoint URLs and model names — feed downstream DMR client code that performs the actual local inference calls. It shares architectural space with sibling provider integrations for Anthropic, OpenAI, Groq, and the mock backend; each of these follows the same lazy-activation pattern, ensuring that providers not in use impose no runtime cost. The loader also has a documented relationship with `integrations/semantic-analysis/docs/configuration.md` (its specification) and `integrations/semantic-analysis/README.md` (its place in the mode-resolution priority chain).
