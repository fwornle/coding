...
`LocalLLMFallback` is a SubComponent of the broader `LLMAbstraction` layer that governs how the system routes inference requests to a local Docker Model Runner (DMR) endpoint instead of public cloud providers. While no dedicated source file is enumerated for the SubComponent itself, its behavior is documented in `docs/puml/local-llm-fallback.puml` and its decision logic is anchored in `getLLMMode()` within `integrations/semantic-analysis/src/mock/llm-mock-service.ts`. The network-level execution is delegated to the `@rapid/llm-proxy` shared package, where DMR is registered as one of five provider targets.
