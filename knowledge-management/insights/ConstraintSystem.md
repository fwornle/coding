...
The ConstraintSystem is a multi-layered constraint monitoring and enforcement framework implemented across several key paths: `lib/agent-api/hooks/hook-manager.js` (UnifiedHookManager), `lib/agent-api/hooks/hook-config.js` (HookConfigLoader), `scripts/violation-capture-service.js` (ViolationCaptureService), and `integrations/semantic-analysis/src/agents/content-validation-agent.ts` (ContentValidationAgent). It validates code actions, file operations, and tool interactions against configured rules during Claude Code sessions, persisting violations to `.mcp-sync/` for downstream consumption by the ConstraintDashboard.
