...
AdapterAndWrapperPatterns is a SubComponent of the broader CodingPatterns family within the `integrations/semantic-analysis/` codebase, documenting how third-party libraries are mediated through dedicated wrapper classes before being exposed to the rest of the system. The canonical implementations live under an infrastructure-oriented directory — for example, `storage/graph-database-adapter.ts` houses the `GraphDatabaseAdapter` class — and the overall pattern is formally described in `integrations/semantic-analysis/docs/architecture/integration.md`.
