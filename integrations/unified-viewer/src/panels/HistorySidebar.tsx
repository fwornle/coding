...
/**
 * A row also belongs in the feed when it CARRIES an insight document, whatever
 * its class.
 *
 * 2026-09-20: a UKB run writes Components/SubComponents/Details, never
 * Insights, so this feed could not show batch output at all. Wave 4 had been
 * working around that by minting one Insight-classed satellite per document
 * whose whole content was "Insight document generated by wave-analysis for Foo
 * (Detail). Source document: /…/Foo.md" — 128 nodes that existed purely to
 * satisfy the line above. Those are deleted and that block is disabled.
 *
 * Widening the predicate here is the honest fix: `has_insight_document` is
 * stamped on the REAL entity, which already holds the genuine description and
 * the path to the document, so the feed now surfaces the thing that has the
 * knowledge instead of a pointer beside it.
 */
const carriesInsightDocument = (meta: Record<string, unknown>): boolean =>
  meta.has_insight_document === true
