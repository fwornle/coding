...
// Visible-cell width lives in its own module so the tests measure with the
// exact function the padder targets, instead of a second copy that drifts.
// See lib/statusline/visible-cell-width.cjs.
...
// statusLeftReserveCells now lives in lib/statusline/pane-cache-key.cjs, because
// it is a CACHE KEY component: it decides the padded width, so two panes that
// disagree on it must not share a cache entry. Imported rather than mirrored —
// see that file's `reserve` note.
