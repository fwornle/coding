...
class SemanticToolInterpreter {
  ...
  async summarize(toolCall, result, conversationContext = {}) {
    if (!toolCall || typeof toolCall !== 'object') {
      return this.genericSummary('unknown', {}, result);
    }
  ...
  interpretGlob(tool, params, result) {
    ...
    return {
      ...
      details: files > 0 ? resultStr.split('\n').filter(f => f.trim()).slice(0, 3).join('\n') + 
  ...
  interpretRead(tool, params, result) {
    ...
    return {
      ...
      details: resultStr.includes('error>') ? 'File not found or permission denied' : null
    };
  }
  ...
  interpretBash(tool, params, result) {
    ...
    const resultStr = typeof result === 'string' ? result : JSON.stringify(result);
    const success = !resultStr.includes('error') && !resultStr.includes('Error') && !resultStr.includes('command not found');
    ...
    return {
      ...
      details: success ? resultStr.slice(0, 200) + (resultStr.length > 200 ? '...' : '') : resultStr
    };
  }
  ...
  interpretGrep(tool, params, result) {
    ...
    return {
      ...
      details: fileCount > 0 ? resultStr.split('\n').slice(0, 3).join('\n') + 
  ...
  genericSummary(tool, params, result) {
    ...
    return {
      ...
      details: resultStr.length > 100 ? 
        resultStr.slice(0, 100) + '...' : resultStr
    };
  }
}
