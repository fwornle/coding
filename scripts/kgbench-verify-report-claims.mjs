...
if (!existsSync(`${RUN}/report.json`)) {
  // Unlike results.jsonl this IS tracked, so absence is a defect rather than a fresh clone.
  process.stdout.write(`kgbench: ${RUN}/report.json is missing — it is tracked; restore it.\n`);
  process.exit(2);
}
const rows = readFileSync(`${RUN}/results.jsonl`, 'utf8').trim().split('\n').map((l) => JSON.parse(l));
const md = readFileSync('docs/benchmarks/coding-v1/README.md', 'utf8');
// THE RUN'S OWN report.json, not docs/benchmarks/coding-v1/report.json.
//
// The published report.json is regenerated by whatever run is published NEXT, and on
// 2026-08-19 it was: a 128-cell, 1-rep run at continuation budget 2 on corpus 7924e45bd
// replaced r9's there while this page still analysed r9. Four checks went red at once —
// tree commit, sandbox path count, disagreement count — reporting a page/data mismatch
// that did not exist. The page had not moved; the file underneath it had.
//
// Reading the run's own copy makes the comparison what it always claimed to be: THIS page
// against THE run it describes. It also means publishing a new run can no longer turn this
// checker red without touching either.
const report = JSON.parse(readFileSync(`${RUN}/report.json`, 'utf8'));
...
process.stdout.write('\n== agent axis table (PARSED OUT OF THE PAGE) ==\n');
// THESE EXPECTATIONS ARE READ FROM README.md, NOT TYPED HERE. That is the whole point.
//
// This block used to hold six hardcoded rows. They were correct — r9's, matching
// results.jsonl — and every one of them passed, while the table on the page had been r8's
// since it was written (defect 44). The checker compared its own constants to the data and
// never looked at the rendered table, so the page and the checker disagreed for two runs
// with nothing to notice it. A constant that is supposed to mirror the page has to be
// TAKEN from the page, or it is just a second copy that can drift independently.
const AGENT_ROW = /^\| *(grep|graphify|codegraph|hybrid) *\| *(claude|copilot|opencode) *\| *\*\*(\d+)\/(\d+)\*\* *\| *([\d.]+) *\| *([\d,]+) *\| *([\d.]+)s *\|$/gm;
const parsedRows = [...md.matchAll(AGENT_ROW)].map((m) => ({
  arm: m[1], agent: m[2], ranked: +m[3], runs: +m[4],
  score: +m[5], content: +m[6].replace(/,/g, ''), lat: +m[7],
}));
// A dropped row must fail too: without this, deleting a row from the page silently reduces
// the check count and everything left still passes.
check('agent-axis table row count', parsedRows.length, 6);
for (const r of parsedRows) {
  const tag = `${r.arm}/${r.agent}`;
  check(`${tag} ranked (page vs run)`, ok(r.arm, r.agent).length, r.ranked);
  check(`${tag} runs (page vs run)`, all(r.arm, r.agent).length, r.runs);
  check(`${tag} correctness (page vs run)`, Number(med(ok(r.arm, r.agent).map((x) => x.score)).toFixed(2)), r.score, 0.005);
  check(`${tag} content (page vs run)`, Math.round(med(ok(r.arm, r.agent).map((x) => x.content_tokens))), r.content, 1);
  check(`${tag} latency (page vs run)`, Number(med(ok(r.arm, r.agent).map((x) => x.wall_s)).toFixed(1)), r.lat, 0.05);
}

// The ratio sentence beneath the table, likewise read from the page. This is the sentence
// that carried r8's 1.85x/1.38x — the numbers a reader actually quotes.
const ratioSentence = md.match(/opencode costs \*\*([\d.]+)× claude's content tokens\*\* and copilot \*\*([\d.]+)×\*\*/);
check('page states both agent ratios', Boolean(ratioSentence), true);
if (ratioSentence) {
  const perGrep = (ag) => med(ok('grep', ag).map((x) => x.content_tokens)) / med(ok('grep', 'claude').map((x) => x.content_tokens));
  check('opencode ratio (page vs run)', Number(perGrep('opencode').toFixed(2)), Number(ratioSentence[1]), 0.005);
  check('copilot ratio (page vs run)', Number(perGrep('copilot').toFixed(2)), Number(ratioSentence[2]), 0.005);
}

// The comparison the section's heading now rests on: the arm spread must actually EXCEED the
// agent spread, or the rewritten claim is as wrong as the one it replaced.
const armSpread = med(ok('graphify', 'claude').map((x) => x.content_tokens)) / med(ok('grep', 'claude').map((x) => x.content_tokens));
const agentSpread = med(ok('grep', 'opencode').map((x) => x.content_tokens)) / med(ok('grep', 'claude').map((x) => x.content_tokens));
const spreadClaim = md.match(/across claude's four arms the spread is ([\d.]+)×/);
check('page states the arm spread', Boolean(spreadClaim), true);
if (spreadClaim) check('arm spread (page vs run)', Number(armSpread.toFixed(2)), Number(spreadClaim[1]), 0.005);
check('arm spread really exceeds agent spread', armSpread > agentSpread, true);
// The narrower survivor: the three non-graphify arms sit inside the agent spread.
const nonGraphify = ['grep', 'codegraph', 'hybrid'].map((a) => med(ok(a, 'claude').map((x) => x.content_tokens)));
const nonGraphifySpread = Math.max(...nonGraphify) / Math.min(...nonGraphify);
const tightClaim = md.match(/grep, codegraph and hybrid sit within ([\d.]+)× of each other/);
check('page states the non-graphify spread', Boolean(tightClaim), true);
if (tightClaim) check('non-graphify spread (page vs run)', Number(nonGraphifySpread.toFixed(2)), Number(tightClaim[1]), 0.005);
check('non-graphify arms really sit inside the agent spread', nonGraphifySpread < agentSpread, true);
...
claims('defect table has 44 rows', '| 44 |');
claims('says forty-four defects', 'Forty-four defects were found');
