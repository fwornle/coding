 ...
 * require it.
 *
 * The key has five components, each earning its place:
 ...
 *   reserve  — the cells status-left occupies. The renderer pads to
 *              (pane width - reserve), and reserve is derived from the tmux
 *              SESSION NAME, which the key carried no trace of. Two panes on
 *              one project/agent/width whose session names differ in length —
 *              `coding-claude-7871` vs `coding-claude-78715`, i.e. any two
 *              agents whose pids differ in digit count — therefore shared a
 *              cache entry and served each other a line padded for the other's
 *              status-left. A one-cell mismatch is exactly the leftover-digit
 *              residue ("15:322", "07:407") the padding exists to prevent, so
 *              this belongs in the key for the same reason width does.
...
/**
 * Cells tmux gives status-left, which the renderer subtracts from the pane
 * width before padding.
 *
 * status-left is `[#{session_name}] ` (see ~/.tmux.conf) capped at
 * status-left-length; both arrive as env from scripts/tmux-session-wrapper.sh.
 * No session name means no reservation — the historical full-width behaviour.
 *
 * Lives here rather than in the renderer because it is now a key component,
 * and a second copy of a width rule is the drift this file exists to prevent.
 *
 * @param {object} [env=process.env]
 * @returns {number} cells to reserve, 0 when the pane carries no session name
 */
function statusLeftReserveCells(env = process.env) {
  const name = env.TMUX_SESSION_NAME || '';
  if (!name) return 0;
  const cap = parseInt(env.TMUX_STATUS_LEFT_LENGTH || '40', 10) || 40;
  return Math.min(name.length + 3, cap);
}

/**
 * @param {object} [env=process.env] environment to read pane identity from
 * @returns {{projectPath:string, projectName:string, agent:string, paneWidth:string,
 *            reserve:number, features:string, suffix:string}}
...
function paneIdentity(env = process.env) {
  ...
  // 0 stays out of the filename so panes with no session name — and every
  // install that predates this component — keep the names they have always had.
  const reserve = statusLeftReserveCells(env);
  const features = featureFingerprint(env);

  const suffix = projectName
    ? `-${projectName}${agent ? `-${agent}` : ''}${paneWidth ? `-w${paneWidth}` : ''}${reserve ? `-r${reserve}` : ''}${features ? `-f${features}` : ''}`
    : '';

  return { projectPath, projectName, agent, paneWidth, reserve, features, suffix };
}
 ...
 *
 * @param {{paneWidth?: string, reserve?: number, features?: string}} [parts={}]
 * @returns {string} suffix to test candidate filenames with `endsWith`
 */
function borrowTail({ paneWidth = '', reserve = 0, features = '' } = {}) {
  return `${paneWidth ? `-w${paneWidth}` : ''}${reserve ? `-r${reserve}` : ''}${features ? `-f${features}` : ''}.txt`;
}

module.exports = { paneIdentity, featureFingerprint, statusLeftReserveCells, borrowTail };
