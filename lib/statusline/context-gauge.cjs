...
/**
 * Excludes the sessions a HEADLESS `opencode run` creates in this directory.
 *
 * `opencode run "say OK"` is not a pane and has no conversation, but it does
 * create a real top-level, non-archived session row in the cwd, stamped NOW —
 * so it clears every other filter above — and its prompt is a genuine user
 * message, so it also wins the ranking below outright. Measured, on the pane
 * that reported this: three probe runs fired from the coding directory while an
 * interactive session sat at 758,494 tokens, and the gauge followed each probe
 * in turn. Each probe's one assistant row carries all-zero tokens, so the read
 * fell through to OPENCODE_FRESH and the bar dropped from 72% to 0% on a
 * conversation that had lost nothing.
 *
 * This is the residual the ranking comment below names — "two opencode panes
 * started in the SAME directory are indistinguishable" — in its strongest form:
 * not a second pane, but no pane at all. And unlike two panes it is separable,
 * because `run` stamps the session it creates. From opencode 1.15.13's `run`
 * command, verbatim:
 *
 *   let n = D.interactive ? [] : [
 ...
 *   ...
 *   await b.session.create({ title: g, permission: [...n] })
 *
 * So `session.permission` is a creation-time override, and exactly this triple
 * is written by exactly one thing. Three consequences worth stating, because
 * each is a case this must NOT touch:
 *
 *   - The TUI supplies no override at all, so its rows are NULL.
 *   - `run --interactive` supplies `[]`, and that IS a pane. Requiring length 3
 *     keeps it.
 *   - `run --continue` / `--session` reuse an existing row rather than creating
 *     one, so a headless run against the pane's own session still reads as the
 *     pane — which is correct, it really did add to that conversation.
 *
 * WHY IT IS A SQL PREDICATE AND NOT A .filter() ON THE RESULT. The LIMIT is
 * applied before anything JS sees, and OPENCODE_SESSION_CANDIDATES is 5: on the
 * day this was reported, three probes plus two conversations already filled the
 * pool, and a filter downstream of the LIMIT would have thrown away the pane's
 * own session with them.
 *
 * The CASE, rather than a chain of ANDs, is what makes it total: json_*() on a
 * NULL, an empty string or a non-JSON value raises, and SQLite does not promise
 * to evaluate AND operands left to right. Only the taken CASE branch is
 * evaluated, so the cheap LIKE genuinely guards the JSON calls. Verified
 * against NULL, '', '[]', both key orderings of the real triple, and the
 * agent-level default permission list (which also denies plan_exit, and is
 * kept — it has more than three entries).
 *
 * Residual: a user who sets exactly these three denies, and nothing else, as a
 * session-creation override would be filtered out. No configuration path
 * observed does that — agent config lands in the agent's permission set, not
 * the session row.
 */
const OPENCODE_NOT_HEADLESS_RUN = `
      CASE WHEN permission LIKE '%"plan_exit"%' THEN
        NOT (
          json_array_length(permission) = 3
          AND (
            SELECT COUNT(*) FROM json_each(session.permission)
             WHERE json_extract(value, '$.action') = 'deny'
               AND json_extract(value, '$.permission')
                   IN ('question', 'plan_enter', 'plan_exit')
          ) = 3
        )
      ELSE 1 END`;
 ...
 *
 * THE FILTERS. Four ways the old `WHERE directory = ?` query read a session the
 * pane was not in. The first three were observed on one machine at once, and
 * together they are why a freshly restarted opencode rendered 94% red; the
 * fourth arrived later and drove a live 72% pane to 0%.
 *
 ...
 *
 *   NOT a headless `opencode run` — see OPENCODE_NOT_HEADLESS_RUN. A one-shot
 *     run creates a top-level session in the cwd stamped NOW, so it passes all
 *     three filters above, and its prompt is a user message, so it also wins the
 *     ranking below. Nothing else here can see it: it is not a subagent, not
 *     archived, and newer than pane start.
 *
 ...
 */
const OPENCODE_CANDIDATES_HEAD = `SELECT id, model, time_created FROM session
        WHERE directory = ?
          AND parent_id IS NULL
          AND time_archived IS NULL
          AND time_created >= ?`;
const OPENCODE_CANDIDATES_TAIL = `
        ORDER BY time_updated DESC
        LIMIT ?`;

/**
 * The candidate pool, with the headless-run rows removed where that is possible.
 *
 * The fallback is the same bargain the user-message ranking below makes, and it
 * is made for the same reason: `permission` is an original column of opencode's
 * `session` table and json_each has been compiled into SQLite for a decade, but
 * a status line must not go dark because one of them is missing on some
 * machine. A build without JSON1, or a schema predating the column, drops back
 * to the unfiltered pool — which is exactly today's behaviour, gauge included.
 * Never a thrown query, never a hole.
 */
function opencodeSessionCandidates(db, projectPath, since) {
  try {
    return db
      .prepare(
        `${OPENCODE_CANDIDATES_HEAD}
          AND ${OPENCODE_NOT_HEADLESS_RUN}${OPENCODE_CANDIDATES_TAIL}`
      )
      .all(projectPath, since, OPENCODE_SESSION_CANDIDATES);
  } catch {
    return db
      .prepare(OPENCODE_CANDIDATES_HEAD + OPENCODE_CANDIDATES_TAIL)
      .all(projectPath, since, OPENCODE_SESSION_CANDIDATES);
  }
}
...
function currentOpencodeSession(db, projectPath, tmuxSession) {
  ...
  const candidates = opencodeSessionCandidates(db, projectPath, since);
