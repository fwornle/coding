...
/**
 * Race a promise against a deadline, and CLEAR the deadline afterwards.
 *
 * `Promise.race([work, new Promise((_, reject) => setTimeout(reject, ms))])`
 * does not cancel the loser: when the work wins, the timer stays armed and
 * holds the event loop open for the rest of its duration. In a long-lived
 * service starter that is invisible; in any process that wants to exit — a
 * test runner, a one-shot CLI — it is a hang of exactly `ms`.
 *
...
async function startServiceWithRetry(serviceName, startFn, healthCheckFn, options = {}) {
  ...
  for (let attempt = 1; attempt <= maxRetries; attempt++) {
    try {
      ...
      serviceInfo = await withDeadline(
        startFn(),
        timeout,
        `Startup timeout after ${timeout}ms`
      );
