...
class CopilotHTTPServer {
    ...
    async stop() {
        ...
        if (this.wss) {
            // Per the ws docs, close() "prevents the server from accepting new
            // connections" but "existing connections are not automatically closed" —
            // and it closes the underlying HTTP server only if it created that server
            // internally, which it did not here (it was handed this.server). Every
            // still-open WebSocket therefore keeps its TCP socket alive, and
            // http.Server.close() waits for all connections to end, so without this
            // terminate loop the close below never calls back at all.
        ...
        if (this.server) {
            // Awaited, not returned. This used to be `return new Promise(...)`, which
            // exited stop() before the adapter cleanup below — so whenever a server
            // existed, i.e. always after start(), the adapter's memory, browser and
            // logging services were never cleaned up.
            //
            // The callback's error argument is ignored deliberately: close() reports
            // one only when the server was already closed, which is not actionable
            // during shutdown and must not reject a second stop().
            await new Promise((resolve) => this.server.close(() => resolve()));
        }
        
        // Guarded because stop() may run before start() ever assigned an adapter —
        // reachable for the first time now that the early return is gone.
        if (this.adapter) {
...
if (import.meta.url === new URL(process.argv[1], 'file://').href) {
    // An arrow const rather than a block-scoped function declaration: inside a block
    // the declaration's hoisting is mode-dependent, which is what no-inner-declarations
    // objects to. It is called immediately below, so nothing relies on hoisting.
    const main = async () => {
