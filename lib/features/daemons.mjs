...
/**
 * The uid launchd's GUI domain is keyed on — `gui/<uid>`.
 *
 * WHY THIS IS A FUNCTION AND NOT `process.getuid()` AT THE CALL SITE.
 * `process.getuid` DOES NOT EXIST ON WINDOWS. Reading it bare threw
 * `TypeError: process.getuid is not a function` and took out both launchd
 * contract tests on the windows-latest runner — the only two failures in that
 * job, red on every run of it in this repo's history.
 *
 * That is not a Windows bug in a macOS code path. It is a hole in the premise
 * this whole file is built on, stated in its own test header: every backend is
 * asserted on every platform, so a Linux regression is caught on a Mac. Building
 * a launchd command line must therefore not require the host to BE a Mac — the
 * `platform` seam above exists for exactly this reason, and the uid was the one
 * piece of ambient host state that had no such seam.
 *
 * `opts.uid` is that seam. The tests pin a literal through it, so the assertion
 * no longer depends on whatever uid the runner happens to have — which is also
 * what the test's own `process.getuid ? process.getuid() : 0` was papering
 * over: two sides independently guessing the same fallback is an agreement that
 * holds only until one of them changes.
 */
function guiUid(opts = {}) {
  if (Number.isInteger(opts.uid)) return opts.uid;
  if (typeof process.getuid !== 'function') {
    // Deliberately NOT `?? 0`, which is what this used to do. `gui/0` is ROOT's
    // launchd domain — booting a per-user LaunchAgent into it is not a benign
    // default, it is a different and privileged target. A fallback that exists
    // only so a test stops throwing, and that lies about where the command
    // lands, is worse than the throw it replaced. Unreachable in production:
    // this branch is only entered when platform() === 'macos', and every macOS
    // host has process.getuid.
    throw new Error(
      'launchd: cannot resolve a uid — process.getuid() does not exist on this '
      + 'platform. Pass opts.uid to build a launchd command line off a non-POSIX host.'
    );
  }
...
/**
 * Home directory, with the same seam as `platform`, `exec`, `exists` and `uid`.
 *
 * The last piece of ambient host state these path builders read. Without it a
 * contract test cannot assert a whole command line — only a regex with a `.*`
 * where the path should be, which is exactly what the launchd assertion had to
 * settle for, and exactly where a wrong path would hide.
 */
...
/**
 * launchd and systemd paths are POSIX paths. ALWAYS — including when this code
 * is running on Windows.
 *
 * These used to be built with `join` from 'node:path', which on Windows IS
 * `path.win32.join`. So on the windows-latest runner the launchd bootstrap
 * argument came out as
 *
 *   C:\Users\runneradmin\Library\LaunchAgents\com.coding.obs-api.plist
 *
 * — backslashes, and a drive letter, in a launchctl command line. Neither
 * service manager can exist on a host where that is the right separator:
 * launchd is macOS-only and systemd --user is Linux-only, so the host's
 * separator is never the correct one to use here. `posix.join` is not a
 * portability concession, it is the accurate joiner for both targets.
 *
 * On macOS and Linux this changes nothing at all — homedir() is already a POSIX
 * path there and posix.join and win32.join agree on it. What it changes is that
 * the command line is now identical on every host, which is the property the
 * contract tests are for.
 */
const plistPath = (name, opts) =>
  posixPath.join(homeDir(opts), 'Library', 'LaunchAgents', `${label(name)}.plist`);
const unitPath = (name, opts) =>
  posixPath.join(homeDir(opts), '.config', 'systemd', 'user', `${name}.service`);
...
export async function isInstalled(name, opts = {}) {
  ...
  switch (platform(opts)) {
    case 'macos':
      return exists(plistPath(name, opts));
    case 'linux':
      return exists(unitPath(name, opts));
    case 'windows': {
...
export async function stop(name, opts = {}) {
  ...
  switch (platform(opts)) {
    case 'macos': {
      const target = `gui/${guiUid(opts)}/${label(name)}`;
...
export async function start(name, opts = {}) {
  ...
  switch (platform(opts)) {
    case 'macos': {
      const domain = `gui/${guiUid(opts)}`;
      await run('launchctl', ['enable', `${domain}/${label(name)}`], opts);
      const boot = await run('launchctl', ['bootstrap', domain, plistPath(name, opts)], opts);
