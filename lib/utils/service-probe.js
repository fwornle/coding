...
/**
 * Sentinel `error` value meaning "the target accepted the connection but did
 * not answer inside the deadline" — alive, but not responding right now.
 *
 * This is materially different from a refused connection: a dead process has
 * no listening socket, so connect() fails instantly with ECONNREFUSED, while a
 * process whose event loop is blocked still has the kernel accept into its
 * listen backlog and simply never replies. The coordinator relies on that
 * distinction to reclassify a blocked-but-alive service as 'busy' rather than
 * 'stopped' (see health-coordinator.js reclassifyBusyService). Do not rename
 * or reword this value without updating that call site.
 */
export const PROBE_TIMEOUT_ERROR = 'timeout';
...
export async function probeTcpPort(host, port, timeoutMs = 2000) {
  ...
  return new Promise((resolve) => {
    ...
    sock.once('timeout', () => settle('stopped', PROBE_TIMEOUT_ERROR));
