    ...
    try {
      ...
      for (const team of this.teams) {
        ...
        const entities = graphEntities
          ...
          .map(entity => {
            // 2026-06-11: km-core entities store the source field under
            // `metadata.source` (set by ObservationWriter for online/auto and
            // by the manual import path for batch). The legacy SQLite shape
            // had a top-level `source` column. Read both — metadata wins —
            // so online-learned entities actually map to 'online' (red) in
            // the viewer instead of defaulting to 'batch' (blue/Manual badge).
            const rawSource = (entity.metadata && entity.metadata.source) || entity.source;
            const frontendSource = rawSource === 'auto' ? 'online' : 'batch';
            this.logger.info(`[DEBUG] Transforming entity "${entity.entity_name}": rawSource="${rawSource}" → frontend source="${frontendSource}"`);
            return {
    ...
    for (const entity of entities) {
      ...
      if (entity.observations && Array.isArray(entity.observations)) {
        for (const obs of entity.observations) {
          ...
          for (const [otherName] of entityNameMap.entries()) {
