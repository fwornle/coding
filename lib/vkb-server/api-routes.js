...
import { registerKgbenchRoutes } from './kgbench-routes.js';
...
export class ApiRoutes {
  ...
  registerRoutes(app) {
    ...
    // kgbench Benchmarks family (/api/kgbench/*) — the retrieval benchmark's control surface
    // behind Performance → Benchmarks. Registered from its own module because the family is
    // self-contained; it shares only the repo root and the coordinator POST helper with the
    // experiment handlers. Same split of responsibilities: reads are pure file+compute in this
    // container, launch/cancel/probe DELEGATE to the host coordinator (the agent CLIs a
    // benchmark cell spawns do not exist here).
    registerKgbenchRoutes(app, {
      repoRoot: () => this._repoRoot(),
      coordinatorPost: (seamPath, body) => this._coordinatorPost(seamPath, body),
      logger,
    });
