...
export class ApiRoutes {
  ...
  async handleExperimentRun(req, res) {
    try {
      ...
      // 409 source #1 — an interactive measurement span holds the slot.
      ...
      // traversal segment (never a listed basename).
      ...
      if (!specFiles.includes(spec)) {
        return res.status(400).json({
          error: 'Unlisted spec',
          message: `spec '${spec}' is not one of the server-listed experiment specs (${specFiles.join(', ') || 'none'}).`,
        });
      }
      const specPath = path.join(this._repoRoot(), 'config', 'experiments', spec);
      ...
      const { ok, json } = await this._coordinatorPost('/experiments/run', {
        spec, run_id: runId, run_dir: runDir, overrides: ov,
      });
      ...
      return res.status(200).json({ run_id: runId, pid: json.pid, run_dir: runDir });
    } catch (error) {
  ...
  async handleRunCancel(req, res) {
    try {
      ...
      const { ok, json } = await this._coordinatorPost('/experiments/cancel', {
        run_id, run_dir: runDir, pid: runJson.pid,
      });
      if (!ok || json.success === false) {
        return res.status(502).json({
