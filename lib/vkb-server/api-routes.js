...
export class ApiRoutes {
  ...
  async handleTimeline(req, res) {
    try {
      ...
      // wall-clock window, NON-attributed. Window is derived from the run's own
      // foreground timeline min/max timestamp (the run's rows are the only wall-clock
      // anchor this handler has — it must not open the experiment LevelDB). Empty
      // timeline -> no window -> readAmbientBackground returns [] gracefully.
      const stamps = [];
      ...
      stamps.sort();
      const ambient = stamps.length
        ? await readAmbientBackground(taskId, stamps[0], stamps[stamps.length - 1])
        : [];
