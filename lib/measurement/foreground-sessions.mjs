 ...
 *
 * NOT newestMatchingFile: the state root holds one dir PER SESSION EVER RUN
 * (thousands on a long-lived machine), and the generic BFS's MAX_SCAN_ENTRIES
 * cap exhausts in readdir order long before covering them all — whether a LIVE
  ...
  for (const root of Array.isArray(dirs) ? dirs : []) {
    ...
    for (const entry of readDirSafe(root)) {
      ...
      try {
        ...
        if (!best || st.mtimeMs > best.mtimeMs) best = { filePath, mtimeMs: st.mtimeMs };
      } catch { /* session dir without events.jsonl — skip */ }
    }
  }
  if (!best) return null;
  const sessionId = path.basename(path.dirname(best.filePath));
  if (!sessionId) return null;
  return { agent: 'copilot', sessionId, lastActivityMs: best.mtimeMs };
}
...
export function detectPi() {
  ...
  for (const entry of readDirSafe(root)) {
    ...
    try {
      const st = fs.statSync(filePath);
      if (!best || st.mtimeMs > best.mtimeMs) best = { name: entry.name, mtimeMs: st.mtimeMs };
    } catch { /* vanished between readdir and stat — skip */ }
  }
  if (!best) return null;

  const base = best.name.replace(/\.jsonl$/, '');
  const sep = base.lastIndexOf('_');
  const sessionId = sep >= 0 ? base.slice(sep + 1) : base;
  if (!sessionId) return null;
  return { agent: 'pi', sessionId, lastActivityMs: best.mtimeMs };
}
