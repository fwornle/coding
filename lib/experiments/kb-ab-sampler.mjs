...
export function selectFacts(candidates, signals) {
  ...
  for (const c of candidates) {
    const s = signals.get(c.id) ?? { injected: false, coined: false, inSandbox: false };
    if (!s.injected) {
...
export async function deriveTask(insight, probes) {
  ...
  // NO REFERENCE FILTER. It asked a model to write the ideal runbook from the insight and kept
  // facts appearing in all of them — a stand-in for "what a correct deliverable contains". Measured
  // on the first full pilot, the stand-in was wrong in the direction that matters: a reference is
  // written with no sandbox, no tools, no execution directive and no length pressure, while a cell
  // writes under all four, so conjunctions every reference satisfied were still jointly
  // unsatisfiable by real cells. Seven of eight tasks landed in `neither-solves`.
  //
  // scripts/kb-ab-mine-facts.mjs now answers the same question from REAL kb-on deliverables, which
  // is the condition the filter was approximating. Keeping both would cost calls to reject tasks
  // mining rescues: BOTH of the pilot's exclusions were facts the reference filter dropped.
  const coinage = injectedIds.size ? await probes.coinage(goal) : [];
  ...
  for (const c of candidates) {
    ...
    signals.set(c.id, {
      injected,
      coined: injected && coinage.some((text) => re.test(text)),
      inSandbox: injected ? await probes.inSandbox(c) : false,
    });
  }
