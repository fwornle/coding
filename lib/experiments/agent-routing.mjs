 ...
 *   • pi        → TASK_ID=<taskId>. pi's models.json provider carries
 *                 `"x-task-id": "$TASK_ID"` and interpolates it from its own process env at
 *                 config load, so exporting the var IS the per-request binding — no base-URL
 *                 or config splice needed. Verified live.
 *   • unknown   → no change.
 *
...
export function buildAgentRoutingEnv(agent, baseEnv, {
  ...
  switch (agent) {
    ...
    case 'pi':
      // pi is self-routed through the models.json custom provider pi.sh writes, so there is
      // no base URL to set here. The binding seam is the environment: that provider declares
      // `"x-task-id": "$TASK_ID"`, and pi resolves $VAR in header values from its own process
      // environment at config load. Because the runner spawns one pi process per cell, setting
      // TASK_ID here binds every request that cell makes — which is precisely why pi is NOT in
      // AMBIENT_BOUND_AGENTS, unlike the mastracode it replaces.
      if (taskId) env.TASK_ID = taskId;
      break;
    default:
      // Any future agent with no launcher-controlled seam → ambient-bound. No env change.
      break;
  }
  return env;
}
