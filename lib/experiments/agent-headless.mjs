 ...
 * @param {string} agentName  one of claude | opencode | pi | copilot
...
export function argvForAgent(agentName, goal, { model } = {}) {
  ...
  // Phase 91 (headless narration-termination fix): an ANALYSIS-shaped goal
  // ("normalize X so that…") makes single-turn headless CLIs — copilot `-p`,
  // opencode `run` — emit a PLAN as their first assistant message ("I'll explore
  // the code…") with no tool call, at which point they consider the prompt
  // "complete" and exit having written nothing (copilot 6s/165 out-tokens;
  // opencode yields on the first toolless step). claude `-p` runs the full agentic
  // loop so it pushes past it. We append a uniform EXECUTION directive to EVERY
  // agent's prompt (symmetric — claude tolerates it, so the cross-agent comparison
  // stays unbiased) so the model reaches for its edit tools on turn 1. This
  // augments ONLY the CLI prompt; the measurement span's goal_sentence — hence the
  // task_hash — is set separately and is unchanged. See memory
  // reference_experiment_headless_termination_rootcause.
  ...
  const execGoal = `${goal}${EXECUTION_DIRECTIVE}`;
  switch (agentName) {
    case 'claude':
      // headless edits REQUIRE --permission-mode acceptEdits (Pitfall 6)
      return ['-p', execGoal, '--model', model, '--permission-mode', 'acceptEdits'];
    case 'opencode':
      ...
      // Its single-turn `run` still ends on a toolless step, so the EXECUTION directive above is the
      // real lever for convergence (no loop-until-done flag exists).
      return ['run', execGoal, '-m', model, '--dangerously-skip-permissions'];
    case 'pi':
      // --approve is REQUIRED for non-interactive runs (Pitfall 6, the pi analogue of
      // claude's --permission-mode acceptEdits). Without it pi blocks on a project-trust
      // prompt it cannot answer headlessly and hangs until the wall-clock SIGKILL —
      // observed directly during this phase: a bare `pi -p` sat for the full 120s timeout
      // having produced no output and made no LLM call at all.
      // --provider pins the proxy-routed custom provider from pi.sh's models.json, so the
      // cell cannot silently fall through to a provider pi found its own credential for.
      // NO --dir; cwd is set via the spawn cwd option by the caller (78-03).
      return ['-p', execGoal, '--provider', 'rapid-proxy-pi', '--model', model, '--approve'];
    case 'copilot':
      // --allow-all-tools REQUIRED for non-interactive. Plain `-p` is single-turn ("exits after
      // completion"), so on a toolless first turn it quits in ~6s having done nothing. Run it in
      // AUTOPILOT so a yielded turn auto-continues instead of exiting; --no-ask-user keeps it from
      // blocking on the ask_user tool headlessly; --max-autopilot-continues raises the loop budget.
      return ['-p', execGoal, '--allow-all-tools', '--no-ask-user',
 ...
 *   • unknown (out of scope)               → { ok: true } (no call)
 ...
 * @param {string} agent  one of copilot | claude | opencode | pi (unknown → ok, no call).
 * @param {object} [opts]
...
export async function preflightAgent(agent, {
  ...
  // Out-of-scope agent (unknown) — nothing to validate, never a false skip.
