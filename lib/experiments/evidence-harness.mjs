...
function runTestCommand(argv, repoRoot) {
  ...
  const [cmd, ...args] = argv;
  // The acceptance command belongs to the EXPERIMENT, not to the artifact under test. A cell runs
  // with cwd = its restored sandbox, so a repo-relative script path resolves inside that sandbox —
  // and the sandbox is a snapshot of an older commit that does not contain it. The gate then exits
  // non-zero for BOTH arms and the cell measures nothing (a floor effect that looks exactly like
  // "the agent failed"). Resolve a `node <script>` gate against THIS repo when the cell's own tree
  // has no such file.
  //
  // Deliberately NOT solved by copying the checker into the sandbox: the fact patterns it asserts
  // literally spell out the answers, so a readable checker would hand the graded content to the
  // agent — the same leak class as the restored KB corpus. Keeping it outside the sandbox is what
  // makes it safe to grade with.
  //
  // Narrow by construction: only when cmd is `node`, only when the script path is relative, and
  // only when it is absent from the cell but present here. `grep -q -F <pattern> <file>` gates are
  // untouched — their operand is a deliverable that must exist in the cell.
  if (cmd === 'node' && typeof args[0] === 'string' && !path.isAbsolute(args[0])) {
    const inCell = path.join(repoRoot, args[0]);
    const inRepo = path.join(REPO_ROOT, args[0]);
    if (!fs.existsSync(inCell) && fs.existsSync(inRepo)) args[0] = inRepo;
  }
  ...
  try {
    ...
    //
    // `command` is carried so the judge can tell WHAT passing means. A bare
    // `status: 0` is ambiguous — "the tests passed" reads very differently from
    // "grep -q -F persistOnClose <file> passed", and only the latter tells the judge
    // the produced artifact actually contains the required content. It is the spec's
    // own fixed argv (already shell-metachar-rejected by resolveTestCommand), so this
    // leaks no tool inputs or file contents (T-73-04-LEAK).
    return {
      command: argv.join(' '),
