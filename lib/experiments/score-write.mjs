 ...
 *   `gate_passed` is the OBJECTIVE per-cell test-gate (CMP-01 / D-04a), threaded by
 *   the close orchestrator from the evidence harness; absent ⇒ null (ungated, D-02).
...
export async function writeScore(store, { span, judgment }) {
  ...
  const scoreId = await store.putEntity({
    ...
    metadata: {
      ...
      //    from the subjective rubric above (D-04). true = test exited 0; false = non-zero;
      //    null = no test_command (ungated per D-02). A JUDGED field — overwritten on
      //    re-judge like the rubric dims (sits here, NOT in the corrected_* block).
      //    NEVER `?? false` — null-not-zero: an ungated run must never masquerade as a
      //    gate failure downstream (Phase 79 aggregator, Plan 02). ──
