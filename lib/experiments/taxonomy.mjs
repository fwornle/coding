...
export function loadTaxonomy(filePath = DEFAULT_TAXONOMY_PATH) {
  ...
  // Null-guard (WR-03): yaml.load() returns undefined for an empty file and null
  // for an explicit YAML null document; a malformed file may also lack `classes`.
  // Throw a CLEAR error instead of an opaque "Cannot read properties of undefined".
...
export function deriveClassFromText(text, taxonomy) {
  ...
  let best = null;
  let bestScore = 0;
  // Tolerate a taxonomy with no classes (WR-03): no crash, just no derivation.
  ...
  // Confidence threshold (WR-04): a LONE bare-token hit (score 1) is too weak to
  // auto-accept headless — common words like "add"/"fix"/"new" match accidentally.
  // Require bestScore >= 2: either a hyphenated-keyword substring hit (+2) or two
  // bare-token hits (+1 each). A single bare token derives a class but is NOT
  // confident, so the headless path quarantines it (D-06) instead of auto-accepting.
