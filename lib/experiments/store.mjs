 ...
 *
 * @param {object} [opts]
 * @param {string} [opts.repoRoot] - Explicit repo root override. Defaults to
 *   `process.env.CODING_REPO` then the canonical checkout path. An explicit arg
 *   lets tests open an isolated throwaway store/ontology without mutating the
 *   shared `process.env.CODING_REPO` (which races under concurrent test runs).
 * @param {boolean} [opts.readOnly] - Open for reading only: `close()` will NOT
 *   write the graph snapshot back to LevelDB. Default false (writers unchanged).
 *
 *   MANDATORY for anything on a polled HTTP route. km-core's `persistGraph` is a
 *   single `db.put('graph:state', JSON.stringify(graph))`, so an ordinary
 *   open/read/close still rewrites the WHOLE graph as one ~8 MB value. The
 *   dashboard polls `/api/experiments/runs` every 30s (performance.tsx:173);
 *   each poll appended one ~1 MB SST until the store reached 4,821 files /
 *   4.6 GB backing a graph that exports to 8.3 MB, at which point `open()`
 *   alone blew past the container's 4 GiB cgroup limit and the kernel
 *   OOM-killed vkb-server on every poll — 175 kills, and each SIGKILL is an
 *   unclean close, so compaction never caught up. Reads must not write.
 * @returns {Promise<import('@fwornle/km-core').GraphKMStore>} the opened store
 */
export async function openExperimentStore(opts = {}) {
  const repoRoot = opts.repoRoot || REPO;
  const dbPath = path.join(repoRoot, '.data', 'experiments', 'leveldb');
  const store = new GraphKMStore({
    dbPath,
    exportDir:   path.join(repoRoot, '.data', 'experiments', 'exports'),
    ontologyDir: path.join(repoRoot, '.data', 'ontologies-experiment'),
    ontologyStrict: false,
    debounceMs: 0,
    domains: ['experiment'],
    persistOnClose: !opts.readOnly,
  });
  await store.open();
  process.stderr.write(`[experiments] opened store dbPath=${dbPath}\n`);
  return store;
}
