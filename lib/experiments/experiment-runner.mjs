...
export async function configureProxyRoutingEnv(agent, baseEnv, {
  ...
  switch (agent) {
    case 'opencode': {
      ...
      // Phase 84: per-request task binding for opencode measured cells (parity with claude's
      //   x-task-id header). Without this, opencode capture is AMBIENT-span-bound (resolveLiveTaskId)
      //   → flaky/empty context-turn capture ("works in claude, broken in opencode"). Opencode may
      //   use EITHER wire depending on model→provider, so we bind BOTH via an OPENCODE_CONFIG_CONTENT
      //   provider splice (opencode's single config seam; see config/agents/opencode.sh):
      //     • anthropic provider  → proxy /v1 (the @ai-sdk/anthropic client appends /messages →
      //       /v1/messages tap) + x-task-id/x-agent:opencode headers.                        [seam A]
      //     • openai/copilot providers → task-scoped base-URL PATH /v1/opencode/t/<taskId>
      //       (the openai client appends /chat/completions → matches the SHIM task-scoped path
      //       accepted at server.mjs; the proxy derives task_id from the <taskId> segment). [seam B]
      //   x-task-id/x-agent headers are ALSO set on the openai/copilot providers as belt-and-suspenders
      //   (highest precedence if opencode forwards custom headers; the path binds otherwise).
      //   No taskId → env is byte-identical to the prior behaviour (ANTHROPIC_BASE_URL only).
      if (taskId) {
        const scoped = `${base}/v1/opencode/t/${encodeURIComponent(taskId)}`;
        const hdr = { 'x-task-id': taskId, 'x-agent': 'opencode' };
        const provider = {
          anthropic: { options: { baseURL: `${base}/v1`, headers: hdr } },
          'github-copilot-enterprise': { options: { baseURL: scoped, headers: hdr } },
          'github-copilot': { options: { baseURL: scoped, headers: hdr } },
          openai: { options: { baseURL: scoped, headers: hdr } },
        };
        let cfg = {};
