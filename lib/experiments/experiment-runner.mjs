...
export async function runMatrix(spec, opts = {}) {
  ...
  const summary = [];
  // Parallel-mode locks (NO_LOCK pass-throughs when serial — runOne is mode-agnostic).
  const setupLock = parallel ? makeMutex() : NO_LOCK;
  const storeLock = parallel ? makeMutex() : NO_LOCK;

  // The per-(cell, rep) body — extracted VERBATIM from the former sequential loop so both
  // modes share one code path. `continue` became `return`.
  const runOne = async (cell, rep) => {
    const variant = cellName(cell);
    {
      const taskId = composeTaskId(expId, cell, rep);
      ...
      if (pf && pf.ok === false) {
        ...
        try {
          ...
          await storeLock.run(() => writeSkipRun({ cell, rep, expId, goal, taskClass, skipReason, parallel, runMeasurement }));
        } catch (err) {
  ...
  if (!parallel) {
    // (c) STRICTLY sequential (D-09 unchanged): await each item in matrix order.
    for (const cell of cells) {
      for (let rep = 0; rep < repeats; rep += 1) {
        await runOne(cell, rep);
      }
    }
  } else {
    // Bounded worker pool over the flat (cell, rep) list. runOne never throws (its runCell is
    // wrapped in try/catch, D-12), so a worker chain never rejects; the single JS event loop
    // makes the shared `next`/`done`/`summary` mutations safe.
    const items = [];
    ...
    const workers = Array.from({ length: Math.min(Math.max(1, maxParallel), items.length) }, async () => {
      for (;;) {
        ...
        await runOne(items[i].cell, items[i].rep);
      }
    });
