...
import { restoreForCell, neutralizeSandboxRules, neutralizeSandboxKnowledge } from './experiment-restore.mjs';
...
import { injectCellKnowledge } from './cell-injection.mjs';
 ...
 * wire), pi via TASK_ID into its models.json x-task-id header, copilot via the BYOK
 * /v1/copilot/t/<taskId> base-URL
 ...
 *   • pi        → TASK_ID=<taskId>; pi interpolates it into its models.json x-task-id header
 *                 per process, so each cell is per-request bound (NOT ambient).
 ...
 */
export function runMeasurementCli(phase, argv, { env, cwd, spawn = realSpawn, stdio = 'inherit' } = {}) {
...
export async function runCell({
  ...
  // Parallel matrix mode: slotless spans + serialized setup/store phases. Serial callers omit
  // all three → NO_LOCK pass-throughs keep the control flow byte-identical.
  parallel = false,
  setupLock = NO_LOCK,
  storeLock = NO_LOCK,
  ...
  // Per-cell log: every cell gets its own append-only log at <runDir>/cells/<taskId>.log — the
  // live surface for the dashboard mini-terminal view AND the fix for parallel-mode interleaving
  // (stdio:'inherit' would interleave N agents into one runner.log). Fail-soft: an open failure
  // falls back to 'inherit'. Serial runs get per-cell logs too (the viewer works in both modes).
  let cellLogFd = null;
  if (runDir) {
    try {
      const cellsDir = path.join(runDir, 'cells');
      fs.mkdirSync(cellsDir, { recursive: true });
      cellLogFd = fs.openSync(path.join(cellsDir, `${sanitizeTaskId(taskId)}.log`), 'a');
    } catch (err) {
      process.stderr.write(`[experiment-runner] cell log open failed (non-fatal): ${err.message}\n`);
    }
  }
  const cellStdio = cellLogFd != null ? ['ignore', cellLogFd, cellLogFd] : 'inherit';

  // PARALLEL setup gate: restore → rules-strip → baseline commit → measurement-start run under
  // the setup mutex — restoreForCell's `git worktree add` + `submodule update` against the SHARED
  // repo .git is not concurrency-safe (lock contention, same-ms worktree-name collisions). The
  // agent launch itself (minutes) runs OUTSIDE the lock; restores are seconds. Serial mode:
  // setupLock is the NO_LOCK pass-through — control flow identical.
  let releaseSetupLock = () => {};
  await new Promise((acquired) => {
    setupLock.run(() => {
      acquired();
      return new Promise((resolve) => { releaseSetupLock = resolve; });
    });
  });
  ...
  // active-measurement.json from (Pitfall 1); aggregateByTaskId also reads the MAIN token-usage.db.
  ...
  ({ worktree, sandboxDataDir } = await restore(snapshotId, restoreOpts));
  ...
  // (1a-ii) Knowledge isolation: strip the restored KB corpus and the pointer to the host's agent
  //      memory dir. `kb-off` sets CODING_KNOWLEDGE_INJECTION=0, which kills the injection SEAM but
  //      not the knowledge — the restored tree still ships the 45 MB graph export, 989 insight files,
  //      and a .claude/settings.local.json allow-list naming the memory dir by absolute path. On the
  //      2026-08-22 A/B that pointer was followed by 13 of 18 cells (first tool call, in one case),
  //      which is why both arms scored 3/3 and the comparison carried no information. Runs on BOTH
  //      arms — isolation applied to one arm only would be the confound it removes. Safe for kb-on:
  //      cell-injection.mjs retrieves from the LIVE obs-api, never from this tree.
  const strippedKnowledge = neutralizeSandboxKnowledge(worktree);
  if (strippedKnowledge.length) {
    process.stderr.write(`[experiments] sandbox knowledge neutralized (${taskId}): ${strippedKnowledge.join(', ')}\n`);
  }
  ...
  escapeBaseline = captureRepoState(realRepoRoot);
  ...
  // (spanEnv/mainDataDir are declared ABOVE the setup gate — shared by start here and stop in the finally.)
  ...
  const startArgv = [
    ...
    // Parallel mode: slotless span (per-cell pending file — the global slot would cross-archive
    // under concurrency) + execution-mode provenance (dashboard gates the shared-background
    // overlay + disclaimer on 'parallel').
    ...(parallel ? ['--no-slot'] : []),
    '--execution-mode', parallel ? 'parallel' : 'serial',
    '--goal', goal,
  ];
  await runMeasurement('start', startArgv, { env: spanEnv, stdio: cellStdio });
  } catch (err) {
    // Setup failed before the launch try/finally exists — release resources, then rethrow so
    // runMatrix's catch records the abort (D-12 unchanged).
    if (cellLogFd != null) { try { fs.closeSync(cellLogFd); } catch { /* best-effort */ } }
    throw err;
  } finally {
    releaseSetupLock();
  }
  ...
  // Drop every inherited variable that names the LIVE repo by absolute path. Measured
  // 2026-08-23: this alone does NOT stop the memory-index leak (that is the git-identity channel,
  // fixed by restoring cells into an independent clone — see experiment-restore.restoreForCell),
  // so do not read this as the isolation fix. It is defence-in-depth for the OTHER direction:
  // these point a cell's own tooling — hooks, adapters, the SQLite path, the KB export — at live
  // state, so a cell that shells out could read or write the real project through them. Scrubbed
  // for BOTH arms, so it can never become an asymmetry between kb-on and kb-off.
  for (const key of [
    'CODING_REPO', 'CODING_PROJECT_DIR', 'CODING_KNOWLEDGE_BASE', 'CODING_KNOWLEDGE_EXPORT',
    'CODING_HOOKS_CONFIG', 'CODING_TOOLS_PATH', 'CODING_AGENT_ADAPTER_PATH',
    'SQLITE_PATH', 'TRANSCRIPT_SOURCE_PROJECT',
  ]) {
    delete agentBaseEnv[key];
  }
  ...
  // UserPromptSubmit hook and the bash injector for opencode/copilot) early-return when it
  // is '0'.
  //
  ...
  // Injection-quality gate (2026-07-23): the composite task_id ('<exp>--<variant>--rN') is
  // otherwise threaded ONLY via the proxy x-task-id header, so the injection hooks never see it
  // (they fall back to the Claude Code session UUID) and cannot tell they are inside an experiment
  // cell. Surface it explicitly so every seam sends it as the retrieval task_id — this both keys
  // the Phase-B capture to the run AND lets the retrieval service detect the experiment cell
  // (taskId contains '--') to suppress the task-agnostic Working Memory scaffold and inject only
  // curated, relevant know-how (retrieval-service.js). Set for ALL cells; harmless for kb-off
  // cells (injection is disabled there anyway).
  agentBaseEnv.CODING_EXPERIMENT_TASK_ID = taskId;
  // Also surface the goal text so the file-adapter seams (opencode/copilot), which
  // otherwise retrieve on a generic "project context for <project>" query, can retrieve on — and
  // have the LLM judge assess against — the ACTUAL task. The claude hook already uses the real
  // prompt as its query, so it needs nothing here. Agent env only (never the span).
  if (goal) agentBaseEnv.CODING_EXPERIMENT_GOAL = String(goal);
  ...
  try {
    ...
    let argv = argvForAgent(cell.agent, goal, { model: launchModel });
    // kb-on axis: the runner launches raw agent binaries (no launch-agent-common.sh injector) and
    // `claude -p` does not fire the UserPromptSubmit hook, so cells get NO live injection unless we
    // do it here. For kb-on cells, fetch the GATED knowledge (obs-api /api/retrieve — IDF + LLM
    // judge + WM-suppression + fail-closed, keyed by the composite task_id) and inject it via the
    // agent's native channel: claude/pi → --append-system-prompt; opencode/copilot → a context
    // file in the sandbox cwd. Done AFTER the sandbox rules-strip (above) so it survives to launch.
    // A trivial task (fizzbuzz) retrieves nothing → nothing injected. Fail-open (never blocks).
    if (cell.env === 'kb-on') {
      try {
        const inj = await injectCellKnowledge({ agent: cell.agent, goal, taskId, worktree });
        if (inj.argvExtra.length) argv = [...argv, ...inj.argvExtra];
        process.stderr.write(
          `[experiments] kb-on injection (${cell.agent} ${taskId}): ${inj.injectedChars} chars` +
          `${inj.target ? ` → ${inj.target}` : ' → (nothing)'}\n`,
        );
      } catch (e) {
        process.stderr.write(`[experiments] kb-on injection failed (fail-open): ${e.message}\n`);
      }
    }
    const result = await spawnAgent({ bin, argv, worktree, sandboxDataDir, env: agentEnv, timeoutMs, stdio: cellStdio });
    ...
    const stopArgv = [
      ...
      // Parallel mode: close the slotless per-cell pending span (never the global slot).
      ...(parallel ? ['--no-slot', '--task-id', taskId] : []),
    ];
    // storeLock: measurement-stop reopens the single-owner LevelDB store (writeRun/writeScore) —
    // concurrent closes must serialize. Serial mode: NO_LOCK pass-through.
    await storeLock.run(() => runMeasurement('stop', stopArgv, { env: spanEnv, stdio: cellStdio }));
...
/**
 * Promise-chain mutex for the PARALLEL matrix mode. Serializes the two operations that are
 * NOT concurrency-safe while cells otherwise run in parallel:
 *   • setupLock — restoreForCell runs `git worktree add` + `submodule update` against the
 *     SHARED repo/.git (lock contention + same-millisecond worktree-name collisions);
 *   • storeLock — measurement-stop reopens the single-owner LevelDB experiment store.
 * `run(fn)` returns fn's own promise (rejections propagate to the caller); the internal
 * chain swallows them so one failed job never wedges the queue.
 */
export function makeMutex() {
  let chain = Promise.resolve();
  return {
    run(fn) {
      const res = chain.then(() => fn());
...
/** Pass-through lock used in serial mode — keeps runCell's control flow identical. */
const NO_LOCK = { run: (fn) => fn() };
 ...
 */
async function writeSkipRun({ cell, rep, expId, goal, taskClass, skipReason, parallel = false, runMeasurement = runMeasurementCli }) {
  ...
  const startArgv = [
    ...
    '--framework', cell.framework,
    ...(cell.test_command ? ['--test-command', cell.test_command] : []),
    // Parallel mode: slotless — a skip-Run's open/close must not clobber a live cell's slot.
    ...(parallel ? ['--no-slot'] : []),
    '--execution-mode', parallel ? 'parallel' : 'serial',
    '--goal', goal,
  ];
...
export async function runMatrix(spec, opts = {}) {
  const {
    ...
    // Parallel mode (opt-in): run (cell, rep) items through a bounded worker pool instead of
    // the strictly-sequential loop. Setup (git worktree restore) and store writes (measurement
    // close) stay serialized via mutexes; everything else overlaps. Default OFF — serial path
    // is byte-identical.
    parallel = false, maxParallel = 4,
  ...
  let done = 0;
  const executionMode = parallel ? 'parallel' : 'serial';
  await emitProgress(runDir, { done, total, overall: 'running', execution_mode: executionMode });
  // Ambient-bound agents lack per-request cell binding: their proxy rows carry the agent's
  // OWN session id (opencode `rapid-proxy` provider → ses_…),
  // and SERIAL mode only attributes them via the global active-measurement.json slot — which
  // parallel (slotless) mode deliberately removes. So in parallel mode these cells measure ~0
  // tokens even though the agent ran fine. claude (x-task-id header) and copilot (task-scoped
  // adapter) ARE per-request bound and unaffected. See lib/experiments/agent-routing.mjs.
  // pi is deliberately ABSENT from this set, where mastracode used to sit. mastracode had no
  // way to attach a per-request identity at all; pi's provider sends `x-task-id: $TASK_ID`,
  // which buildAgentRoutingEnv sets per cell — so pi measures correctly in parallel mode too.
  const AMBIENT_BOUND_AGENTS = new Set(['opencode']);
  if (parallel && cells.some((c) => AMBIENT_BOUND_AGENTS.has(c.agent))) {
    const affected = [...new Set(cells.filter((c) => AMBIENT_BOUND_AGENTS.has(c.agent)).map((c) => c.agent))];
    process.stderr.write(
      `[experiment-runner] WARNING: parallel mode with ${affected.join('/')} cell(s) — these agents `
      + 'are ambient-slot-bound (no per-request task binding: opencode rapid-proxy traffic carries its '
      + 'own session id). Their tokens attribute via the sequential global '
      + 'span slot, which parallel mode removes, so these cells will measure ~0 tokens even though the '
      + 'agent runs. Run serially to measure them, or see agent-routing.mjs for the per-request seam.\n',
    );
  }
  const summary = [];
  // Real mutexes ALWAYS (not NO_LOCK): a run may switch serial→parallel mid-flight (operator
  // ticks "run in parallel" on a live run), so the setup/store phases must be able to serialize
  // the moment concurrency begins. In pure-serial they never contend (one cell at a time), so
  // this is behaviour-identical to the old NO_LOCK path.
  const setupLock = makeMutex();
  const storeLock = makeMutex();

  // The per-(cell, rep) body. `isParallel` is per-invocation (NOT the launch-time `parallel`) so a
  // switched run can start later cells in parallel while earlier ones ran serially.
  const runOne = async (cell, rep, isParallel) => {
    const variant = cellName(cell);
    {
      const taskId = composeTaskId(expId, cell, rep);
      ...
      if (pf && pf.ok === false) {
        ...
        try {
          // storeLock: the skip-Run's open/close touches the span slot + single-owner store —
          // under parallel it must both serialize AND go slotless (NO_LOCK pass-through serial).
          await storeLock.run(() => writeSkipRun({ cell, rep, expId, goal, taskClass, skipReason, parallel: isParallel, runMeasurement }));
        } catch (err) {
      ...
      try {
        const res = await runCell({
          ...
          parallel: isParallel, setupLock, storeLock,
          restore, spawnAgent, runMeasurement, configureRouting,
        });
  ...
  // Flat (cell, rep) work list — shared by the serial loop and the parallel pool.
  const items = [];
  for (const cell of cells) for (let rep = 0; rep < repeats; rep += 1) items.push({ cell, rep });

  // Live serial→parallel switch: the dashboard writes <runDir>/control.json {parallel:true} when
  // the operator ticks "run in parallel" on a running serial matrix. The serial loop checks it
  // between cells and, once set, releases ALL remaining cells into the worker pool.
  const wantsParallelNow = () => {
    if (parallel) return true;              // launched parallel
    if (!runDir) return false;
    try { return JSON.parse(fs.readFileSync(path.join(runDir, 'control.json'), 'utf8'))?.parallel === true; }
    catch { return false; }                 // no signal / unreadable → stay serial
  };

  let idx = 0;
  let runningParallel = parallel;
  if (!parallel) {
    // (c) Serial, but switchable: await each cell in matrix order until the operator flips to
    //     parallel (D-09 default path is byte-identical when the signal never fires).
    while (idx < items.length) {
      if (wantsParallelNow()) { runningParallel = true; break; }
      const it = items[idx]; idx += 1;
      await runOne(it.cell, it.rep, false);
    }
  }
  if (runningParallel && idx < items.length) {
    // Remaining cells run through the bounded worker pool (slotless spans). When this is a MID-RUN
    // switch (launched serial), flip execution_mode so the dashboard overlays shared background +
    // greys the "run in parallel" checkbox, and warn about any ambient-bound remaining cells.
    if (!parallel) {
      process.stderr.write('[experiment-runner] operator switched to PARALLEL — releasing remaining cells to the pool\n');
      await emitProgress(runDir, { execution_mode: 'parallel' });
      const remAmbient = [...new Set(items.slice(idx).map((it) => it.cell.agent).filter((a) => AMBIENT_BOUND_AGENTS.has(a)))];
      if (remAmbient.length) {
        process.stderr.write(`[experiment-runner] WARNING: switched ${remAmbient.join('/')} cell(s) will measure ~0 tokens (ambient-bound; see serial note)\n`);
      }
    }
    const remaining = items.slice(idx);
    let next = 0;
    const workers = Array.from({ length: Math.min(Math.max(1, maxParallel), remaining.length) }, async () => {
      for (;;) {
        const i = next; next += 1;
        if (i >= remaining.length) break;
        await runOne(remaining[i].cell, remaining[i].rep, true);
      }
    });
