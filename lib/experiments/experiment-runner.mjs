...
export async function runCell({
  ...
  // Also surface the goal text so the file-adapter seams (opencode/copilot/mastra), which
  ...
  try {
    ...
    // agent's native channel: claude → --append-system-prompt; opencode/copilot/mastra → a context
...
export async function runMatrix(spec, opts = {}) {
  ...
  // OWN session id (opencode `rapid-proxy` provider → ses_…) or land task_id='' (mastracode),
  ...
  const AMBIENT_BOUND_AGENTS = new Set(['opencode', 'mastracode']);
  if (parallel && cells.some((c) => AMBIENT_BOUND_AGENTS.has(c.agent))) {
    ...
    process.stderr.write(
      ...
      + "own session id; mastracode lands task_id=''). Their tokens attribute via the sequential global "
