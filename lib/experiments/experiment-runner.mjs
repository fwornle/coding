...
export async function runCell({
  ...
  try {
    ...
    // Terminal cell state = the closed enum (complete | timeout | abort).
    await emitProgress(runDir, {
      cells: [{ variant, rep, task_id: taskId, state: terminalState, ended_at: new Date().toISOString() }],
    });
  }
...
export async function runMatrix(spec, opts = {}) {
  const {
    ...
    variantOverrides = {}, runId, rerunOf,
    ...
    for (let rep = 0; rep < repeats; rep += 1) {
      ...
      try {
        const res = await runCell({
          ...
          repoRoot, dataDir, ontologyDir, port, runDir,
          restore, spawnAgent, runMeasurement, configureRouting,
        });
