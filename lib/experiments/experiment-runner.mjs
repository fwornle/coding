 ...
 *   • claude / copilot / mastracode → NO base-URL change (claude/copilot use their file adapters;
 *     copilot Enterprise-OAuth / mastracode self-routed have no seam anyway).
...
export async function configureProxyRoutingEnv(agent, baseEnv, {
  ...
  switch (agent) {
    case 'opencode':
      ...
      break;
    default:
      // claude → deliberately NOT proxy-routed. It is captured at close via its ~/.claude
      //   transcript adapter (cladpt), which reads the FULL usage incl. cache_read/cache_write.
      //   Routing claude through the proxy records cache-EXCLUDED Route-1 passthrough rows that
      //   share the same (user_hash, tool_call_id) dedup key and BEAT the accurate transcript
      //   rows (observed: cache dropped, 691 vs a real ~360k). CRITICAL: the coding session
      //   EXPORTS ANTHROPIC_BASE_URL, so the inherited value must be DELETED (not merely
      //   left unset) or claude silently routes anyway. copilot (Enterprise OAuth) / mastracode
      //   (self-routed) don't use it — deleting is harmless there.
...
export async function runCell({
  ...
  const agentEnv = await configureRouting(cell.agent, agentBaseEnv, { port });
  ...
  try {
    ...
    terminalState = await spawnAgent({ bin, argv, worktree, sandboxDataDir, env: agentEnv, timeoutMs });
  } finally {
...
export async function runMatrix(spec, opts = {}) {
  const {
    repoRoot, dataDir, ontologyDir, timeoutMs, agentsDir, taskClass, port,
    ...
    for (let rep = 0; rep < repeats; rep += 1) {
      ...
      try {
        const res = await runCell({
          ...
          repoRoot, dataDir, ontologyDir, port, restore, spawnAgent, runMeasurement, configureRouting,
        });
