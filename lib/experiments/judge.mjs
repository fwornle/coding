...
// structured Haiku `/api/complete` call that, given the consequential trace
...
// Body { process, messages, taskType }
// → resp { content, provider, model, tokens, latencyMs } (NOT OpenAI-wrapped).
// `content` is a STRING that must be defensively parsed + validated + clamped.
// taskType:'route_judge' routes to the fast (Haiku) tier; if config.taskTiers
// lacks it, set ROUTE_JUDGE_TIER=fast (the proxy's <TASKTYPE>_TIER env override).
//
 ...
 */
function nullJudgment({ pending = false, not_scored = null } = {}) {
  return {
 ...
 *     pending, not_scored }.
 */
export async function runJudge({ span, trace, evidence, callProxy } = {}) {
  // (1) D-04 trivial guard — short-circuit BEFORE paying the proxy.
  if (isTrivialRun(trace)) {
  ...
  // (2) ONE structured request. taskType:'route_judge' → fast (Haiku) tier.
  const body = {
    process: 'route-judge',
