...
const JUDGE_SYSTEM_PROMPT = [
  ...
  '  },',
  '  "rubric_rationale": "<the DEVIATION explanation — see the rubric_rationale rules below>"',
  '}',
  '',
  ...
  '- goal_achieved: the run\'s own acceptance command (evidence.testRun) + VERIFICATION',
  '  verdict + test summary + goal-vs-diff.',
  ...
  '',
  'READING evidence.testRun — the run\'s OWN acceptance command, declared by its spec',
  'and executed against the tree the run actually produced. `command` is that command.',
  '  - status 0  = an OBJECTIVE PASS. This is direct, first-hand evidence the run met',
  '    its acceptance criterion. Weigh it as such for goal_achieved, and do NOT call',
  '    the run uncorroborated when it is present — read `command` to see what passing',
  '    proves (e.g. a `grep` for a required string passing means the produced file',
  '    genuinely contains that string).',
  '  - status non-zero = an OBJECTIVE FAIL of that criterion.',
  '  - counts null = the command reports no pass/fail totals (a `grep` assertion has',
  '    none). It does NOT mean no test ran. Leave test_coverage null — there is no',
  '    rate to score — but still credit goal_achieved for the pass.',
  '  - testRun null = no command was resolvable; only then is there no test evidence.',
  'Note that `testSummary` is a SEPARATE slot read from a phase artifact and is null',
  'for most runs. Its absence says nothing about testRun; never treat a null',
  'testSummary as "no tests ran" when testRun is present.',
  '',
  ...
  '',
  'rubric_rationale rules — EXPLAIN THE DEVIATION, not the metric. The UI already',
  'shows what each dimension measures, so do NOT restate that. Instead, for EACH',
  'dimension you did NOT score at its best value (1.0 — or 0 for regressions and',
  'spec_drift), write one short clause, prefixed with the dimension name and score,',
  'saying WHY it fell short IN THIS RUN. Distinguish the two causes explicitly:',
  '  - an IDENTIFIED DEFICIENCY in the evidence (e.g. "test_coverage 0.50: the new',
  '    merge helpers have no accompanying tests"), versus',
  '  - a CONFIDENCE CAP from MISSING corroborating evidence — no review ran, no',
  '    acceptance command was resolvable, or file contents are not visible to you —',
  '    which limits how high you can score, and is NOT the same as finding a defect',
  '    (e.g. "code_quality 0.60: no review findings corroborate it and contents are',
  '    unseen, so capped below 1.0 on low confidence, not for an observed problem").',
  '    A PASSING evidence.testRun is corroboration and must NOT be cited as missing.',
  'Omit any dimension that IS at its best value. If every scored dimension is perfect,',
  'say so briefly. Never pad with the generic evidence-absence sentence alone.',
].join('\n');
