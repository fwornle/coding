...
export function registerKgbenchRoutes(app, { repoRoot, coordinatorPost, logger = console }) {
  ...
  app.get('/api/kgbench/config', async (req, res) => {
    try {
      ...
      // Every KNOWN_AGENT is offered. This used to filter out `mastracode`, which had an
      // adapter but had never produced a cell in any run on disk — offering it would have
      // invited an operator to spend hours discovering that. pi, which replaces it, is a
      // first-class agent whose launch path is verified, so there is nothing to withhold
      // and the two lists are the same again.
