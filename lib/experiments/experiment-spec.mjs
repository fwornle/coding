...
// This is a SUPERSET of route-trace-resolve.mjs:27's KNOWN_AGENTS: it adds `pi` — the agent
// literal `argvForAgent`/AGENT_CONFIG_FILE in agent-headless.mjs understands. pi is spec-legal
// but has no separate route-trace FAMILY, because it is self-routed through the models.json
// custom provider pi.sh writes rather than through a launcher-controlled base URL; it is
// therefore intentionally absent from the route-trace set, and the drift test asserts a
// SUPERSET, not strict equality. Note that self-routed does NOT mean unbound here: pi's
// provider carries `x-task-id: $TASK_ID`, so its cells are per-request bound.
//
// One literal, not two. The agent this replaces was `mastra` in the launcher/LSL/token world
// and `mastracode` in this one, which cost a display mapping in the variant picker and a
// name-translation entry in agent-headless. pi is spelled the same everywhere.
// Frozen so later phases extend it deliberately.
//
...
// injector for opencode/copilot) early-return when that var is off.
export const KNOWN_AGENTS = Object.freeze(['claude', 'copilot', 'opencode', 'pi']);
 ...
 */
export function resolveExperimentSpec(filePathOrObj = DEFAULT_SPEC_PATH, _opts = {}) {
  let parsed;
  let pathLabel;
