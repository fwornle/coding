...
//       git_sha + sandbox `.data/knowledge-graph/` + `.data/llm-settings.json` routing.
...
export async function restoreForCell(snapshotId, opts = {}) {
  ...
  const digest = digestRestoredState({ kbDir, settingsPath, worktree: res.worktree });
...
export async function runVariantRepeats(snapshotId, repeats = 2, opts = {}) {
  ...
  const n = Number.isInteger(repeats) && repeats > 0 ? repeats : 2;
  const results = [];
  for (let i = 0; i < n; i += 1) {
