...
const SANDBOX_KNOWLEDGE_PATHS = [
  ...
  // The sampler's own derived output. A fact set SPELLS OUT the graded answers (each fact is the
  // regex the deliverable is checked against, with a `why` explaining its significance) and the
  // frozen insight population is the treatment corpus itself. While that directory is untracked a
  // sandbox never sees it, because a worktree materialises COMMITTED content only — but that made
  // the isolation depend on a .gitignore entry rather than on the strip, and the round's artefacts
  // are exactly the thing someone reaches for when making the experiment reproducible. Stripped
  // here so tracking them cannot hand kb-off the answer key.
  path.join('.data', 'kb-ab-sampler'),
