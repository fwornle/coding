/**
 * Experiment API — the /api/experiments surface, extracted verbatim from
 * lib/vkb-server/api-routes.js so it can outlive that server.
 *
 * WHY THIS MOVED
 * The 27 experiment endpoints were hosted by vkb-server on :8080, which also
 * served the discontinued VKB viewer and its legacy knowledge API. Retiring
 * VKB would have taken the whole Experiments feature with it — the dashboard
 * proxies /api/experiments/* straight through, so the /experiment skill, run
 * control, comparison reports, measurement and avenue promote/prune all
 * depended on a server that is going away.
 *
 * These handlers never touched VKB's legacy store: they use
 * openExperimentStore() against the DEDICATED experiment LevelDB
 * (.data/experiments/leveldb) and nothing else, which is what made lifting
 * them out clean. The bodies are copied unchanged; only the class wrapper is
 * new, so behaviour is identical and the diff stays reviewable.
 *
 * SINGLE-OWNER WARNING (unchanged, and the reason these opens are read-only):
 * km-core's close() persists the WHOLE graph under one key, so a read-path
 * handler that opens read-write rewrites the entire store on every request.
 * That is what OOM-killed vkb-server through /api/experiments/runs. Any new
 * read handler here MUST pass { readOnly: true }.
 */
...
const logger = createLogger('experiment-api');
const gunzip = promisify(zlib.gunzip);

export class ExperimentApi {
  /**
   * @param {{ experimentRepoRoot?: string|null, debug?: boolean }} options
   *   experimentRepoRoot overrides the repo root for the dedicated experiment
   *   store; production leaves it null so openExperimentStore() resolves
   *   CODING_REPO. Tests inject an isolated tmp root so the single-owner real
   *   store is never opened or mutated.
   */
  constructor(options = {}) {
  ...
  /** Register every /api/experiments route on the given express app. */
  registerRoutes(app) {
    ...
    // Always-on auto-measure behavior toggle (config/behavior.json). Read/write the
    // autoMeasure block consumed by scripts/measurement-reconciler.mjs (hot-reloaded).
    app.get('/api/experiments/measurement/behavior', (req, res) => this.handleMeasurementBehaviorGet(req, res));
    app.post('/api/experiments/measurement/behavior', (req, res) => this.handleMeasurementBehaviorSet(req, res));
    // Reset always-on auto-measure: clears reconciler-owned per-agent slots so the
    // reconciler rebinds each live session fresh on its next tick.
    app.post('/api/experiments/measurement/reset', (req, res) => this.handleMeasurementReset(req, res));
    ...
    // Per-cell live log tail (mini-terminal view): offset-based incremental read of
    // <runDir>/cells/<taskId>.log — a PURE file read like run-status (Pitfall 6).
    app.get('/api/experiments/run-log/:runId/:taskId', (req, res) => this.handleRunLog(req, res));
    // Newest in-progress run, so the dashboard Run monitor can AUTO-ATTACH to a run
    // launched from anywhere (CLI, the /experiment skill, another tab) — not only the
    // tab that clicked Launch. A pure runs-dir scan (never opens the store).
    app.get('/api/experiments/active-run', (req, res) => this.handleActiveRun(req, res));
    // Live control: switch a running SERIAL matrix to parallel (operator ticked the box mid-run).
    // Writes <runDir>/control.json {parallel:true}; the runner picks it up between cells.
    app.post('/api/experiments/run-control/:runId', (req, res) => this.handleRunControl(req, res));
  ...
  /**
   * GET /api/experiments/active-run
   * Scan every <dataDir>/experiments/runs/<runId>/progress.json and return the NEWEST run whose
   * `overall` is still in-progress (not complete/cancelled/timeout/abort), so any
   * dashboard tab can auto-attach its Run monitor to a run started elsewhere. A pure
   * file scan (never opens the experiment store — Pitfall 6). A run whose progress.json
   * has gone stale (>15 min since last write) is treated as dead and NOT surfaced, so a
   * crashed run never pins the monitor open forever.
   */
  async handleActiveRun(req, res) {
    try {
      ...
      const TERMINAL = new Set(['complete', 'cancelled', 'timeout', 'abort', 'unknown']);
      const STALE_MS = 15 * 60 * 1000;
      const now = Date.now();
      let best = null;
      for (const ent of entries) {
        if (!ent.isDirectory() || !this._validRunId(ent.name)) continue;
        const p = path.join(runsDir, ent.name, 'progress.json');
        try {
          const st = await fs.stat(p);
          if (now - st.mtimeMs > STALE_MS) continue; // dead/crashed run — ignore
          const prog = JSON.parse(await fs.readFile(p, 'utf8'));
          const overall = String(prog.overall || 'unknown');
          if (TERMINAL.has(overall)) continue;
          if (!best || st.mtimeMs > best.mtimeMs) {
            best = { runId: ent.name, mtimeMs: st.mtimeMs, overall, execution_mode: prog.execution_mode ?? null };
          }
        } catch { /* unreadable/torn progress.json — skip */ }
      }
      return res.status(200).json(
        best ? { runId: best.runId, overall: best.overall, execution_mode: best.execution_mode } : { runId: null },
      );
    } catch (error) {
      logger.error('Active-run scan failed', { error: error.message });
      return res.status(500).json({ error: 'Active-run scan failed', message: error.message });
    }
  }

  /**
   * GET /api/experiments/run-log/:runId/:taskId?offset=N[&tokens=1]
   * Incremental tail of the per-cell log <runDir>/cells/<taskId>.log written by the
   * experiment runner (serial AND parallel modes). A PURE file read (never opens the
   * experiment store — Pitfall 6), offset-polled by the dashboard mini-terminal at ~1.5s.
   *   • offset >= size → { chunk:'' } (nothing new);
   *   • offset > size (truncation/restart) → reset to 0 and serve from the start;
   *   • ENOENT → graceful-empty (cell not started yet), mirrors handleRunStatus;
   *   • chunk capped at 64 KiB per poll (the client catches up across polls);
   *   • &tokens=1 → append live aggregateByTaskId totals (reads token-usage.db readonly).
   * taskId is a composeTaskId slug — validate charset BEFORE the path build (traversal guard).
   */
  ...
  async handleComparison(req, res) {
    ...
    try {
      ...
      // readOnly: buildComparison is pure — it derives the report from `rows`
      // and never touches the store. Nothing here should write back.
      const store = await openExperimentStore({ readOnly: true, ...(this.experimentRepoRoot ? { repoRoot: this.experimentRepoRoot } : {}) });
      try {
  ...
  /** Absolute path to config/behavior.json (sibling of .data). */

  /**
   * GET /api/experiments/measurement/behavior
   * Returns the effective autoMeasure config (defaults applied when file absent).
   */
  async handleMeasurementBehaviorGet(req, res) {
    try {
      let raw = {};
      try {
        raw = JSON.parse(await fs.readFile(this._behaviorPath(), 'utf8'));
      } catch (e) {
        if (e.code !== 'ENOENT') throw e;
      }
      return res.status(200).json({ autoMeasure: this._normalizeAutoMeasure(raw) });
    } catch (error) {
      logger.error('Behavior read failed', { error: error.message });
      return res.status(500).json({ error: 'Behavior read failed', message: error.message });
    }
  }

  /**
   * POST /api/experiments/measurement/behavior  { enabled?, agents?, freshnessMs?, pollMs? }
   * Merges the patch into config/behavior.json's autoMeasure block (preserving other
   * top-level keys) so the reconciler hot-reloads it on the next tick.
   */
   ...
   * top-level keys) so the reconciler hot-reloads it on the next tick.
   */
  async handleMeasurementBehaviorSet(req, res) {
    try {
      const patch = req.body || {};
      const behaviorPath = this._behaviorPath();
      let raw = {};
      try {
        raw = JSON.parse(await fs.readFile(behaviorPath, 'utf8'));
      } catch (e) {
        if (e.code !== 'ENOENT') throw e;
      }
      const merged = this._mergeAutoMeasure(this._normalizeAutoMeasure(raw), patch);
      raw.autoMeasure = merged;
      await fs.mkdir(path.dirname(behaviorPath), { recursive: true });
      await fs.writeFile(behaviorPath, JSON.stringify(raw, null, 2) + '\n', 'utf8');
      return res.status(200).json({ autoMeasure: merged });
    } catch (error) {
      logger.error('Behavior write failed', { error: error.message });
      return res.status(500).json({ error: 'Behavior write failed', message: error.message });
    }
  }

  /** Apply a validated patch onto the current autoMeasure block. */

  /**
   * POST /api/experiments/measurement/reset
   * Clears reconciler-owned per-agent auto slots (active-measurement.<agent>.json) so the
   * standalone reconciler rebinds each live session fresh on its next tick. Only removes
   * slots stamped meta.source === 'reconciler'; never touches the global manual span or
   * operator-pinned per-agent slots. Returns the list of agents whose slot was cleared.
   */
  async handleMeasurementReset(req, res) {
    try {
      const cleared = [];
      for (const agent of ['claude', 'opencode', 'copilot']) {
        const slotPath = path.join(this._dataDir(), `active-measurement.${agent}.json`);
        let span = null;
        try {
          span = JSON.parse(await fs.readFile(slotPath, 'utf8'));
        } catch (e) {
          if (e.code !== 'ENOENT') throw e;
          continue;
        }
        if (span && span.meta && span.meta.source === 'reconciler') {
          await fs.rm(slotPath, { force: true });
          cleared.push(agent);
        }
      }
      return res.status(200).json({ reset: true, cleared });
    } catch (error) {
      logger.error('Measurement reset failed', { error: error.message });
      return res.status(500).json({ error: 'Measurement reset failed', message: error.message });
    }
  }
  ...
  async handleReportsQuery(req, res) {
    try {
      ...
      // readOnly: readReports is a pure reader (the writer is handleSaveReport).
      const store = await openExperimentStore({ readOnly: true, ...(this.experimentRepoRoot ? { repoRoot: this.experimentRepoRoot } : {}) });
      try {
  ...
  /**
   * POST /api/experiments/run-control/:runId  body: { parallel: true }
   * Write <runDir>/control.json so a running SERIAL matrix switches its remaining queued cells to
   * the parallel worker pool. Idempotent; a pure file write (never opens the store). Only `parallel`
   * is honored today. ENOENT run-dir → 404 (no such run).
   */
  async handleRunControl(req, res) {
    try {
      ...
      if (req.body?.parallel !== true) {
        return res.status(400).json({ error: 'Invalid control', message: 'body must be { parallel: true }.' });
      }
      const runDir = path.join(this._dataDir(), 'experiments', 'runs', runId);
      try {
        await fs.stat(runDir);
      } catch (e) {
        if (e.code === 'ENOENT') return res.status(404).json({ error: 'No such run', runId });
        throw e;
      }
      const controlPath = path.join(runDir, 'control.json');
      const tmp = `${controlPath}.tmp-${process.pid}`;
      await fs.writeFile(tmp, JSON.stringify({ parallel: true, requested_at: new Date().toISOString() }, null, 2));
      await fs.rename(tmp, controlPath);
      return res.status(200).json({ ok: true, runId, parallel: true });
    } catch (error) {
      logger.error('Run control write failed', { error: error.message });
      return res.status(500).json({ error: 'Run control write failed', message: error.message });
    }
  }
  ...
  async handleRunLog(req, res) {
    try {
      ...
      if (typeof taskId !== 'string' || !/^[A-Za-z0-9._-]{1,200}$/.test(taskId)) {
        return res.status(400).json({ error: 'Invalid taskId', message: 'taskId must match [A-Za-z0-9._-] and be 1–200 chars.' });
      }
      let offset = Number(req.query.offset);
      if (!Number.isFinite(offset) || offset < 0) offset = 0;
      const filePath = path.join(this._dataDir(), 'experiments', 'runs', runId, 'cells', `${taskId}.log`);
      ...
      try {
        ...
        size = st.size;
        if (offset > size) offset = 0; // truncated/rotated — restart from the top
        if (offset < size) {
          const CAP = 64 * 1024;
          const toRead = Math.min(size - offset, CAP);
        ...
        if (e.code !== 'ENOENT') throw e;
        // No log yet (cell pending / pre-restore) — graceful empty, offset stays 0.
        offset = 0;
      }

      const body = { runId, taskId, offset, size, chunk };
      if (req.query.tokens === '1') {
        try {
          const { aggregateByTaskId } = await import('../experiments/token-aggregate.mjs');
          const agg = aggregateByTaskId(taskId);
          body.tokens = agg?.totals ?? null;
        } catch {
          body.tokens = null; // token DB unavailable — never fail the log poll over it
        }
      }
      return res.status(200).json(body);
    } catch (error) {
      logger.error('Run log read failed', { error: error.message });
      return res.status(500).json({ error: 'Run log read failed', message: error.message });
    }
  }
  ...
  async handleRunsQuery(req, res) {
    try {
      const { openExperimentStore } = await import('../experiments/store.mjs');
      const { readRuns } = await import('../experiments/query.mjs');
      // readOnly: this route is polled every 30s by the Performance page
      // (performance.tsx:173) and reads nothing but rows. Without it, close()
      // rewrites the whole graph as one ~8 MB LevelDB value per poll — the
      // write amplification that OOM-killed this server 175 times.
      const store = await openExperimentStore({ readOnly: true, ...(this.experimentRepoRoot ? { repoRoot: this.experimentRepoRoot } : {}) });
      try {
  ...
  async handleTimeline(req, res) {
    try {
      ...
      const { readTimeline, readAmbientBackground, readAmbientTimeline } = await import('../experiments/timeline-read.mjs');
      ...
      // via runWindow), because non-opencode agents (claude/copilot) barely
      ...
      const ambient = (winFrom && winTo)
        ...
        : [];
      // Per-CALL background rows (same window) so the dashboard can interleave the
      // knowledge/infra turns chronologically with foreground — distinct from the
      // `ambient` GROUP BY process rollup above, which still feeds the summary cards.
      const ambientTimeline = (winFrom && winTo)
        ? await readAmbientTimeline(taskId, winFrom, winTo)
        : [];
      return res.status(200).json({ timeline, ambient, ambientTimeline });
    } catch (error) {
  ...
  /** Absolute path to config/behavior.json (sibling of .data). */
  _behaviorPath() {
    return path.join(this._dataDir(), '..', 'config', 'behavior.json');
  }

  /**
   * Normalize a raw behavior.json into the autoMeasure block the reconciler reads,
   * applying the same safe defaults (enabled unless explicitly false).
   */
   ...
   *
   * Lost in the vkb-server lift (2fb090da6) and restored here. It sat between
   * _validRunId and _coordinatorPost in the old file, both of which came across,
   * so nothing flagged the gap — but _coordinatorPost calls it on the very next
   * line, which made every POST to a coordinator seam throw
   * `this._coordinatorUrl is not a function` and answer 500. That is
   * handleExperimentRun and handleRunCancel: launching and cancelling an
   * experiment through the API, both broken in production, not only in tests.
   */
  _coordinatorUrl() {
  ...
  /** Apply a validated patch onto the current autoMeasure block. */
  _mergeAutoMeasure(current, patch) {
    const next = { ...current, agents: { ...current.agents } };
    if (typeof patch.enabled === 'boolean') next.enabled = patch.enabled;
    if (Number(patch.freshnessMs) > 0) next.freshnessMs = Number(patch.freshnessMs);
    if (Number(patch.pollMs) > 0) next.pollMs = Number(patch.pollMs);
    if (patch.agents && typeof patch.agents === 'object') {
      for (const agent of ['claude', 'opencode', 'copilot']) {
        if (typeof patch.agents[agent] === 'boolean') next.agents[agent] = patch.agents[agent];
      }
    }
    return next;
  }
   ...
   * applying the same safe defaults (enabled unless explicitly false).
   */
  _normalizeAutoMeasure(raw) {
    const am = (raw && typeof raw.autoMeasure === 'object' && raw.autoMeasure) || {};
    const agents = am.agents && typeof am.agents === 'object' ? am.agents : {};
    return {
      enabled: am.enabled !== false,
      freshnessMs: Number(am.freshnessMs) > 0 ? Number(am.freshnessMs) : 120000,
      pollMs: Number(am.pollMs) > 0 ? Number(am.pollMs) : 5000,
      agents: {
        claude: agents.claude !== false,
        opencode: agents.opencode !== false,
        copilot: agents.copilot !== false,
      },
    };
  }
   ...
   * is rejected in-container BEFORE the seam POST, never coerced into a branch ref.
   *
   * Also lost in the vkb-server lift (2fb090da6), and more dangerous than the
   * missing _coordinatorUrl: this is the guard that stops an unmappable id
   * reaching the seam. Without it _proxyAvenue threw before validating, so the
   * avenue endpoints answered 500 rather than a 400 naming the bad id — the
   * failure was loud, but the check it replaced was the security-relevant half.
   */
  ...
  async _resolveOriginRun(originSpanId) {
    const { openExperimentStore } = await import('../experiments/store.mjs');
    const { readRuns } = await import('../experiments/query.mjs');
    // readOnly: resolving an origin Run is a lookup — see handleRunsQuery.
    const store = await openExperimentStore({ readOnly: true, ...(this.experimentRepoRoot ? { repoRoot: this.experimentRepoRoot } : {}) });
    try {
  ...
  _validOriginSpanId(id) {
    return typeof id === 'string'
      ...
      && id.length <= 256
      && !id.includes('..')
      // The rule's subject IS the check: this rejects ids containing control
      // characters, so the pattern has to name them.
  ...
  _validateOverrides(ov, resolvedNames) {
    ...
    // Parallel worker-pool mode (dashboard checkbox → --parallel). Strict boolean; the
    // optional pool size mirrors the runner's own 1-8 bound so a bad value fails HERE.
    if (ov.parallel !== undefined && typeof ov.parallel !== 'boolean') {
      return `overrides.parallel must be a boolean (got ${JSON.stringify(ov.parallel)}).`;
    }
    if (ov.max_parallel !== undefined) {
      if (!Number.isInteger(ov.max_parallel) || ov.max_parallel < 1 || ov.max_parallel > 8) {
        return `overrides.max_parallel must be an integer 1-8 (got ${JSON.stringify(ov.max_parallel)}).`;
      }
    }
    return null;
  }
