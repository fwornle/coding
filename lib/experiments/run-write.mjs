...
export async function writeRun(store, { span, taskClass, pending, tags, totals, heuristics }) {
  ...
  const runId = await store.putEntity({
    ...
    metadata: {
      ...
      // ── 1–2 sentence natural-language overview of what the session did, from the
      //    same LLM summarizer that produces the title. null where unavailable. ──
      session_summary:   t.session_summary ?? null,
      ...
      // ── D-05/D-07 (Phase 85-01): re-run pairing + override provenance. Same null-preserved
      //    idiom (never `?? ''` — the null-not-zero house rule). rerun_of = the ORIGINAL run's
      //    run_id (null for a first run); base_variant = the ORIGINAL variant name when a
      //    per-variant model/agent override derived a suffixed variant (null with no override).
      //    task_hash stays CONSTANT across a run_id salt — comparability preserved (D-05).
      rerun_of:       t.rerun_of ?? null,
      base_variant:   t.base_variant ?? null,
      // ── AVN-01 (Phase 87-03): the forked origin span's task_id, so avenues group by origin.
      //    Same null-preserved idiom (never `?? ''`) — null for a non-avenue (first-class) run. ──
      origin_span_id: t.origin_span_id ?? null,
      // ── Parallel experiments: 'serial' | 'parallel' | null (pre-feature spans). The dashboard
      //    gates the shared-background overlay + disclaimer on 'parallel'. Null-preserved. ──
