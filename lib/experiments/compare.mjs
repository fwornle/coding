...
/**
 * Validate + sanitize an operator/request-supplied task_hash before it is used as
 * a report filename (CLI) or a store-read key (the vkb-server comparison endpoint).
 * Allows ONLY `[A-Za-z0-9._-]`; rejects empty/non-string, any path separator
 * (`/`, `\`), a parent-dir token (`..`), and a null byte (T-79-03-01 / T-80-01-01).
 *
 * SHARED (Phase 80): lives here — in the mounted lib/experiments module — so BOTH
 * the CLI (scripts/experiments-compare.mjs re-exports it) AND the in-container
 * vkb-server handler can import it without adding a scripts/ mount.
 *
 * @param {string} hash
 * @returns {string} the safe basename (=== input for a valid hash)
 * @throws {Error} on any unsafe or empty value.
 */
...
/**
 * Map a report group name to the per-variant gate_outcome enum value (D-13).
 *
 * SHARED SOURCE OF TRUTH (Phase 80): the CLI
 * (scripts/experiments-compare.mjs writeReportJson/writeReportCsv) AND the
 * vkb-server `handleComparison` endpoint both import this map + `withGateOutcomes`
 * to stamp `gate_outcome` onto each VariantEntry. Duplicating the stamping would
 * silently diverge the live endpoint JSON from the CLI-written report file, so it
 * lives here — one place, imported by both consumers.
 */
export const GROUP_GATE_OUTCOME = Object.freeze({
  ranked: 'passed',
  failed: 'failed',
  ungated: 'ungated',
  unscored: 'unscored',
...
/**
 * Stamp each VariantEntry in a group with its group's gate_outcome (D-13) — a pure
 * copy (never mutates the input entries). A non-array group yields [].
 *
 * @param {Array<object>} group one of the four buildComparison group arrays
 * @param {'passed'|'failed'|'ungated'|'unscored'} outcome the group's gate outcome
 * @returns {Array<object>} the same entries with `gate_outcome` added
 */
export function withGateOutcomes(group, outcome) {
