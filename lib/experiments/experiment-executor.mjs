...
function resolveRunDir(runDir) {
  return path.isAbsolute(runDir) ? runDir : path.resolve(REPO_ROOT, runDir);
}
...
export async function cancelExperiment({
  ...
  // resolves against the HOST repo root; absolute passes verbatim.
  const hostRunDir = resolveRunDir(run_dir);
  ...
  const resolvedDataDir =
    dataDir || env.LLM_PROXY_DATA_DIR || path.resolve(hostRunDir, '..', '..', '..');
