...
export function buildRunArgv(specPath, runId, runDir, overrides = {}, scriptPath = EXPERIMENT_RUN_SCRIPT) {
  ...
  const valueFlags = [
    ...
    ['model', '--model'],
    ['agent', '--agent'],
    // Phase 87-07 (CR-01): the forked origin span task_id an avenue launch carries. When
    // handleExperimentRun folds origin_span_id into the forwarded overrides, emit
    // `--origin-span-id <id>` so the runner threads it into measurement-start → run-write
    // stamps origin_span_id (AVN-01). Absent → no flag (non-fork argv byte-identical).
    ['origin_span_id', '--origin-span-id'],
    // Parallel mode: optional pool size (1-8; runner validates + defaults to 4).
  ...
  // Parallel worker-pool mode (checkbox in the dashboard launcher): present only when true.
  if (overrides.parallel === true) {
    argv.push('--parallel');
  }

  // Phase 87-07 (CR-01): the avenue-fork presence flag. handleExperimentRun folds `avenue:true`
  // into the forwarded overrides for a real dashboard fork → emit `--avenue` so the runner runs
  // an AVENUE matrix (restore onto avenue/<task_id>, commit on close). Absent → byte-identical.
  if (overrides.avenue === true) {
    argv.push('--avenue');
  }
 ...
 * (D-01). Spawns process.execPath (node) with the fixed argv, `detached:true`, `child.unref()`s
 * it, persists run.json {run_id, pid, spec, started_at, log}, and returns {pid, runDir, logPath}.
 *
 * DIAGNOSABILITY (85-06 A1): the detached child's stdout+stderr are redirected to a per-run
 * `runner.log` (opened here, its fd passed to the spawn stdio slots) instead of being discarded
 * with `stdio:'ignore'`. Without this, ANY runner-level failure (a cell aborting, an unresolved
 * proxy model, a missing agent binary on PATH) left ZERO evidence on disk — an opencode cell
 * could abort silently with no `reason` anywhere. stdin stays ignored; the child remains
 * detached+unref'd so it survives a launcher restart (D-01 preserved — the log fd is inherited
 * by the child and lives independent of the parent).
 *
 * @param {object} args
 ...
 * @param {Function} [args.openLogFn] injectable log-fd opener (default node:fs openSync) — tests
 *                                    pass a stub so no real file descriptor is opened.
 * @param {string} [args.repoRoot]   cwd for the child (default REPO_ROOT)
 * @param {string} [args.scriptPath] runner script path override
 * @returns {Promise<{pid:number, runDir:string, logPath:string}>}
 */
export async function launchRun({
  ...
  // A1 diagnosability: capture the detached child's stdout+stderr to <runDir>/runner.log so a
  // silent abort leaves evidence. Open in append mode ('a') so a resume/re-launch never truncates
  // a prior run's log. Fail-soft: if the log cannot be opened, fall back to 'ignore' (a missing
  // log must NEVER block a launch — D-01 lineage).
  const logPath = path.join(runDir, 'runner.log');
  let childStdio;
  try {
    const logFd = openLogFn(logPath, 'a');
    // stdin ignored; stdout+stderr → the log fd (inherited by the detached child).
    childStdio = ['ignore', logFd, logFd];
  } catch (err) {
    process.stderr.write(`[run-launch] could not open ${logPath} (${err.message}) — stdio falls back to ignore\n`);
    childStdio = 'ignore';
  }
  ...
  return { pid: child.pid, runDir, logPath };
}
