...
function parseLSLFile(absPath) {
  ...
  for (let i = 0; i < anchors.length; i++) {
    ...
    // Format A (older specstory style): `### User\n` / `### Assistant\n` blocks.
    const userMatchA = section.match(/### User\s*\n([\s\S]*?)(?=\n### Assistant|\n### User|$)/);
    if (userMatchA) {
      userText = userMatchA[1].trim();
      ...
      assistantText = assistantBlocks.join('\n\n');
    }

    // Format B (newer specstory style, dominant in 2026 LSLs): inline labels
    // `**User Message:**`, `**User Request:**`, `**Assistant Response:**`.
    // The labels appear on a single line followed by the content until the next
    // labeled block or section boundary.
    if (!userText) {
      const userMatchB = section.match(/\*\*User (?:Message|Request|Prompt):\*\*\s*([\s\S]*?)(?=\n\*\*(?:Assistant|User) (?:Message|Request|Response|Prompt|Reply):\*\*|$)/);
      if (userMatchB) userText = userMatchB[1].trim();
    }
    if (!assistantText) {
      const asstBlocks = [];
      const asstReB = /\*\*Assistant (?:Response|Reply|Message):\*\*\s*([\s\S]*?)(?=\n\*\*(?:Assistant|User) (?:Message|Request|Response|Prompt|Reply):\*\*|$)/g;
      let am;
      while ((am = asstReB.exec(section)) !== null) {
        asstBlocks.push(am[1].trim());
      }
      assistantText = asstBlocks.join('\n\n');
    }
...
export function getLSLWindow(observation, {
  ...
  // Candidate files: those whose filename start time is within a generous
  // boundary around the observation's created_at. The filename time is the
  // "work period" name (e.g. 0600-0700 UTC), not a strict lower bound on the
  // first prompt inside — anchor timestamps may run an hour or two earlier
  // than the filename's start (verified against the live LSL tree). We allow
  // any file whose filename start is ≤ createdMs + a 2h grace window, then
  // let the per-exchange `ex.tsMs > createdMs` check filter individual
  // exchanges that are actually in the future of the observation.
  const fileCandidateGraceMs = 2 * 60 * 60 * 1000;
  const candidateFiles = files.filter(f => f.startMs <= createdMs + fileCandidateGraceMs);
