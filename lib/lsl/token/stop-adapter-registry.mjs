...
import { reconcileRow, matchWireRow } from './reconcile.mjs';
 ...
 *         rows — the no-double-count invariant, T-83-04-01); the wire request-id
 *         is tracked for the post-loop unmatched_wire query.
 ...
 *
 * `unmatched_wire` is computed by a POST-LOOP DB query (countUnmatchedWireRows),
 * NOT accumulated per-row and NEVER defaulted to 0 — Plan-07's golden comparison
 * asserts `unmatched_wire === 0`, which a silent default would make trivially
 * pass.
 *
...
function reconcileBatches({ db, batches, span, agent, adapter, taskId, opts }) {
  ...
  for (const batch of batches) {
    ...
    for (const row of batch.rows) {
      ...
      let result;
      try {
        result = reconcileRow(db, transcriptRow, span, opts);
      } catch (err) {
      ...
      if (result.matched) {
        report.matched += 1;
        // Track the WIRE row's request-id so the post-loop unmatched_wire query
        // excludes it. A request-id match shares the transcript tool_call_id; a
        // fuzzy match's wire id differs, so re-probe (read-only) to learn it.
        if (result.method === 'fuzzy') {
          try {
            const { wireRow } = matchWireRow(db, transcriptRow, span, opts);
            if (wireRow && wireRow.tool_call_id) matchedRequestIds.add(wireRow.tool_call_id);
          } catch {
            // best-effort — an un-tracked fuzzy id at worst over-counts unmatched_wire.
          }
        } else if (typeof tcid === 'string' && tcid.length > 0) {
          matchedRequestIds.add(tcid);
        }
  ...
  // unmatched_wire: POST-LOOP query over span-window wire rows whose tool_call_id
  // never matched during the loop (never a per-row accumulation / silent 0).
  report.unmatched_wire = countUnmatchedWireRows(db, span, taskId, adapter.userHash, matchedRequestIds);

  return report;
}

/**
 * Count the ORPHAN wire rows for a measured span (Phase 83-04, Test 7). A wire
 * row is one written by the proxy (a DIFFERENT `user_hash` than this adapter's
 * cladpt/copadt) carrying the span's `task_id`; an orphan is a span-window wire
 * row whose `tool_call_id` never appeared in `matchedRequestIds` during the
 * transcript loop. Uses the SAME withinSpanWindow bounds as the transcript loop.
 * Parameterized binds only; best-effort (never throws) — a query failure yields 0
 * so a reconciliation failure never blocks the close.
 *
 ...
 * @param {string} adapterUserHash the adapter's user_hash to EXCLUDE (own rows)
 * @param {Set<string>} matchedRequestIds wire ids matched during the loop
 * @returns {number} orphan wire-row count
 */
function countUnmatchedWireRows(db, span, taskId, adapterUserHash, matchedRequestIds) {
  try {
    const rows = db
      .prepare(
        'SELECT tool_call_id, timestamp FROM token_usage WHERE task_id = ? AND user_hash != ?',
      )
      .all(taskId ?? '', adapterUserHash);
    let unmatched = 0;
    for (const r of rows) {
      if (!withinSpanWindow(r, span)) continue;
      const id = r.tool_call_id;
      if (typeof id === 'string' && id.length > 0 && matchedRequestIds.has(id)) continue;
      unmatched += 1;
    }
    return unmatched;
  } catch (err) {
    process.stderr.write(
      `[stop-adapter] unmatched_wire query failed (non-fatal): ${err.message}\n`,
    );
    return 0;
  }
}
