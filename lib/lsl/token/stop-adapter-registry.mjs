 ...
 * copilot / opencode / pi ALL route through the rapid-llm-proxy and are
...
import { buildOpencodeTokenRows, DEFAULT_OPENCODE_DB } from './opencode-token-rows.mjs';
import {
  openTokenDb,
 ...
 */
const DEFAULT_TOKEN_DB = path.join(
 ...
 * - 'transcript' (opencode, Phase 85): a HYBRID case. OpenCode routes through the proxy
 *   for a proxy-backed provider (anthropic via ANTHROPIC_BASE_URL) BUT bypasses it entirely
 *   for a direct-backend provider (github-copilot → GitHub Copilot's endpoint). Its build,
 *   buildOpencodeTokenRows, is PROVIDER-GATED: it emits a row ONLY for a bypass provider
 *   (BYPASS_PROVIDERS) and SKIPS proxy-backed providers (already in token_usage). So the
 *   no-double-count invariant holds — the adapter reconstructs strictly what the proxy never
 *   saw. Source: OpenCode's own SQLite store (`~/.local/share/opencode/opencode.db`), which
 *   records native PER-MESSAGE tokens (per-turn granularity).
 * - 'stamp-only' (pi): routes through the proxy (self-routed via its models.json custom
 *   provider), so its foreground rows are ALREADY in token_usage. It carries NO `build`
 *   property; adding one would DOUBLE-COUNT the proxy rows (D-04 anti-pattern).
 *   Unlike mastra — the agent this replaces — pi is not missing the task_id either: its
 *   provider sends `x-task-id: $TASK_ID`, which pi interpolates per process, so the proxy
 *   binds each row at capture time. stamp-only here means "the proxy already has it", not
 *   "there is a gap to fill later".
 */
export const STOP_ADAPTERS = {
  ...
  // OpenCode on a bypass provider (github-copilot) never hits the proxy; buildOpencodeTokenRows
  // is provider-gated so proxy-backed opencode (anthropic) is skipped (no double-count).
  opencode: {
    mode: 'transcript',
    userHash: ADAPTER_USER_HASH_OPENCODE,
    build: buildOpencodeTokenRows,
    locate: locateOpencodeStoreForSpan,
    subagents: false,
  },
  pi: { mode: 'stamp-only' },
};
...
function encodeCwd(cwd) {
  // Underscore is a separator too — see the note on encodeCwd in
  // lib/lsl/adapters/claude-jsonl-tree.mjs. Kept character-for-character
  // identical to that one; the two are the same rule in two places.
...
/**
 * Locate the OpenCode SQLite store for the span (Phase 85). Unlike claude/copilot
 * (per-session files), OpenCode keeps ONE store for all sessions, so there is no
 * per-span file to select — the span-window clamp happens downstream in
 * withinSpanWindow on each per-message row's timestamp. This locator therefore
 * just returns the store path when it exists (existence gate); buildOpencodeTokenRows
 * applies the uid-ownership gate + bounded recent scan. The `span` arg is accepted
 * for signature parity with the other locators.
 *
 * @param {object} span
 * @returns {string} absolute opencode.db path, or '' if absent
 */
function locateOpencodeStoreForSpan(span) { // eslint-disable-line no-unused-vars
  try {
    return fs.existsSync(DEFAULT_OPENCODE_DB) ? DEFAULT_OPENCODE_DB : '';
  } catch (err) {
    process.stderr.write(
      `[stop-adapter] opencode-store locate failed (non-fatal): ${err.message}\n`,
    );
    return '';
  }
}
 ...
 * `span.agent`):
 *   - For a STAMP-ONLY agent (copilot/opencode/pi) OR an unknown agent, it
...
function reconcileBatches({ db, batches, span, agent, adapter, taskId, opts }) {
  ...
  // copilot cache-merge; matchedWireRowIds tracks the matched WIRE-row ROWIDs for
  // the unmatched_wire diff. wireRowIds is the PRE-LOOP snapshot of candidate
  // wire-row ROWIDs — taken BEFORE any transcript-adapter row is inserted, so every
  // captured row is a proxy-tap wire row (indistinguishable from the cladpt
  // transcript rows by user_hash/process for claude — only row identity separates
  // them). CR-01 (re-review): identity is `rowid`, NOT the bare `id` (composite PK
  // `(user_hash, id)` — per-hash id sequences collide across adapter hashes).
  //
  ...
  // WR-11 (re-review): the snapshot carries TWO sets. `wireRowIds` (all rows in
  // the task_id/'' arms + window) drives the unmatched_wire diff;
  // `fuzzyCandidateIds` EXCLUDES the task_id='' rows — a concurrent interactive
  // session's tap rows carry '' (D-08 no-inherit) and must never be
  // fuzzy-bound to the measured span (the CR-03 gap-fill would then stamp the
  // span task_id onto an unrelated session's row — cross-session attribution
  // capture). A ''-row can still be matched by the UNSCOPED request-id probe
  // (the span's own transcript carries the request-id, an unrelated session's
  // does not), so legitimate interactive-launch wire rows keep matching.
  const { wireRowIds, fuzzyCandidateIds } = snapshotWireRowIds(db, span, taskId, adapter.userHash);
  ...
  const reconcileOpts = { ...(opts || {}), candidateWireIds: fuzzyCandidateIds, consumedWireIds: matchedWireRowIds };
  ...
  for (const batch of batches) {
    ...
    for (const row of batch.rows) {
      ...
      // WR-01 (re-review): EVERY row is window-scoped to the measured span (same
      // clamp as the interactive loop) — including :reason:N rows. D-02 justifies
      // bypassing the MATCH for reason rows (the wire never carries a reasoning
      // split — reconcileRow still always-inserts them), NOT the window:
      // buildClaudeTokenRows returns the whole ambient transcript, so an unclamped
      // reason row from hours outside a short measured span would inflate the
      // run's reasoning_tokens with unrelated thinking.
      ...
      if (result.matched) {
        ...
        // WR-02 (re-review): COMPOSE provenance instead of replacing it — a
        // sub-agent batch row must keep its sub-agent identity through a
        // proxy-down fallback (`token-adapter-<agent>-fallback-subagent`), else
        // the canonical picker (measurement-stop isSubagentGroup) stops
        // recognizing it and a cheap high-volume Explore sub-agent can hijack
        // the run's canonical chat model.
        report.unmatched_transcript += 1;
        report.fallback += 1;
        const provenance =
          batch.process === SUBAGENT_PROCESS
            ? `${fallbackProcess}-subagent`
            : fallbackProcess;
        insertTokenRowDeduped(db, { ...transcriptRow, process: provenance });
      }
 ...
 *
 * CR-01 (re-review): the snapshot keys on `rowid` — NOT the bare `id` column.
 * The PK is composite `(user_hash, id)` and each hash runs its own MAX(id)+1
 * sequence, so cladpt id=1 / copadt id=1 / machine-hash id=1 coexist by design
 * (D-11). A bare-`id` Set let a different-hash row with a colliding numeric id
 * pass the fuzzy candidate filter, mark the legitimate snapshot row consumed,
 * and hide it from the unmatched_wire diff. `rowid` is table-unique (rowid
 * table — the D-11 migration repair already keys on it).
 *
 * WR-11 (re-review): returns TWO sets. `wireRowIds` is the full snapshot (the
 * unmatched_wire diff basis — '' rows are still REPORTED when orphaned).
 * `fuzzyCandidateIds` excludes the task_id='' arm: a concurrent interactive
 * session's tap rows carry '' (D-08 no-inherit) and are indistinguishable from
 * the span's own interactive-launch rows by task_id — letting fuzzy bind one
 * would silently drop the turn's tokens AND let the CR-03 gap-fill stamp the
 * span task_id onto the unrelated session's row. ''-rows remain matchable via
 * the unscoped request-id probe (which only the span's own transcript can hit).
 *
 ...
 * @returns {{wireRowIds: Set<number>, fuzzyCandidateIds: Set<number>}} the
 *   snapshot ROWIDs (all) and the fuzzy-eligible subset (task_id-bound rows)
 */
function snapshotWireRowIds(db, span, taskId, adapterUserHash) {
  const wireRowIds = new Set();
  const fuzzyCandidateIds = new Set();
  try {
    const rows = db
      .prepare(
        "SELECT rowid AS rid, tool_call_id, timestamp, task_id FROM token_usage WHERE user_hash = ? AND (task_id = ? OR task_id = '')",
      )
      .all(adapterUserHash, taskId ?? '');
    for (const r of rows) {
      if (!withinSpanWindow(r, span)) continue;
      wireRowIds.add(r.rid);
      // WR-11: only task_id-BOUND rows are fuzzy candidates; neutral '' rows
      // stay diff-only (request-id matching still reaches them).
      if (r.task_id !== '') fuzzyCandidateIds.add(r.rid);
    }
  } catch (err) {
    process.stderr.write(
  ...
  return { wireRowIds, fuzzyCandidateIds };
}
