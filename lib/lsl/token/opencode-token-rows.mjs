...
import { ADAPTER_USER_HASH_OPENCODE, DIRECT_ROUTING_SOURCE } from './token-db.mjs';

/**
 * The route these calls belong to, even though no route resolved them.
 *
 * `fg-chat/opencode` is what llm-routing.yaml calls foreground opencode traffic
 * (job `fg-chat` + agent `opencode`, resolved by server.mjs's three-step
 * lookup). A bypassed call never reaches that lookup — but it IS the same job,
 * from the same agent, in the same pane, and naming it is what lets the routing
 * dashboard show opencode work beside the claude work it competes with.
 *
 * The honesty is carried by `routing_source: 'direct'`, which says the provider
 * on this row was chosen by the agent and not by the router. Band is left ''
 * deliberately: `fg-chat/opencode` is `complexity: from-caller`, and a bypassed
 * call declared no band to anyone. Inventing one from the model would read as a
 * routing decision, which is precisely what did not happen here.
 */
const DIRECT_ROUTE_KEY = 'fg-chat/opencode';
  ...
  try {
    ...
    for (const rec of records) {
      ...
      rows.push({
        ...
        // Routing attribution: this call did not traverse the proxy, and saying
        // so is what makes it visible in the routing dashboard instead of being
        // filtered out as an un-attributed row. See DIRECT_ROUTE_KEY.
