...
function fuzzyMatch(db, transcriptRow, span, opts) {
  ...
  for (const c of candidates) {
    ...
    if (candidateWireIds && !candidateWireIds.has(c.id)) continue;
    if (consumedWireIds && consumedWireIds.has(c.id)) continue;
...
export function reconcileRow(db, transcriptRow, span = {}, opts = {}) {
  const base = {
    ...
    deltas: {},
    flagged: false,
    // CR-02 (83-08): the matched WIRE row's PK + tool_call_id so the caller can
    // record it WITHOUT re-probing (the PK drives the unmatched_wire snapshot diff;
    // the tool_call_id feeds the copilot cache-merge). null on every non-match path.
    wireRowId: null,
    wireToolCallId: null,
  };
  try {
