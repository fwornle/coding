 ...
 *     large-cache pair).
 *   - Cross-key join (D-04): the match probes on `tool_call_id` ALONE
 *     (== upstream request-id) via `probeWireRowByRequestId` — NOT the
 *     `(user_hash, tool_call_id)` dedup key — because retry/fuzzy needs the
 *     request-id and copilot wire rows carry a DIFFERENT `user_hash` than the
 *     copadt adapter. NOTE (CR-02): for CLAUDE this cross-hash framing is FALSE —
 *     the proxy tap stamps claude wire rows with the SAME `cladpt` hash the
 *     transcript adapter uses, so wire and transcript rows are indistinguishable
 *     by user_hash (and by `process`); only ROW IDENTITY (PK) separates them.
 *     Probing on `tool_call_id` alone is correct BECAUSE it never depends on the
 *     hash — but any downstream "which rows are wire" decision MUST key on the PK,
 *     never on a user_hash exclusion (the root of the CR-02 mis-design).
 *
 * Security / robustness:
 ...
 * CR-01 (re-review): selects `rowid AS rid` because the bare `id` column is NOT
 * unique — the PK is composite `(user_hash, id)` and each adapter hash runs its
 * own MAX(id)+1 sequence, so cladpt id=1 and copadt id=1 coexist by design
 * (D-11). Row identity for the scoping Sets MUST therefore key on `rowid`
 * (the table is a rowid table), never the bare `id`.
 */
 ...
 *
 * CR-01 (83-09): when the caller supplies them, fuzzy candidates are scoped to the
 * PRE-LOOP wire snapshot (`opts.candidateWireIds`) MINUS the rows already consumed
 * by earlier transcript rows (`opts.consumedWireIds`). Both are OPTIONAL Sets of
 * wire-row PKs; when ABSENT the loop is byte-behavior-identical to before (direct
 * `matchWireRow` unit tests + the interactive path are unaffected). This prevents
 * a same-model turn from fuzzy-matching a fallback row inserted seconds earlier in
 * the SAME reconcile loop (which would silently drop that turn's tokens — golden
 * property 2) and from double-consuming a single wire row across two turns.
 */
function fuzzyMatch(db, transcriptRow, span, opts) {
  ...
  // CR-01 scoping sets (optional): resolved once so the loop stays cheap.
  const candidateWireIds = opts && opts.candidateWireIds instanceof Set ? opts.candidateWireIds : null;
  const consumedWireIds = opts && opts.consumedWireIds instanceof Set ? opts.consumedWireIds : null;

  const candidates = db.prepare(FUZZY_CANDIDATES_SQL).all(model);
  let best = null;
  let bestDiff = Infinity;
  for (const c of candidates) {
    // CR-01: restrict to the pre-loop wire snapshot and skip already-consumed
    // rows. No-ops when the caller omits the sets (backward compatible).
    // Identity is `rowid` (c.rid) — NOT the bare `id`, which is per-user_hash
    // and collides across hashes (cladpt id=1 vs copadt id=1).
 ...
 *
 * CR-01 (83-09): the request-id probe is UNSCOPED (a partial-unique index gives
 * ≤1 row per (user_hash, tool_call_id) and a fallback row never carries a later
 * turn's request-id — safe as-is). The fuzzy fallback is scoped to
 * `opts.candidateWireIds` (the pre-loop wire snapshot) MINUS `opts.consumedWireIds`
 * (rows matched by earlier transcript rows) when the caller supplies them, so
 * fuzzy can never land on a loop-inserted fallback row or double-consume one wire
 * row. Both opts are OPTIONAL — absent = legacy unscoped fuzzy behavior.
 *
 ...
 * @param {object} [opts] { fuzzyWindowMs, candidateWireIds?:Set, consumedWireIds?:Set }
...
/**
 * Roll up the per-field SUM of every `perRequest[].deltas[field].delta` across a
 * reconciliation report's per-request array (Phase 83-08, CR-01). Each entry's
 * `deltas` is `computeDeltas`' output — a map of `field -> {wire,transcript,delta,
 * flagged}`, NOT a plain number. The CR-01 defect was an inline roll-up that tested
 * `typeof val === 'number'` against the whole delta OBJECT (always false → the
 * aggregate was always `{}`); this helper unwraps `.delta` FIRST, then accumulates.
 *
 * Pure — no DB, no throw. Robust to a null/undefined/non-object entry or a
 * non-object `deltas` (skipped, never thrown); returns the partial map. A field's
 * `.delta` is only added when it is a finite number.
 *
 * @param {Array<{deltas?: object}>} perRequest the report's per-request array
 * @returns {Record<string, number>} per-field SUM of the recorded deltas
 */
export function aggregatePerRequestDeltas(perRequest) {
  const out = {};
  if (!Array.isArray(perRequest)) return out;
  for (const entry of perRequest) {
    const deltas = entry && typeof entry.deltas === 'object' && entry.deltas ? entry.deltas : null;
    if (!deltas) continue;
    for (const [field, val] of Object.entries(deltas)) {
      const d = val && typeof val === 'object' ? val.delta : undefined;
      if (typeof d === 'number' && Number.isFinite(d)) {
        out[field] = (out[field] ?? 0) + d;
      }
    }
  }
  return out;
 ...
 *   wireRowId: number|null, wireToolCallId: string|null }}
 */
export function reconcileRow(db, transcriptRow, span = {}, opts = {}) {
  const base = {
    ...
    deltas: {},
    flagged: false,
    // CR-02 (83-08): the matched WIRE row's identity + tool_call_id so the caller
    // can record it WITHOUT re-probing (the identity drives the unmatched_wire
    // snapshot diff; the tool_call_id feeds the copilot cache-merge). null on
    // every non-match path. CR-01 (re-review): identity is the ROWID — the bare
    // `id` is per-user_hash and collides across adapter hashes.
    wireRowId: null,
    wireToolCallId: null,
  };
  try {
    ...
    const enriched = reconcileGapFill(db, wireRow.tool_call_id, {
      ...
      // CR-03 (83-08): backfill the span task_id onto a task_id='' wire row
      // (interactive-launch, blank x-task-id per D-08). transcriptRow.task_id is
      // the span taskId (set by reconcileBatches). The gap-fill CASE-guard stamps
      // it ONLY when the wire task_id is currently '' — span-scoped, never ambient.
      task_id: transcriptRow.task_id,
    });
    ...
    return {
      ...
      deltas,
      flagged,
      // CR-01 (re-review): rowid, NOT the composite-PK `id` fragment — cross-hash
      // id collision would otherwise defeat the snapshot/consumed-set scoping.
      wireRowId: wireRow.rid ?? null,
      wireToolCallId: wireRow.tool_call_id ?? null,
    };
  } catch (err) {
