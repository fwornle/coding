...
import { loadTaxonomy } from '../../experiments/taxonomy.mjs';
 ...
 */
export async function llmSessionTitle(dbPath, sessionId, log = () => {}, ctx = {}) {
  const storeSignal = gatherSignal(dbPath, sessionId);
  // Fall back to a caller-supplied signal when the opencode store has nothing for
  // this session — a CLAUDE session isn't in the opencode DB, so gatherSignal is
  // empty; the claude auto-measure pass passes its transcript-derived userMsgs +
  // actions (+ a growing turn count) so claude runs get a real LLM title instead
  // of the raw first-prose-line heuristic.
  const hasStore = storeSignal.userMsgs.length || storeSignal.actions.length;
  const signal = hasStore
    ? storeSignal
    : {
      userMsgs: Array.isArray(ctx.userMsgs) ? ctx.userMsgs : [],
      actions: Array.isArray(ctx.actions) ? ctx.actions : [],
      turns: Number.isFinite(ctx.turns) ? ctx.turns : storeSignal.turns,
    };
...
// ── Session task-class (Option A: LLM classification) ──────────────────────
// The zero-LLM keyword scorer (deriveClassFromText) quarantines most ambient
// sessions as 'unclassified' — a descriptive title like "Fix session class
// column" scores only +1 (below the confidence threshold), and titles like
// "Research …" contain no taxonomy keyword at all. This asks the same Haiku
// endpoint to pick the single best-fit class from the closed taxonomy (D-10
// single source of truth), which reads intent far better than a keyword match.

// Lazy, module-level taxonomy cache — loaded once, reused across sessions/passes.
let _taxCache;
function taxonomy() {
  if (_taxCache !== undefined) return _taxCache;
  try { _taxCache = loadTaxonomy(); } catch { _taxCache = null; }
  return _taxCache;
}

// Build the closed-class menu (name + definition) for the prompt, and the Set of
// valid class names for validating the model's answer. Empty when no taxonomy.
function classMenu(tax) {
  const classes = (tax && tax.classes) || {};
  const keys = Object.keys(classes);
  const text = keys.map((k) => `- ${k}: ${classes[k]?.definition || ''}`).join('\n');
  return { keys, text };
}

// Bump when the classification PROMPT changes so cached classes are re-derived
// (the cache is keyed on session id + turn count, which a prompt change alone
// would not invalidate → stale classes would survive the improvement).
const CLASS_PROMPT_VERSION = 2;

function buildClassRequest({ userMsgs, actions }, menuText, ctx = {}) {
  const requests = userMsgs.length
    ? userMsgs.map((m, i) => `${i + 1}. ${m}`).join('\n')
    : '(no clear textual request captured)';
  const activity = actions.length ? actions.join('\n') : '(no tool actions captured)';
  // The title/summary (distilled from the same signal) name the PRIMARY objective,
  // so they anchor the class on the dominant work — not a secondary side-task.
  const titleTag = ctx.title ? `<title>${ctx.title}</title>\n` : '';
  const summaryTag = ctx.summary ? `<summary>${ctx.summary}</summary>\n` : '';
  return {
    process: 'auto-measure-class',
    // taskType routes dedup/cheap calls to Haiku (see CLAUDE.md llm-proxy note).
    taskType: 'classification',
    messages: [
      {
        role: 'system',
        content:
          'You classify a coding work session into exactly ONE task class by its PRIMARY ' +
          'objective. Choose the single best-fit class from this closed set:\n' + menuText + '\n' +
          'Rules:\n' +
          '- Output ONLY the class name, lowercase, exactly as written above. Nothing else.\n' +
          '- Classify by the DOMINANT goal — the main thing the session set out to do. A session ' +
          'that fixes a defect or diagnoses a problem is bugfix/debug even when it ALSO updates docs ' +
          'or refactors along the way; only choose docs/refactor/etc. when THAT is the primary ' +
          'deliverable, not an incidental side-task.\n' +
          '- The <title> names the primary objective — weight it most heavily.\n' +
          '- If none fit, or it is not a coding task, output exactly: unclassified.',
      },
      {
        role: 'user',
        content: `${titleTag}${summaryTag}<requests>\n${requests}\n</requests>\n\n<actions>\n${activity}\n</actions>\n\nClass:`,
      },
    ],
  };
}

// Map the model's free text to a valid class. Substring match (longest-first) so
// "Class: new-feature" still resolves; anything unrecognised → 'unclassified'.
function cleanClass(s, keys) {
  const lower = String(s || '').toLowerCase();
  for (const c of [...keys].sort((a, b) => b.length - a.length)) {
...
/**
 * Return an LLM-derived task class for a session (a closed-taxonomy member or the
 * 'unclassified' sentinel). Cached + regen-gated like the title/summary. NEVER
 * throws. Returns null ONLY when the LLM is unavailable with no cache (so the
 * caller can fall back to the keyword scorer) or when the taxonomy is missing.
 *
 * @param {string} dbPath     opencode SQLite path
 * @param {string} sessionId  opencode session id
 * @param {(msg:string)=>void} [log]
 ...
 */
export async function llmSessionClass(dbPath, sessionId, log = () => {}, ctx = {}) {
  const { keys, text } = classMenu(taxonomy());
  if (!keys.length) return null; // no taxonomy → let the caller fall back

  const signal = gatherSignal(dbPath, sessionId);
  // Classify from EITHER the store signal OR the caller-supplied title/summary —
  // a long, compacted session may have lost its user-text parts from the store,
  // but its distilled title+summary (the run's goal_sentence) still name the work.
  const hasCtx = !!(ctx.title || ctx.summary);
  if (!signal.userMsgs.length && !signal.actions.length && !hasCtx) return null;

  const cache = readCache();
  const hit = cache[sessionId];
  // Reuse only when the class is fresh, was derived by the CURRENT prompt, AND is
  // a REAL class. 'unclassified' is "couldn't tell yet", not a terminal verdict —
  // never cache it as final, or a session whose freshness signal is static (e.g.
  // a claude session, absent from the opencode store, so signal.turns is always 0)
  // would be pinned to 'unclassified' forever. Retrying it re-derives once the
  // caller supplies richer ctx (title + action digest).
  if (hit?.taskClass && hit.taskClass !== 'unclassified'
      && hit.classPromptV === CLASS_PROMPT_VERSION
      && typeof hit.classTurns === 'number' && signal.turns - hit.classTurns < REGEN_TURN_DELTA) {
    return hit.taskClass; // fresh enough — skip the LLM call
  }

  try {
    const res = await callProxy(buildClassRequest(signal, text, ctx));
    const cls = cleanClass(res?.content, keys);
    cache[sessionId] = {
      ...hit, taskClass: cls, classTurns: signal.turns,
      classPromptV: CLASS_PROMPT_VERSION, updatedAt: new Date().toISOString(),
    };
    writeCache(cache);
    log(`classified ${sessionId.slice(0, 12)} via ${res?.model || 'llm'}: ${cls}`);
    return cls;
  } catch (e) {
    log(`class llm unavailable (${e.message}) — ${hit?.taskClass ? 'keeping cached' : 'falling back'}`);
    return hit?.taskClass || null;
  }
}
