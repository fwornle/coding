...
function buildClassRequest({ userMsgs, actions }, menuText) {
  const requests = userMsgs.length
    ? userMsgs.map((m, i) => `${i + 1}. ${m}`).join('\n')
    : '(no clear textual request captured)';
  const activity = actions.length ? actions.join('\n') : '(no tool actions captured)';
    ...
    messages: [
      {
        role: 'system',
        content:
          'You classify a coding work session into exactly ONE task class. Choose the ' +
          'single best-fit class from this closed set:\n' + menuText + '\n' +
          'Rules:\n' +
          '- Output ONLY the class name, lowercase, exactly as written above. Nothing else.\n' +
          '- If the session spans several, pick the DOMINANT one.\n' +
 ...
 */
export async function llmSessionClass(dbPath, sessionId, log = () => {}) {
  ...
  const cache = readCache();
  const hit = cache[sessionId];
  if (hit?.taskClass && typeof hit.classTurns === 'number'
      && signal.turns - hit.classTurns < REGEN_TURN_DELTA) {
    return hit.taskClass; // fresh enough — skip the LLM call
  }

  try {
    const res = await callProxy(buildClassRequest(signal, text));
    const cls = cleanClass(res?.content, keys);
    cache[sessionId] = { ...hit, taskClass: cls, classTurns: signal.turns, updatedAt: new Date().toISOString() };
    writeCache(cache);
