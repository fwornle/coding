...
export function buildCopilotTokenRows(eventsJsonlPath, ctx = {}) { // eslint-disable-line no-unused-vars
  ...
  for (const line of raw.split('\n')) {
    ...
    for (const [model, m] of Object.entries(mm)) {
      ...
      // Cache tokens surfaced SEPARATELY (not in total_tokens) — copilot's modelMetrics reports
      // cacheReadTokens (prompt-cache hit) + cacheWriteTokens (cache creation), parallel to
      // Claude's cache_read_input_tokens / cache_creation_input_tokens.
