...
export function buildClaudeTokenRows(jsonlPath, ctx = {}) { // eslint-disable-line no-unused-vars
  ...
  for (const line of lines) {
    ...
    // Cache tokens are surfaced SEPARATELY (not folded into total_tokens). Claude's transcript
    // reports them as cache_read_input_tokens (prompt-cache HIT) + cache_creation_input_tokens
    // (cache WRITE). These dominate a heavily-cached run (e.g. 143k read + 215k write vs 7 fresh
    // input) — without them claude looks ~500× cheaper than it is.
    ...
    for (const block of content) {
      ...
      // Skip redacted/empty thinking blocks: with no thinking text the estimate
      // would floor to 1 and surface a misleading "1 reasoning" row that is
      // indistinguishable from a real 1-token estimate. Only emit a
      // per-reasoning-step row when there is actual thinking content to estimate.
      if (thinkingText.trim() === '') continue;
      rows.push({
        timestamp: str(rec.timestamp),
        agent: 'claude',
        provider: 'claude-code',
