...
// OpenCode foreground adapter hash (Phase 85). Used ONLY for opencode sessions
// whose backend provider BYPASSES the rapid-llm-proxy (e.g. github-copilot) —
// proxy-routed opencode (provider=anthropic) is already in token_usage and is
// NEVER emitted by the adapter (the no-double-count invariant, provider-gated).
export const ADAPTER_USER_HASH_OPENCODE = 'opnadt';
...
/**
 * Routing attribution, appended when the proxy's schema carries these columns.
 *
 * WHY AN ADAPTER WRITES THEM AT ALL. These columns exist because the proxy fills
 * them on the way past — they describe a routing decision the proxy made. An
 * adapter row, by construction, describes a call that NEVER WENT PAST THE PROXY:
 * that is the entire reason the adapter reconstructs it. So the adapter has no
 * decision to record, left them at their `DEFAULT ''`, and that is where the
 * rows disappeared.
 *
 * The routing dashboard's "Recent decisions" table filters on
 * `routing_source` being non-empty — reasonably, since a row with no attribution
 * has nothing to show in a decisions table. The consequence was that opencode
 * traffic on a direct provider was invisible there. Not filtered out as
 * uninteresting: absent, with nothing on the page to say so, while the same
 * calls were plainly present in the totals above it.
 *
 * "Bypassed the proxy" IS the decision worth recording, so the adapter records
 * it — see DIRECT_ROUTING_SOURCE.
 */
const ROUTING_COLUMNS = [
  'route_key', 'route_band', 'route_step',
  'offloaded_from', 'chain_position', 'attempt_trail', 'routing_source',
];

/**
 * Exported so tests can assert against THIS list rather than a copy of it.
 * A duplicated column list drifts the moment one is extended, and the failure
 * it produces — a test that still passes while the adapter stopped writing a
 * column — is the one worth designing against.
 */
export { ROUTING_COLUMNS };

/**
 * `routing_source` for a call that never traversed the proxy.
 *
 * The column's other values are the proxy's own: 'live' (recorded as the
 * decision was made) and 'backfill' (re-derived afterwards by
 * rapid-llm-proxy/scripts/backfill-routing-decisions.mjs). Both mean the call
 * went through routing. 'direct' means it did not, and the provider on the row
 * is the one the AGENT picked for itself.
 *
 * Naming this rather than leaving it '' matters twice over. It makes these rows
 * visible where they belong, and it stops the backfill script — which selects
 * `WHERE routing_source = ''` — from later stamping them 'backfill', i.e. from
 * claiming a routing decision that never happened.
 */
export const DIRECT_ROUTING_SOURCE = 'direct';

/** Positional binds mirroring BASE_COLUMNS, in order. */
function baseValues(row, id, userHash) {
  return [
    id,
    text(row.timestamp),
    text(row.provider),
...
/** Positional binds mirroring ROUTING_COLUMNS, in order. */
function routingValues(row) {
  return [
    text(row.route_key),
    text(row.route_band),
    num(row.route_step),
    text(row.offloaded_from),
    num(row.chain_position),
    text(row.attempt_trail),
    text(row.routing_source),
  ];
}

/**
 * The INSERT for THIS database, memoised per handle.
 *
 * The routing columns are the proxy's, added by its own migration, and the
 * adapter is the second writer on someone else's schema — so their presence is
 * a runtime fact, not an assumption. Naming a column that does not exist would
 * turn every adapter insert into a hard error and lose the token accounting the
 * adapter exists to provide, which is a far worse failure than an unattributed
 * row. Probed once per open; a DB without them behaves exactly as before.
 */
const _insertShape = new WeakMap();
function insertShapeFor(db) {
  let shape = _insertShape.get(db);
  if (shape) return shape;
  let hasRouting = false;
  try {
    const cols = new Set(db.prepare('PRAGMA table_info(token_usage)').all().map((c) => c.name));
    hasRouting = ROUTING_COLUMNS.every((c) => cols.has(c));
  } catch {
    hasRouting = false; // unreadable schema → base columns only
  }
  const columns = hasRouting ? [...BASE_COLUMNS, ...ROUTING_COLUMNS] : BASE_COLUMNS;
  shape = {
    hasRouting,
    sql: `INSERT INTO token_usage (\n  ${columns.join(', ')}\n) VALUES (${columns.map(() => '?').join(', ')})`,
  };
  _insertShape.set(db, shape);
  return shape;
}
 ...
 *
 * WR-05 (re-review): the MAX(id)+1 → INSERT pair is not atomic, and a
 * concurrent proxy write into the SAME adapter hash space (the tap stamps
 * `cladpt`; post-83-09 the shim stamps `copadt`/`opcadt`/`mstadt`) between the
 * SELECT and the INSERT raises SQLITE_CONSTRAINT on the composite PK. That row
 * was previously silently dropped. The INSERT now runs a bounded
 * recompute-and-retry (the proxy logCall's D-11 discipline): a constraint
 * error that is NOT a `(user_hash, tool_call_id)` duplicate (probe-
 * disambiguated) recomputes MAX(id)+1 and retries; a genuine request-id
 * duplicate is dropped (return false, dedup semantics — never double-count).
 *
 ...
 */
const INSERT_ID_RETRY_ATTEMPTS = 3;

export function insertTokenRow(db, row) {
  try {
    const userHash = row.user_hash;
    const shape = insertShapeFor(db);

    for (let attempt = 1; attempt <= INSERT_ID_RETRY_ATTEMPTS; attempt += 1) {
      // Recomputed EVERY attempt — a lost race means another writer took the id.
      const seed = db.prepare(NEXT_ID_SQL).get(userHash);
      const id = seed ? seed.next : 1;

      try {
        db.prepare(shape.sql).run(
          ...baseValues(row, id, userHash),
          ...(shape.hasRouting ? routingValues(row) : []),
        );
        return true;
      } catch (err) {
        const isConstraint =
          err && typeof err.code === 'string' && err.code.startsWith('SQLITE_CONSTRAINT');
        if (!isConstraint) throw err; // non-constraint → outer catch (locked/closed/malformed)

        // Probe-disambiguation (D-11): a `(user_hash, tool_call_id)` duplicate
        // (partial-unique index) is a genuine dedup hit — drop, never retry.
        const tcid = row.tool_call_id;
        if (typeof tcid === 'string' && tcid.length > 0) {
          const dup = db
            .prepare('SELECT 1 FROM token_usage WHERE user_hash = ? AND tool_call_id = ? LIMIT 1')
            .get(userHash, tcid);
          if (dup) return false; // concurrent duplicate request-id — dedup drop
        }

        // Composite-PK id collision (a concurrent writer won MAX(id)+1) —
        // recompute and retry; give up (outer catch) after the final attempt.
        if (attempt === INSERT_ID_RETRY_ATTEMPTS) throw err;
      }
    }
    return false; // unreachable (loop returns or throws) — defensive
  } catch (err) {
 ...
 * enrich. CR-01 (re-review): it also selects `rowid AS rid` — the ROW IDENTITY
 * the reconcile loop's snapshot/consumed-set scoping keys on. The bare `id`
 * column is NOT unique (composite PK `(user_hash, id)` — per-hash sequences
 * collide across cladpt/copadt/machine hashes), so `ORDER BY rowid` makes the
 * earliest-INSERTED (wire) row the deterministic winner when more than one row
 * shares the request-id (pre-merge tap + adapter rows).
 */
const RECONCILE_PROBE_SQL =
  'SELECT rowid AS rid, id, tool_call_id, model, timestamp, input_tokens, output_tokens, ' +
  'reasoning_tokens, cache_read_tokens, cache_write_tokens, parent_call_id, granularity_tier ' +
  'FROM token_usage WHERE tool_call_id = ? ORDER BY rowid LIMIT 1';
