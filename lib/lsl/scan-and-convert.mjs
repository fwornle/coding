    ...
    if (ext === '.jsonl') {
      ...
      try {
        ...
        if (Array.isArray(messages) && messages.length > 0) {
          if (dryRun) {
            // Dry-run writes nothing, and reports nothing: observationsWritten is
            // the LIVE-write count the caller reads, so it stays 0 here.
            // NOTE: this branch used to call groupExchanges(messages) and drop the
            // result on the floor, under a comment promising to "record the would-be
            // count". Nothing recorded it. The dead call is removed rather than wired
            // up, because making dry-run report a non-zero observationsWritten would
            // change what the caller sees — a behaviour decision, not a cleanup.
