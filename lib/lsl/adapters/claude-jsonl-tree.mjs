...
/**
 * The inverse: a filesystem path to the encoded-cwd directory name Claude Code
 * uses under ~/.claude/projects.
 *   '/Users/you/Agentic/coding'  → '-Users-you-Agentic-coding'
 *   '/Users/you/Agentic/_work/rec' → '-Users-you-Agentic--work-rec'
 *
 * Exported because four callers need to locate THIS repo's transcript directory and
 * were otherwise each inlining the rule. It lives next to decodeEncodedCwd so the
 * two halves of one format cannot drift apart.
 *
 * SEPARATORS ARE '/', '.' AND '_' — all three become '-'. The underscore was
 * missing until 2026-09-17 and it is not a theoretical gap: every project under
 * `~/Agentic/_work/` encodes to `...-Agentic--work-...` (double dash), so this
 * function returned `-Users-Q284340-Agentic-_work-rec` — a directory that does
 * not exist — and every caller that builds a transcript path from it died with
 * ENOENT on exactly the projects most likely to need one. Verified against the
 * live `~/.claude/projects/` listing, which carries six such `--work-` dirs and
 * no `-_work-` dir at all.
 *
 * Note this is NOT pi's encoding, which wraps the result in a trailing '--'
 * (see encodePiSessionDir in lib/statusline/context-gauge.cjs).
 */
export function encodeCwd(dir) {
  return String(dir).replace(/\/+$/, '').replace(/[/._]/g, '-');
}
...
/**
 * parseClaudeExchanges — read a Claude sub-agent JSONL transcript and emit
 * the writer's `Array<{role:'user'|'assistant', content, timestamp}>` shape.
 *
 * Plan 51-06 Task 3 helper. The sub-agent JSONL contains one record per
 * message; each record has `type: 'user'|'assistant'`, `message.content`
 * (string OR array-of-blocks for tool-use frames), and `timestamp`.
 *
 * Filters non-sidechain records defensively (RESEARCH-claude.md
 * landmine #3). Truncated/malformed last lines are skipped without
 * aborting the whole file (landmine #5).
 *
 * For assistant messages whose `message.content` is a content-block
 * array (tool_use/tool_result mixed with text), only the text segments
 * are concatenated — tool-use payloads are NOT inlined here; the
 * writer's `shrinkLargeToolBlocks` cleanup covers the per-segment case
 * for parent-transcript backfills but Claude sub-agent transcripts
 * already store tool payloads outside `message.content`.
 *
 * @param {string} jsonlPath
 * @returns {Promise<Array<{role, content, timestamp}>>}
 */
export async function parseClaudeExchanges(jsonlPath) {
  const out = [];
  let text;
  try {
  ...
  for (const line of lines) {
    ...
    try {
      ...
      // Malformed line — skip defensively (landmine #5).
      continue;
    }
    if (!obj || typeof obj !== 'object') continue;
    // Defense-in-depth — only sidechain records belong in a sub-agent
    // transcript (landmine #3).
    if (obj.isSidechain === false) continue;
