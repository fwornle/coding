 ...
 * @returns {Promise<Array<{ row, observations: number }>>}
 */
async function convertToObservations(rows, { dryRun = false, tag } = {}) {
  if (!Array.isArray(rows) || rows.length === 0) return [];
  if (dryRun) {
    return rows.map((row) => ({ row, observations: 0 }));
  }
  ...
  for (const row of rows) {
    ...
    try {
      ...
      results.push({ row, observations: r?.observations || 0 });
    } catch (err) {
      process.stderr.write(`[copilot-adapter] convert failed for ${row.sub_hash}: ${err.message}\n`);
      results.push({ row, observations: 0, error: err.message });
    }
  }
