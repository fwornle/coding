...
async function convertToObservations(rows, { dryRun = false, tag = 'sub-agent-backfill' } = {}) {
  ...
  let writer = null;
  if (!dryRun) {
    // 2026-07-06: write via obs-api (single km-core owner). The former
    // ObservationWriter({dbPath}) path died with Phase 44 Plan 12 (SQLite
    // write path removed): it spent the LLM call, then threw on the km-core
    // write, so swept rows were never persisted and re-burned every sweep.
    ...
    try { await writer.init(); } catch (err) {
      process.stderr.write(
        `[opencode-adapter] ObservationApiClient.init failed: ${err.message}\n`,
      );
    }
  }
