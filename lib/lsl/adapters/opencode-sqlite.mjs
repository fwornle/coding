...
// ---- Locked schema-version allowlist (T-51-03-SV) -------------------------
// RESEARCH-opencode.md (file-format-detail / Known-landmines #3): the
// inspected box (OpenCode 1.15.1) has __drizzle_migrations.MAX(id) = 4.
// Refuse silent operation on a future schema. If OpenCode adds migrations:
//   (a) audit this file against the schema delta for breaking column changes,
//   (b) extend this allowlist with the additional MAX(id).
const SUPPORTED_MIGRATIONS = Object.freeze([1, 2, 3, 4]);
...
// ---- Schema-version guard (T-51-03-SV) -------------------------------------
...
// `assertSupportedSchema` kept as an internal alias so prior call sites inside
// this module remain byte-identical (no behavior change).
export function checkSchemaVersion(db) {
  let row;
  try {
    row = db.prepare('SELECT MAX(id) AS m FROM __drizzle_migrations').get();
  } catch (err) {
    throw new Error(
      `unsupported opencode schema - could not read __drizzle_migrations: ${err.message}`,
    );
  }
  const maxMigration = row?.m;
  if (!SUPPORTED_MIGRATIONS.includes(maxMigration)) {
    throw new Error(
      `unsupported opencode schema migration=${maxMigration}; ` +
      `supported=${SUPPORTED_MIGRATIONS.join(',')}; ` +
      `if you upgraded opencode, audit lib/lsl/adapters/opencode-sqlite.mjs ` +
      `against the schema before adding to SUPPORTED_MIGRATIONS`,
    );
  }
}
  ...
  for (const entry of searchPaths) {
    ...
    try {
      ...
      const sessionRows = db
        ...
        .all(projectRoot, dirPrefix, sinceMsArg, sinceMsArg, limit);
