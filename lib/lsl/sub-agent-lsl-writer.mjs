...
import { LSL_EXTENSION } from '../../scripts/timezone-utils.js';
import {
...
export function computeLSLFilename({
  ...
  return `${dateStr}_${hhhh_hhhh}_S${parentSlot}-${subIndex}-${subHash}${partSuffix}${LSL_EXTENSION}`;
}
 ...
 */
function renderExchangeSegment(userExchange, assistantExchange, spineId) {
  const userTs = userExchange && userExchange.timestamp
    ? Date.parse(userExchange.timestamp)
    : null;
  // The `<a name="ps_<ms>">` anchor becomes the promptSetId on an
  // `lsl.promptSet` entry. Same identity, same millisecond-timestamp scheme —
  // it is simply a field now instead of an HTML anchor a regex has to find.
  const promptSetId = Number.isFinite(userTs) ? `ps_${userTs}` : `ps_0`;
  const iso = Number.isFinite(userTs) ? new Date(userTs).toISOString() : null;
  ...
  const entries = buildPromptSetEntries({
    promptSetId,
    spineId,
    idGen: makeIdGen(`${spineId}:${promptSetId}`),
    fallbackIso: iso || new Date(0).toISOString(),
    meta: { time: iso, agent: 'sub-agent' },
    blocks: [{
      kind: 'text',
      time: iso,
      userText: userContent || null,
      assistantText: assistantExchange ? (assistantContent || null) : null,
    }],
  });
  return piSerialize(entries);
}
 ...
 */
function pairExchangesIntoSegments(exchanges, spineId) {
  const segments = [];
  let i = 0;
 ...
 */
function buildBody(exchanges, spineId, headerJsonl) {
  const segments = pairExchangesIntoSegments(exchanges, spineId);
  return headerJsonl + segments.map((s) => s.segment).join('');
}

/**
 * Header lines for a sub-agent tranche: pi `session` header, `session_info`
 * title, and the `lsl.tranche` spine.
 *
 * Replaces the YAML frontmatter block. Every frontmatter key survives in the
 * spine's `data` — but as JSON, so a consumer no longer needs a bespoke
 * `^key:\s*(.*)$` scrape per field to read it back.
 */
function buildHeaderJsonl(row, headingTs, filename, { partNumber, partTotal, parentSession }) {
  const spineId = piEntryId(`${filename}:spine`);
  const infoId = piEntryId(`${filename}:info`);
  const iso = headingTs.firstIso;

  const header = piSessionHeader({
    id: uuidFrom(filename), timestamp: iso, cwd: row.project || process.cwd(),
    ...(parentSession ? { parentSession } : {}),
  });

  // Field names are deliberately the frontmatter's snake_case, not camelCase:
  // consumers already key on them, the locked field set in the writer tests
  // asserts them, and the backfill of the 800 legacy dialect-C files must
  // produce the same keys as the live writer or the two would disagree.
  const meta = {
    kind: 'sub-agent',
    agent: row.agent || null,
    project: row.project || null,
    parent_session_id: row.parent_session_id || null,
    // Derived per-agent from agent_metadata (agent_id / toolCallId /
    // session_id), NOT read off the row — reuse the same helper the frontmatter
    // used so the value cannot diverge between the two formats.
    sub_session_id: subSessionIdFromRow(row),
    sub_index: row.sub_index ?? null,
    sub_hash: row.sub_hash || null,
    lsl_incomplete: Boolean((row.agent_metadata || {}).lsl_incomplete),
    lsl_incomplete_reason: (row.agent_metadata || {}).lsl_incomplete_reason || null,
    captured_via: 'sub-agent-backfill',
    captured_at: new Date().toISOString(),
    firstIso: headingTs.firstIso,
    lastIso: headingTs.lastIso,
    part: partNumber,
    part_total: partTotal,
  };

  let i = 0;
  const fixedGen = () => (i++ === 0 ? infoId : spineId);
  const title = `Sub-agent session — ${headingTs.firstIso} to ${headingTs.lastIso}`
    + (partTotal && partTotal > 1 ? ` (part ${partNumber}/${partTotal})` : '');
  const { entries } = buildTrancheEntries({ ...meta, timeWindow: title }, fixedGen, iso);
  // buildTrancheEntries names the session_info from timeWindow; override to the
  // sub-agent phrasing the markdown heading used.
  entries[0].name = title;

  return { spineId, jsonl: piSerialize([header, ...entries]) };
}

/**
 * Group EXCHANGES (not rendered strings) into size-bounded chunks.
 *
 * The markdown writer chunked pre-rendered text, which was fine when a chunk
 * was just concatenated markdown. Entries must be re-emitted per part instead,
 * because each part carries its own spine id — so chunking has to happen on
 * the inputs and the rendering has to follow.
 */
function chunkExchangesBySize(exchanges, segments) {
  // pairExchangesIntoSegments consumes 2 exchanges for a user+assistant pair
  // and 1 for an orphan; walk the same way to map segments back to inputs.
  const spans = [];
  let i = 0;
  let sIdx = 0;
  while (i < exchanges.length && sIdx < segments.length) {
    const cur = exchanges[i];
    if (!cur) { i += 1; continue; }
    const next = exchanges[i + 1];
    const take = (cur.role === 'user' && next && next.role === 'assistant') ? 2 : 1;
    spans.push({ start: i, take, byteSize: segments[sIdx].byteSize });
    i += take;
    sIdx += 1;
  }
  ...
  for (const sp of spans) {
    if (bytes + sp.byteSize > CHUNK_THRESHOLD_BYTES && current.length > 0) {
      groups.push(current);
      current = [];
      bytes = 0;
    }
    current.push(...exchanges.slice(sp.start, sp.start + sp.take));
    bytes += sp.byteSize;
  }
  if (current.length > 0) groups.push(current);
  return groups.length > 0 ? groups : [exchanges];
}
...
export async function writeSubAgentLSL({
  ...
  // Segments are sized before the spine id is known (the id depends on the
  // filename, which depends on the part count, which depends on the sizes).
  // Sizing with a placeholder is safe: the spine id is a fixed-width 8-hex
  // string, so byte counts do not shift when the real one is substituted.
  const SIZING_SPINE = '0'.repeat(8);
  const segments = pairExchangesIntoSegments(exchanges, SIZING_SPINE);
  const totalBytes = segments.reduce((acc, s) => acc + s.byteSize, 0);

  // Decide single-file vs chunked. Chunking still snaps at segment boundaries;
  // a segment is now a prompt-set subtree's JSONL lines rather than a markdown
  // block, so a chunk boundary can no longer land mid-token the way markdown
  // rotation did.
  const chunkExchangeGroups = totalBytes <= CHUNK_THRESHOLD_BYTES
    ? [exchanges]
    : chunkExchangesBySize(exchanges, segments);
  const totalParts = chunkExchangeGroups.length;
  ...
  for (let i = 0; i < totalParts; i += 1) {
    ...
    // Chain each part to its predecessor so a rotated sub-agent session is
    // navigable rather than a set of orphan fragments.
    const parentSession = i > 0 ? path.basename(prevFilePath) : null;
    const { spineId, jsonl: headerJsonl } = buildHeaderJsonl(row, headingTs, filename, {
      partNumber: totalParts > 1 ? i + 1 : null,
      partTotal: totalParts > 1 ? totalParts : null,
      parentSession,
    });
    const fileContent = buildBody(chunkExchangeGroups[i], spineId, headerJsonl);
