...
// ---- Resume cursor ---------------------------------------------------------
// How far the last run got. Without it a restart begins at lastPollTime=0 and
// the first pollOnce() walks EVERY historical sub-session for this project,
// re-feeding each one's whole message history to the observation writer — a
// long, LLM-backed replay of work that was already processed. Measured
// 2026-09-02 after a daemon restart: 78 sub-sessions, 11.5 minutes, and
// bg-observation-writer went from ~1 call/min to 37.
//
// TWO high-water marks are needed, and persisting only the first is a trap:
// lastPollTime alone stops old sessions being re-listed, but any session that
// IS re-listed (because it genuinely advanced) still re-sends every message it
// ever had, since lastSeenMessageIdBySession would restart empty.
//
// Losing the cursor is always safe, never silent: an absent or malformed file
// degrades to the old cold-start behaviour rather than skipping work, because
// under-replaying would lose observations while over-replaying only costs.
const CURSOR_MAX_SESSIONS = 1000;

/**
 * Normalise a persisted cursor into the shape the watcher state expects.
 * Tolerates absent / malformed / partially-written input by falling back to
 * "no cursor" per field rather than throwing — a corrupt heartbeat file must
 * not stop the daemon from starting.
 *
 * @param {any} raw
 * @returns {{ lastPollTime: number, lastSeenMessageIdBySession: Map<string,string> }}
 */
export function readCursor(raw) {
  const out = { lastPollTime: 0, lastSeenMessageIdBySession: new Map() };
  if (!raw || typeof raw !== 'object') return out;

  const t = raw.last_poll_time;
  if (typeof t === 'number' && Number.isFinite(t) && t > 0) out.lastPollTime = t;

  const seen = raw.last_seen_message_id_by_session;
  if (seen && typeof seen === 'object' && !Array.isArray(seen)) {
    for (const [sessionId, msgId] of Object.entries(seen)) {
      if (typeof sessionId === 'string' && typeof msgId === 'string' && msgId) {
        out.lastSeenMessageIdBySession.set(sessionId, msgId);
      }
    }
  }
  return out;
...
/**
 * Serialise the live state into a persistable cursor. Bounded: a daemon that
 * runs for months would otherwise carry one map entry per sub-session forever.
 * Map iteration is insertion-ordered, so the cap drops the oldest-first — the
 * sessions least likely to be written to again.
 *
 * @param {{ lastPollTime: number, lastSeenMessageIdBySession: Map<string,string> }} state
 * @returns {{ last_poll_time: number, last_seen_message_id_by_session: object }}
 */
export function writeCursor(state) {
  const entries = [...state.lastSeenMessageIdBySession.entries()];
  const kept = entries.length > CURSOR_MAX_SESSIONS
    ? entries.slice(entries.length - CURSOR_MAX_SESSIONS)
    : entries;
  return {
    last_poll_time: state.lastPollTime,
    last_seen_message_id_by_session: Object.fromEntries(kept),
  };
}
 ...
 * @param {object} [args.cursor]           resume point from a previous run —
 *                                         see readCursor() below. Omit for a
 *                                         cold start (replays all history).
...
export async function startOpencodeWatcher({
  ...
  // ---- Watcher state -----------------------------------------------------
  const resumed = readCursor(cursor);
  const state = {
    lastPollTime: resumed.lastPollTime,
    lastSeenMessageIdBySession: resumed.lastSeenMessageIdBySession,
    pollCount: 0,
    errorCount: 0,
    lastPollAt: null,
    stopped: false,
  ...
  const handle = {
    ...
    getStats() {
      return {
        ...
        // Rides along on every heartbeat so the daemon persists it without a
        // second write path — whatever cadence the heartbeat has IS the
        // checkpoint cadence, and a crash between heartbeats costs a re-poll
        // of that window, which is idempotent.
