...
function handleEvent(state, opts, eventType, filename) {
  ...
  const tailHandle = tailFile(fullPath, {
    ...
    onMessage: async (exchange) => {
      ...
      // --- Phase 69 (Plan 69-05 Task 1): additive, FULLY-ISOLATED token-row hook.
      //
      // Invoke the OPTIONAL opts.onTokenRow alongside the observation path,
      // passing the source JSONL path + the paired exchange. This hook is the
      // host-side supervisor's seam to build + insert per-turn / per-reasoning
      // token rows (D-08). It is wrapped in ITS OWN try/catch that writes a
      // `[claude-fs-watch] onTokenRow threw (non-fatal)` stderr line and
      // continues — a token-emission failure must NEVER propagate into the
      // observation path above NOR into the error budget (opts.onError is
      // DELIBERATELY NOT called here, keeping token failures isolated from the
      // daemon's restart accounting — D-08 failure isolation).
      if (typeof opts.onTokenRow === 'function') {
        try {
          await opts.onTokenRow({ fullPath, exchange });
        } catch (err) {
          process.stderr.write(
            `[claude-fs-watch] onTokenRow threw (non-fatal): ${err.message}\n`,
          );
        }
      }
    },
 ...
 * @param {(ctx: {fullPath: string, exchange: Array}) => Promise<void>|void} [opts.onTokenRow]
 *        OPTIONAL Phase-69 additive hook invoked alongside the observation
 *        write. Fully isolated — a throw here NEVER reaches onError or the
 *        observation path (D-08).
