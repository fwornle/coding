 ...
 * first ship): file-tail via a statSync poll on
 ...
 *   T-51-09-SC  Zero new package installs (fs.statSync/setInterval are stdlib).
 *
...
/**
 * Per-file tail polling interval (ms). Matches Plan 51-07's per-file tail
 ...
 *
 * The projectRoot may be a path (`/path/to/coding`) or a
...
/**
 * tailEventsFile — start a polled tail on events.jsonl. Returns
...
function tailEventsFile({ eventsPath, onSubagentStarted, onSubagentEnded, onError, onTokenRow }) {
  ...
  try {
    ...
    // file may not exist yet — the poll below will pick up the first write
  }

  const listener = (curr, _prev) => {
    if (closed) return;
  ...
  // Poll against OUR OWN lastSize rather than letting libuv decide when to look.
  //
  // This was fs.watchFile({interval, persistent:false}). fs.watchFile returns
  // before libuv's StatWatcher has taken its baseline stat off-thread, so an
  // append landing in that window left prev.size already at the post-append
  // value. curr.size never differed from it again, so the listener was never
  // invoked AT ALL — not late, never. The bytes were on disk and lastSize (taken
  // synchronously above, before this line) was correct; nothing could recover
  // them because nothing ever looked.
  //
  // In production that costs a session which writes once and then goes quiet: a
  // subsequent append would have moved libuv's own size and unstuck it, so busy
  // sessions self-healed and hid this. In CI it was a permanent hang that
  // exhausted any budget, on whichever test lost the race (run 34222700075 Test
  // 11, run 34223544188 Test 5) — indistinguishable from timer starvation except
  // that starvation eventually succeeds and this never does.
  //
  // statSync against a lastSize we set ourselves has no baseline to lose. Still
  // unref'd: this must not hold the event loop open on its own.
  const pollTail = () => {
    if (closed) return;
    let st;
    try {
      ...
      return; // file may not exist yet — a later tick will find it
    }
    if (st.size === lastSize) return;
    listener(st);
  };
  const pollTimer = setInterval(pollTail, TAIL_POLL_INTERVAL_MS);
  ...
  async function scanLoop() {
    ...
    for (const { sessionId, sessionDir } of live) {
      ...
      try {
        ...
        // file doesn't exist yet — the per-file poll handles it when it appears
      }
