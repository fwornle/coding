...
import { TranscriptAdapter } from '../transcript-api.js';
...
class CopilotHooksManager extends HooksManager {
  ...
  async installBridge() {
    ...
    // GitHub Copilot CLI native hook schema v1: top-level `version` + `hooks`
    // object; each entry uses `type`/`bash` (a shell command string), NOT
    // `command`/`args`. The `bash` value runs through bash, so an absolute
    // bridgePath is robust regardless of the launch cwd. Do NOT place
    // `$VAR` in `cwd` — Copilot does not expand env vars there and a literal
    // path yields `spawn bash ENOENT`, which fails closed and bricks the CLI.
    const hook = (event) => ({
      type: 'command',
      bash: `'${bridgePath}' ${event}`,
      timeoutSec: event === 'preToolUse' ? 15 : 10
    });
...
class CopilotTranscriptAdapter extends TranscriptAdapter {
  ...
  getTranscriptDirectory(_projectPath) {
