...
import { encodeCwd } from '../../lsl/adapters/claude-jsonl-tree.mjs';
import process from 'node:process';

/** Home default for the Claude transcript store (this repo's project slug). */
const HOME_TRANSCRIPT_DIR = path.join(
  os.homedir(), '.claude', 'projects',
  encodeCwd(process.env.CODING_REPO || path.resolve(import.meta.dirname, '..', '..', '..')),
);
 ...
 * env override → `~/.claude/projects/<encoded-cwd>` home
