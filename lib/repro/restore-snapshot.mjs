...
/**
 * Build the `git worktree add` argv for a restore. avenueMode (Phase 87, AVN-05) → a NAMED
 * `branchName` worktree (`worktree add -b <branch> <path> <sha>`); otherwise the detached default
 * (`worktree add --detach <path> <sha>`) is emitted BYTE-FOR-BYTE (regression anchor). Pure +
 * exported so the argv contract is unit-testable without a live git run.
 * @param {{ worktree: string, sha: string, avenueMode?: boolean, branchName?: string }} args
 * @returns {string[]}
 */
export function buildWorktreeAddArgs({ worktree, sha, avenueMode, branchName } = {}) {
  if (avenueMode === true && typeof branchName === 'string' && branchName.length > 0) {
    return ['worktree', 'add', '-b', branchName, worktree, sha];
  }
  return ['worktree', 'add', '--detach', worktree, sha];
}

/**
 * Materialize the sandbox as an INDEPENDENT repository rather than a linked worktree.
 *
 * WHY THIS EXISTS (measured 2026-08-23). A `git worktree` keeps `--git-common-dir` pointing at
 * the ORIGIN repo's `.git`, and Claude Code derives a session's project identity from that — so
 * a cell running in a worktree of this repo is handed THIS repo's project memory index in its
 * system prompt. Measured directly with a no-tools probe ("name your memory dir from the system
 * prompt alone"): a worktree answered
 *   /Users/…/.claude/projects/-Users-…-Agentic-coding/memory/  + real MEMORY.md filenames
 * and kept answering that after (a) every repo-naming env var was scrubbed and (b) the worktree
 * was moved outside the repo tree. Neither of the two fixes proposed from the 2026-08-22 run
 * touches the actual channel. An independent clone at the same SHA answers with its OWN path and
 * no index — project identity follows `.git`, so the only way to repoint it is to give the
 * sandbox a `.git` of its own.
 *
 * The clone is `--local`, so objects are hardlinked (≈4 s, near-zero new disk). The origin's
 * `.git/modules` is copied in FIRST so `submodule update --init` resolves every submodule from
 * local object stores instead of re-cloning them over the network (≈8 s total; a network fetch
 * here would make cells fail offline and behind the corporate proxy).
 *
 * @param {object} args
 * @param {string} args.repoRoot origin repo to clone from
 * @param {string} args.worktree destination sandbox path
 * @param {string} args.sha commit to check out detached
 * @returns {{ ok: boolean, stderr: string }}
 */
function materializeIndependentRepo({ repoRoot, worktree, sha }) {
  const cloned = git(['clone', '--local', '--no-checkout', repoRoot, worktree], repoRoot);
  if (!cloned.ok) return { ok: false, stderr: `clone failed: ${cloned.stderr.trim()}` };

  // Seed the module store BEFORE checkout so `submodule update --init` (run by
  // reconstructWorkingTree) never reaches the network. Best-effort: a repo with no submodules
  // has no modules dir, and a copy failure degrades to the network path rather than aborting.
  const srcModules = path.join(repoRoot, '.git', 'modules');
  if (fs.existsSync(srcModules)) {
    try {
      fs.cpSync(srcModules, path.join(worktree, '.git', 'modules'), { recursive: true });
    } catch (err) {
      process.stderr.write(
        `[restore-snapshot] module-store seed failed (submodules will fetch over the network): ${err?.message || err}\n`,
      );
    }
  }

  const co = git(['-C', worktree, 'checkout', '--detach', sha], worktree);
  if (!co.ok) return { ok: false, stderr: `checkout ${sha} failed: ${co.stderr.trim()}` };
 ...
 * @param {boolean} [opts.independentRepo=false] materialize the sandbox as its OWN clone instead of
 *                  a linked worktree, so its project identity does not resolve back to `repoRoot`
 *                  (2026-08-23 isolation fix — see materializeIndependentRepo). Rejects avenueMode.
 * @param {string}  [opts.ontologyDir] ontology dir for the sandbox KB hydrate (kb-capture default applies)
 * @param {boolean} [opts.avenueMode=false] create a NAMED `branchName` worktree (Phase 87, AVN-05)
 *                  instead of the detached default. Requires a non-empty `branchName` to take effect.
 * @param {string}  [opts.branchName] the `avenue/<task_id>` branch ref to create (avenueMode only).
...
export async function restoreSnapshot(snapshotId, opts = {}) {
  ...
  // avenueMode (Phase 87, AVN-05): create a NAMED `avenue/<task_id>` branch worktree instead of
  // the detached default so the avenue's edits persist as commits on a real branch (Plan 03/05).
  // When avenueMode is NOT requested, the `--detach` argv below is preserved BYTE-FOR-BYTE — the
  // regression-critical default that cells + repro-restore depend on (RESEARCH §4: the ONE change).
  const avenueMode = opts.avenueMode === true && typeof opts.branchName === 'string' && opts.branchName.length > 0;
  // `independentRepo` (2026-08-23) repoints the sandbox's project identity by giving it its own
  // `.git` — see materializeIndependentRepo for the measurement that motivates it. It is
  // incompatible with avenueMode, whose whole purpose is a NAMED branch in the ORIGIN repo that
  // outlives the sandbox: a clone's branch would be unreachable from here. Fail loudly rather
  // than silently picking one, so a caller asking for both learns it cannot have both.
  const independentRepo = opts.independentRepo === true;
  if (independentRepo && avenueMode) {
    throw new Error(
      'restore aborted: independentRepo and avenueMode are mutually exclusive — an avenue branch ' +
        'must live in the origin repo, but an independent clone has its own object store',
    );
  }
  if (independentRepo) {
    const made = materializeIndependentRepo({ repoRoot, worktree, sha });
    if (!made.ok) {
      throw new Error(`restore aborted: independent clone of ${repoRoot} at ${sha}: ${made.stderr}`);
    }
  } else {
    const wtArgs = buildWorktreeAddArgs({ worktree, sha, avenueMode, branchName: opts.branchName });
    const wt = git(wtArgs, repoRoot);
    if (!wt.ok) {
  ...
  try {
    ...
    return {
      ...
      replayArmed,
      inPlace: false,
      // avenueMode records the created branch for observability; null on the detached default.
      steps: { worktree: true, branch: avenueMode ? opts.branchName : null, ...steps, kb: kbHydrated, config: configWritten },
    };
  } catch (err) {
