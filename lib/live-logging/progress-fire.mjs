 ...
 *
 * Two independent triggers — a fire happens when EITHER is satisfied:
 *   • token-delta: cumulative output tokens grew by >= thresholdTokens since the
 *     last fire. Inert for agents whose transcripts lack usage.output_tokens
 *     (opencode/copilot → setOutputTokens always 0; pi reports real usage).
 *   • wall-clock: >= elapsedThresholdMs since the last fire (or turn start). The
 *     ONLY trigger that covers token-less agents, so their long autonomous turns
 *     still emit periodic snapshots instead of collapsing to one prompt-time row.
 *
 * @param {object} args
 * @param {number} args.setOutputTokens    cumulative output tokens of the live set
 * @param {number} args.firedOutputTokens  the token mark stored at the last fire (0 if none)
 * @param {number} args.thresholdTokens    output-token delta that triggers a fire; <=0 disables the token trigger
 * @param {number} [args.nowMs]            current wall-clock ms (enables the time trigger)
 * @param {number} [args.lastFiredAtMs]    wall-clock ms of the last fire OR the turn start (caller-seeded)
 * @param {number} [args.elapsedThresholdMs] elapsed-ms delta that triggers a fire; <=0 disables the time trigger
 * @returns {{ fire: boolean, mark: number, markAt: number }}
 *   fire   — emit an in-progress observation now
 *   mark   — the value the caller must store as the new firedOutputTokens
 *   markAt — the value the caller must store as the new lastFiredAtMs
 */
export function progressFireDecision({
  setOutputTokens,
  firedOutputTokens,
  thresholdTokens,
  nowMs,
  ...
  const lastAt = typeof lastFiredAtMs === 'number' && lastFiredAtMs > 0 ? lastFiredAtMs : null;
  ...
  const tokenFire = thresholdTokens > 0 && (tokens - base >= thresholdTokens);
  const timeFire =
    elapsedThresholdMs > 0 && now !== null && lastAt !== null && (now - lastAt >= elapsedThresholdMs);

  if (tokenFire || timeFire) {
    return { fire: true, mark: tokens, markAt: now !== null ? now : (lastAt || 0) };
  }
  return { fire: false, mark: base, markAt: lastAt || (now !== null ? now : 0) };
}
