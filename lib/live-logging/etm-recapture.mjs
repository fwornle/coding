...
function makeFire(batch, taskId) {
  ...
  // CR-01: a missing/unparseable timestamp makes tsMs() return NaN and
  // new Date(NaN).toISOString() throws RangeError, which would escape this
  // advertised-pure function. Fall back to the most-recent parseable message
  // in the batch, then to wall-clock — NEVER throw.
  let ms = tsMs(last);
