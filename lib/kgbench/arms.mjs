...
export function resolveArm(armsDoc, id, { repoRoot = REPO_ROOT, env = process.env } = {}) {
  ...
  return {
    ...
    // Turns an answer-file agent may spend RESUMING its own session after finishing without
    // writing an answer. Applied to every such agent identically — the point is to stop the
    // comparison being decided by how many turns each CLI happens to grant by default
    // (claude's loop is unbounded, copilot gets 20 autopilot continues, opencode's headless
    // `run` gets one turn). 0 preserves the pre-existing behaviour exactly.
    continuationBudget: raw.continuationBudget ?? defaults.continuationBudget ?? 0,
