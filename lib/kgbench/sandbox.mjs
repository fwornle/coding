...
export const DEFAULT_EXCLUDES = [
  ...
  // The AGENT ADAPTERS. They carry the answer-file directive verbatim — "write your complete
  // answer to `<file>` … the task is complete ONLY once it exists" — which is an instruction
  // addressed to the very model that is searching the tree. opencode found it, correctly
  // classified it as a prompt-injection attempt, announced it was ignoring it, and stopped;
  // claude never sees the directive (stream-json elicitation) and copilot complied with it 96
  // times without comment. So the file penalised exactly one agent, which is the specific way
  // a cross-agent comparison stops meaning anything. No question's evidence points here.
  'lib/kgbench/agents.mjs',
  // The judge's prompt states, in prose, what a right answer must contain and how it is
  // scored. That is the same crib as the deterministic grader's comments, and it was
  // missed when those two were excluded — grading machinery is a category, not a file.
  'lib/kgbench/judge.mjs',
  ...
  // ANALYSIS MACHINERY, and the fifth instance of the pattern this list already documents
  // twice: a file written to reason ABOUT the benchmark becomes the next leak.
  //
  // tool-selection-analysis.mjs tests whether each question is answerable from a literal
  // grep of a token its own prompt hands over, so it necessarily carries one search term
  // per question — including T3's, which is a phrase whose whole probe is that it appears
  // NOWHERE in this repository. Committing that script put it in the tree and the
  // containment check caught it on the next run, exactly as designed.
  //
  // Excluded as a DIRECTORY rather than a file: anything that lands in scripts/analysis/
  // is by construction about the benchmark and will accrete question content the same way.
  // No question's evidence points here, so removing it costs the benchmark nothing —
  // the same argument that removed graders.mjs and judge.mjs.
  'scripts/analysis',
  ...
  // A GLOB here, unlike everywhere else in this list, and the scoping is the reason it is
  // safe: `docs/measurement/` holds only measurement prose and no question's evidence,
  // whereas the earlier `docs/**/kgbench*.md` reached into docs/benchmarks/ and matched a
  // file that WAS one question's ground truth. Every benchmark-meta doc is named with the
  // `kgbench-` prefix precisely so one narrow glob per mirror covers them — the guide, the
  // experimental-design deep dive and the tutorial all describe what the probes test.
  ...
  // THE docs-content/ MIRROR OF THE SAME REPORT. mkdocs builds from docs-content/
  // (`docs_dir: docs-content`), so every published benchmark doc exists TWICE and an
  // exclusion naming only one tree removes half a leak. The mirror pattern was already
  // applied to the measurement docs above — `docs/measurement/kgbench*.md` AND
  // `docs-content/measurement/kgbench*.md` — and simply never extended to benchmarks/,
  // which is the whole defect: docs-content/benchmarks/coding-v1/analysis/
  // tool-selection-data.md echoed T3's leak term and refused the run after
  // docs/benchmarks/coding-v1 had already been excluded.
  //
  // The mirror holds markdown only (mkdocs does not copy the json/jsonl data files), so it
  // is a subset of its docs/ twin — but a subset that still contains prompts and leak terms.
  //
  // NOT cited by any question: A2's evidence names docs/benchmarks/kgbench-replication/
  // README.md, the docs/ copy specifically, and nothing anywhere cites docs-content.
  // kgbench-replication is therefore excluded from NEITHER tree, as before.
  'docs-content/benchmarks/coding-v1',
  // The ANCESTOR benchmark, and the answer key for the `replication` question set — which
  // is the DEFAULT set (kgbench-run.mjs: `opt('set', 'replication')`), so this blocked every
  // bare `kgbench-run.mjs` invocation. Its queries.json shares the replication set's ids and
  // prompts verbatim: 8 of 9 matched at full window coverage (L3, S2, S3, B1, B2, A1 all
  // 6/6). coding-v1 is unaffected — 0 leaks — which is why this only ever surfaced on the
  // default.
  //
  // The DIRECTORY, not the two offending files, and results.jsonl is the reason. The
  // containment scan greps PROMPT text, and results.jsonl stores `{id, answer, score}` with
  // no prompt — 54 rows mapping every question id to its graded answer. So it is the answer
  // key in the most literal sense and the scan cannot see it. Excluding only queries.json
  // would silence the guard while leaving those answers in the tree: the run would proceed
  // and be contaminated INVISIBLY, which is strictly worse than being refused. Same
  // "end the class, not the instance" argument that took scripts/analysis as a directory.
  //
  // Literal, NOT a glob over docs/benchmarks/ — see the coding-v1 note above:
  // docs/benchmarks/kgbench-replication/README.md is A2's ground truth and a glob would
  // make A2 unanswerable. Nothing cites this directory as any question's evidence, so
  // removing it costs the benchmark nothing.
  'docs/benchmarks/graphify-vs-grep',
  // Its mirror, for the same reason. Currently only README.md — the data files that carry
  // the prompts and the graded answers live solely in docs/ — so it does not leak TODAY.
  // Listed anyway: the pair is the unit, and an exclusion that holds only because the
  // mirror happens to be thin is one published file away from silently reopening.
  'docs-content/benchmarks/graphify-vs-grep',
...
/**
 * The harness's own source — the code that runs and grades the benchmark. A question's
 * wording appearing anywhere in here is a crib by construction, never a coincidence.
 *
 * Matched against the tree-relative path, so a needle hit is attributable without needing the
 * scan to know where the repo root is.
 */
const HARNESS_SOURCE_RE = /(^|\/)(lib\/kgbench\/|scripts\/kgbench-)/;

/**
 * Whether a tree-relative path is one this list already removes. An excluded file cannot be
 * read by an arm, so wording inside it is not a leak however decisive it looks — and treating
 * it as one would make the containment check fail on files whose whole point is to be absent.
 */
function isExcludedPath(rel) {
  return DEFAULT_EXCLUDES.some((e) => {
    if (e.includes('*')) {
      // NUL is a sentinel, not data: `**` is parked as \x00 so the single-`*` pass
      // cannot see it, then restored as `.*`. NUL is chosen because it cannot occur
      // in a path. Written as \x00 rather than the raw byte the source used to
      // carry — a literal NUL renders as nothing and read as `^@` in a pager.
      // eslint-disable-next-line no-control-regex -- deliberate: NUL sentinel
      const rx = new RegExp(`^${e.replace(/[.+^${}()|[\]\\]/g, '\\$&').replace(/\*\*/g, '\x00').replace(/\*/g, '[^/]*').replace(/\x00/g, '.*')}$`);
      return rx.test(rel);
    }
...
export function classifyLeaks(hits, questions, { minWindows = 3, minRatio = 0.5 } = {}) {
  ...
  for (const e of byPair.values()) {
    ...
    //
    // A hit inside the HARNESS'S OWN SOURCE is decisive for the same reason, and this is the
    // check that was missing. The thresholds tolerate coincidence: ordinary repo prose shares
    // vocabulary with a question about that repo, so two matching windows out of five prove
    // nothing. The harness's own code shares no vocabulary with the questions by accident —
    // if a prompt's wording appears there, someone wrote it there, and the arms grepping that
    // file are being handed the question by the thing grading them.
    //
    // It had already happened and passed: a comment in runner.mjs quoted L1's prompt and named
    // the file that is L1's answer. Two of five windows matched, `minWindows` is three, so it
    // was filed as `weak` and the run proceeded. Every arm that grepped L1's subject found the
    // answer in the harness. Files that never reach the tree (graders, judge, this file) are
    // exempt — they are excluded, so what they contain cannot be read by anything.
    const inHarnessSource = HARNESS_SOURCE_RE.test(e.file) && !isExcludedPath(e.file);
    const decisive = e.needle.startsWith('term:') || inHarnessSource;
    const isLeak = decisive || (entry.matched >= minWindows && entry.ratio >= minRatio);
    if (isLeak && inHarnessSource) entry.reason = 'question wording found in harness source';
...
/**
 * Check that a code-graph index actually covers the tree it is supposed to.
 *
 * Takes the PARSED `codegraph status --json` object rather than shelling out, so the
 * policy is unit-testable without a 40-second index build.
 *
 * WHY THIS IS A HARD GATE. The preflight it supplements only asked whether an index FILE
 * existed somewhere on the host — which was true throughout every run to date, while the
 * server was answering about a different tree entirely. "A preflight that proves a resource
 * exists has not proved it is the right resource." Returns the problems; the caller decides,
 * but the only correct decision is to stop.
 */
export function indexCoverageProblems(status, { project }) {
  const problems = [];
  if (!status || typeof status !== 'object') return [`no status returned for ${project}`];
  if (!status.initialized) problems.push(`index not initialized at ${project}`);
  if (!(status.fileCount > 0)) problems.push(`index at ${project} covers 0 files`);
  // The decisive one. An index can be initialized and populated and still describe the
  // WRONG PROJECT — that is exactly the failure this whole change exists to close.
  if (status.projectPath && status.projectPath !== project) {
    problems.push(`index describes ${status.projectPath}, expected ${project}`);
  }
  return problems;
}
 ...
 *
 * TWO OPTIONAL OVERRIDES, both defaulting to the historical behaviour:
 *
 *   `at`  — pin the corpus to a commit instead of HEAD. A rerun that exists to change
 *           ONE variable must hold the searched tree still; without this the harness fix
 *           and the corpus move together and the comparison has two variables.
 *
 *   `dir` — build somewhere specific instead of os.tmpdir(). The tmp default is right for
 *           the arms' tree and WRONG for anything the container must read: `docker exec`
 *           sees only what compose mounts, and /var/folders is not mounted, so a tmp tree
 *           is invisible to a container-side index. Callers that need one pass a path
 *           under .data/, which IS mounted (rw at /coding/.data).
 */
export function createRunTree({
  repoRoot, questions, excludes = DEFAULT_EXCLUDES, verify = true, onWorktreeCreated,
  at = null, dir: dirOverride = null,
}) {
  let commit;
  try {
    commit = git(repoRoot, ['rev-parse', at ?? 'HEAD']);
  } catch (err) {
    throw new SandboxError(
      at
        ? `cannot resolve commit "${at}" to sandbox the run: ${err.message}`
        : `not a git checkout, cannot sandbox the run: ${err.message}`,
    );
  }

  const dir = dirOverride ?? mkdtempSync(path.join(os.tmpdir(), 'kgbench-tree-'));
  // mkdtemp created it; `git worktree add` insists on creating the path itself. An
  // explicit dir gets the same treatment: a leftover from a killed run would otherwise
  // make `worktree add` refuse, and the caller's path is deterministic (keyed on run id)
  // so a retry of the same run must not be blocked by its own debris.
  rmSync(dir, { recursive: true, force: true });
  if (dirOverride) mkdirSync(path.dirname(dir), { recursive: true });
  ...
  // Hand the caller its cleanup NOW, not at the end.
  //
  // Everything after this point — the exclusion sweep and the containment verification —
  // takes about a minute on this repo, and until this function RETURNS the caller has no
  // handle on the worktree that already exists. A signal arriving in that window leaked it:
  // the directory is present, so the `worktree prune` above (which only drops entries whose
  // directories are gone) cannot reclaim it either, and the next run fails at setup.
  //
  // That window used to be unreachable in practice — a person Ctrl-Cs a benchmark minutes in,
  // not seconds. The dashboard's Cancel button makes it a normal thing to do, and hit it on
  // the first try.
  if (typeof onWorktreeCreated === 'function') {
    try {
      onWorktreeCreated({ dir, cleanup });
    } catch {
      // A caller whose registration throws must not cost us the tree we just built; it only
      // costs them the early cleanup they asked for.
    }
  }
