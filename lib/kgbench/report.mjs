...
function renderProvenance({ agents, provenance, byArmAgent }) {
  ...
  if (derivedTokens) {
    ...
    if (provenance?.ambiguous_token_rows) {
      // The cause is NOT asserted here. This warning used to say "another session of that agent
      // runs alongside the benchmark; re-run on an otherwise idle machine", and on run
      // coding-v1-r8 that was wrong and would have sent a reader to a fix that changes nothing:
      // the machine was idle of other opencode work, and the second session was the PREVIOUS
      // CELL'S, absorbed because the resolver's window opens before the cell spawns. It was
      // confirmed by arithmetic — one flagged cell's 274,139 tokens is exactly its own 139,727
      // plus its predecessor's 134,412. Concurrent work is one cause; a neighbour bleeding
      // across the window boundary is another; they need different fixes, so the reader is
      // pointed at the evidence rather than at a guess.
      L.push(`> **${provenance.ambiguous_token_rows} cell(s) had more than one session of the same agent `
        + 'inside their window.** Their token figures may include traffic that is not the cell\'s, and '
        + 'they are the rows to drop before quoting a cost. Two causes produce this and they are not '
        + 'distinguishable from the count alone: another session of that agent running alongside the '
        + 'benchmark, or an adjacent cell of this same run bleeding across the window boundary. Compare '
        + 'a flagged cell\'s total against its neighbours\' in the proxy token DB before concluding '
        + 'which — if the sum equals this cell plus the one before it, an idle machine will not fix it.', '');
    }
  }
