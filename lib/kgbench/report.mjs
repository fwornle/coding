 ...
 */
function renderProvenance({ agents, provenance, byArmAgent, mergedFrom }) {
  const L = [];
  ...
  // A report whose rows come from more than one RUN must say so, in the section a reader
  // checks before quoting a number. Rows carry arm, agent and task_id but not which run
  // produced them, so without this the merge is invisible in the artefact — the same defect
  // class as a silently dropped cell, which this file refuses elsewhere ("Failed runs are
  // counted, never dropped").
  if (mergedFrom?.length) {
    L.push('**Rows in this report come from more than one run.** Every source below used the '
      + 'same sandboxed corpus, question set, reps and continuation budget — that is what makes '
      + 'them poolable — but they were executed separately, so anything that varies between runs '
      + '(model nondeterminism, provider routing at the time) varies across these rows too.', '');
    L.push('| Run | Supplies | Why |', '|---|---|---|');
    for (const m of mergedFrom) L.push(`| \`${m.run}\` | ${m.supplies} | ${m.why} |`);
    L.push('');
  }
...
export function renderMarkdown(report) {
  ...
  if (byArmAgent && agents.length > 1) {
    ...
    L.push('');
    // Was: "blank for any agent elicited by answer file ... only claude reports a tool
    // trace". That stopped being true when copilot and opencode gained toolTraceFrom()
    // parsers, and the sentence then contradicted the very column it was explaining —
    // the table showed copilot 6.0 and opencode 1.0 while the prose called them dashes.
    // A dash still means not-measured; what changed is WHICH cells produce one.
    L.push('`tool calls` is a dash only where the cell produced no tool trace at all — a dash means '
      + '**not measured**, never zero. claude reports one via stream-json; copilot and opencode now '
      + 'report one parsed from their own JSON event streams. Their counts are in THEIR OWN tool '
      + 'vocabulary (`view`/`create`, `read`/`write`) rather than the arm\'s, which is why those cells '
      + 'carry `tool_audit: observed` — what ran is recorded, but conformance to the arm is not '
      + 'decidable from it. copilot\'s count includes one `task_complete` autopilot sentinel per cell, '
      + 'reported separately as control calls; claude has no equivalent, so subtract it before '
      + 'comparing tool counts across agents.', '');
  }
  ...
  L.push(...renderProvenance({ agents, provenance, byArmAgent, mergedFrom: report.merged_from }));
