...
//
// The SECOND version of that mistake sat here until 2026-08-10, and it is subtler: this said
// claude-code/claude-opus-5, which claude-code genuinely does serve — verified stable over 3
// repeats at ~1.7s. Availability was never the problem. Reachability under load was.
//
// claude-code answers RATE_LIMITED on its direct API far more often than not, and the proxy's
// fallback ("via CLI worker pool, same Max subscription, different rate-limit bucket") returns
// claude-haiku-4-5 whatever model it was asked for — the worker is spawned under
// `key=claude-opus-5::…` and still answers as haiku. On 2026-08-09 the judge got 21 opus-5
// calls and 2065 haiku ones, and the haiku stretch covered run coding-v1-x2. So the mismatch
// warning below fired constantly and correctly, and the answer was not to silence it but to
// stop asking for a model the route cannot promise.
//
// Copilot has no Opus at any version — `400 The requested model is not supported`, re-probed
// 2026-08-10 — but it returns exactly what it is asked for on everything it does have. That
// makes claude-sonnet-5 the strongest DETERMINISTIC judge, which is the property that matters
// for a grading instrument: a judge whose model varies with a rate-limit window is not a
// yardstick. Pair with the matching processOverride, or the pin here is only half applied:
//
//     scripts/configure-wave-analysis-routing.sh     # kgbench-judge -> copilot/claude-sonnet-5
//
// Scores from this judge are not comparable to runs graded by haiku (which is most of what
// preceded it). Re-grade a run or re-run it; do not mix.
//
// Do not take the proxy's providerModels catalog as evidence of availability either way: it
// advertises claude-opus-4.6 (which copilot rejects) and omits both claude-opus-5 and
// claude-sonnet-5 (which are served). Establish availability with
// `scripts/llm-model-probe.mjs`, which probes each provider directly and repeats, because a
// COLD model falls back to haiku on the first request and answers correctly on the second.
export const JUDGE_PROVIDER = process.env.KGBENCH_JUDGE_PROVIDER || 'copilot';
export const JUDGE_MODEL = process.env.KGBENCH_JUDGE_MODEL || 'claude-sonnet-5';
