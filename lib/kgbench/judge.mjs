...
function buildPrompt({ question, answer, checklist, rubric }) {
  const facts = (checklist ?? []).map((f, i) => `  ${f.id ?? `f${i + 1}`}: ${f.desc ?? JSON.stringify(f.match)}`).join('\n');
  return [
    ...
    question,
    '',
    checklist?.length ? 'REQUIRED FACTS (grade each independently):' : 'RUBRIC:',
    checklist?.length ? facts : (rubric ?? 'Score 1.0 if fully correct, 0.5 if partial, 0.0 if wrong.'),
    '',
