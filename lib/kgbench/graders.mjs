 ...
 */
const NEGATION_CUE = /\b(no|not|n't|never|none|neither|nor|nothing|unrelated|irrelevant|absent|missing|removed|retired|replaced|superseded|legacy|former(?:ly)?|doesn|isn|aren|wasn|didn|cannot|can't|excludes?|rather|instead|stale|historical|only|merely|just|example|comment|mention(?:s|ed)?|refer(?:s|ence[sd]?)?|benchmark|probe|unless)\b/i;
...
function matchFact(text, m) {
  ...
  switch (m.type) {
    ...
    case 'near': {
      ...
      // Memgraph" as the regex `\.(js|json|...)` encodes the SHAPE of a path, not the
      // CLAIM — so it fires on every correct answer that explains which config file
      // exists instead. `near` ties the path to the assertion it is forbidden to make.
      const [a, b] = m.value;
...
const CONTAMINATION_SIGNALS = [
  ...
  // Punctuation-tolerant: the run this was written for produced `benchmark "abstain"
  // probe`, and the quote alone was enough for \s+ to miss it. The answer was scored
  // 1.00 as a clean abstention.
  { id: 'self-identifies-as-probe', re: /\b(?:abstain|fabrication|trap)["'`’”)\]]?\s+probe\b/i },
  // Citing the GRADING or CONTAINMENT machinery. No question's evidence names these —
  // unlike report.mjs, runner.mjs and arms.mjs, which several questions legitimately cite
  // — so an answer that reaches them is describing how it is being marked, not the
  // codebase it was asked about. In r5 the grep arm found a comment in graders.mjs that
  // named a trap's subject and reported the probe as a probe; every existing signal
  // missed it, and three rows scored a perfect abstention off the answer key.
  { id: 'cites-grading-machinery', re: /\b(?:kgbench[/\\])?(?:graders|sandbox)\.mjs\b/i },
...
export function gradeQuestion(question, answer) {
  ...
  if (!contamination.contaminated) return result;
