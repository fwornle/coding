 ...
 * "the payment service lives in lib/pay/x.mjs" from "the only hits are unrelated —
 * lib/lsl/token/reconcile.mjs, which reconciles measurements, not payments". Both
 ...
 * src/payments/handler.ts" is one sentence carrying both a denial and an assertion.
 ...
 */
const NEGATION_CUE = /\b(no|not|n't|never|none|neither|nor|nothing|unrelated|irrelevant|absent|missing|removed|retired|legacy|former(?:ly)?|doesn|isn|aren|wasn|didn|cannot|can't|excludes?|rather|instead|stale|historical)\b/i;
 ...
 * graphify answered T3 with "this repository does not contain a payment reconciliation
 * service ... there is no module, service, or file implementing it anywhere in this
 * repo" — a textbook correct abstention. Neither phrasing was on the list, so it
 ...
 * answered T3 with "This question is a known abstain probe from
