...
function matchFact(text, m) {
  if (!m) return false;
  // Every branch matches the SAME normalised text. Stripping for `any-of` but not for
  // `near` would make two matchers disagree about the same answer for a reason that has
  // nothing to do with its content.
  const src = stripEmphasis(text);
  ...
  switch (m.type) {
    ...
    case 'regex':
      return new RegExp(m.value, m.flags ?? 'i').test(src);
    case 'symbol':
      return new RegExp(`\\b${escapeRe(m.value)}\\b`, 'i').test(src);
    case 'near': {
      ...
      return new RegExp(`(?:${a})[\\s\\S]{0,${w}}(?:${b})|(?:${b})[\\s\\S]{0,${w}}(?:${a})`, m.flags ?? 'i').test(src);
    }
    case 'any-of':
