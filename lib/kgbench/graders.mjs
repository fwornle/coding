...
/**
 * Remove markdown emphasis before matching.
 *
 * Models write answers in markdown, and a matcher written in plain prose does not see
 * through it: `is **not** affected` failed a pattern for `is not affected` on the two
 * asterisk pairs alone, and the answer it rejected was a heading stating exactly the
 * required fact. Widening the regex would have fixed that phrasing and left the next
 * one, so the decoration is stripped once, here, for every matcher.
 *
 * `*` and backticks only. `_` is left alone because it is load-bearing in the symbols
 * these questions ask about — CODEGRAPH_MAX_DEPTH, ANTHROPIC_BASE_URL, MANAGED_MCP_KEYS.
 */
...
/**
 * Hyphen-versus-space is a phrasing choice, not a fact.
 *
 * A1's f2 wanted `single owner` and `observations-api`; an answer wrote `single-owner`
 * and `Observations API` — the same two facts with the separators exactly inverted, and
 * scored 0 on both. This is the same shape as the emphasis bug above: widening one
 * matcher fixes one phrasing and leaves the next, so the variance is folded out once,
 * here, for every matcher. Unicode dashes are included because models emit en-dashes.
 *
 * `_` is deliberately NOT folded, for the same reason stripEmphasis leaves it alone: it
 * is load-bearing in the symbols these questions ask about — CODEGRAPH_MAX_DEPTH,
 * ANTHROPIC_BASE_URL, MANAGED_MCP_KEYS.
 *
 * Applied to the HAYSTACK and to LITERAL needles only. `regex` and `near` values are
 * regex source, where folding would rewrite character classes — `[a-z]` into `[a z]`.
 */
export function foldSeparators(text) {
  return String(text ?? '').replace(/[-‐-―\s]+/g, ' ');
}

function matchFact(text, m) {
  if (!m) return false;
  // EMPHASIS is stripped for every branch: `is **not** affected` is the same claim as
  // `is not affected`, whoever wrote the matcher.
  //
  // SEPARATORS are folded only where the needle is a LITERAL. A `regex`/`near` value is
  // author-written pattern source, and a hyphen in it is deliberate: folding the text
  // underneath such a pattern silently breaks it. S2's f2 is `graphify-serve\.sh`, and
  // folding the haystack to `graphify serve.sh` cost all 12 of its cells a required fact
  // — in all four arms at once, which is what a grader bug looks like. Pattern authors
  // who want tolerance can write `[-\s]` themselves.
  const raw = stripEmphasis(text);
  const src = foldSeparators(raw);
  const hay = src.toLowerCase();
  const lit = (v) => foldSeparators(String(v)).toLowerCase();
  switch (m.type) {
    case 'path': {
      const want = foldSeparators(normPath(m.value));
      return new RegExp(`(^|[\\s\`'"(])${escapeRe(want)}([\\s\`'")]|$)`, 'im').test(src) || hay.includes(want.toLowerCase());
    }
    case 'regex':
      return new RegExp(m.value, m.flags ?? 'i').test(raw);
    case 'symbol':
      return new RegExp(`\\b${escapeRe(m.value)}\\b`, 'i').test(raw);
    case 'near': {
      ...
      return new RegExp(`(?:${a})[\\s\\S]{0,${w}}(?:${b})|(?:${b})[\\s\\S]{0,${w}}(?:${a})`, m.flags ?? 'i').test(raw);
    }
    case 'any-of':
      return m.value.some((v) => hay.includes(lit(v)));
    case 'all-of':
      return m.value.every((v) => hay.includes(lit(v)));
    default:
      return hay.includes(lit(m.value ?? ''));
  }
}
...
const ABSTAIN_PATTERNS = [
  ...
  // "No <noun phrase> {is a module | exists}" — the subject sits between the negator and
  // its head, so a bound on the gap is unavoidable. That bound is counted in WORDS.
  //
  // It used to be counted in characters, and that cost a real cell: an arm wrote "No
  // payment-processing/financial-transaction reconciliation module exists in this
  // codebase" and was scored 0 with `hallucinated: true` — a correct abstention graded as
  // a fabrication — because the window allowed sixty characters and that subject spans
  // sixty-four. It is three words. Character count is a proxy for "a noun phrase
  // intervenes", and in a code repository it is a systematically biased one: subjects here
  // are hyphenated, slashed, dotted compounds, so the budget tightens exactly as the
  // subject gets more technical. Words do not have that property. The sibling defect was
  // the same bound set to zero — `no <noun>` demanded the noun be adjacent, so a quoted
  // subject ("no `payment reconciliation` module") slipped past too. One shape fixes both.
  //
  // Subjects here are placeholders, not real examples: this file ships in the run tree,
  // and an illustrative subject is a crib an arm can grep. See lib/kgbench/sandbox.mjs.
  new RegExp(String.raw`\bno\b(?:\s+[^\s.!?]+){0,6}\s+`
    + String.raw`(?:modules?|files?|services?|components?|functions?|class(?:es)?|code`
    + String.raw`|results?|matches?|implementations?)\b`, 'i'),
  new RegExp(String.raw`\bno\b(?:\s+[^\s.!?]+){0,6}\s+`
    + String.raw`(?:exists?|is (?:present|defined|implemented)|are (?:present|defined))\b`, 'i'),
...
const CONTAMINATION_SIGNALS = [
  ...
  //
  // The filename match is boundary-anchored, and that is not pedantry: `sandbox.mjs` is a
  // SUBSTRING of `agent-sandbox.mjs`, which is neither grading machinery nor excluded from the
  // tree — it is ordinary source that several questions can legitimately reach. A correct
  // answer citing `lib/kgbench/agent-sandbox.mjs:68-83` while explaining that a gate "fails the
  // benchmark cell" put the word `benchmark` within 120 characters of the substring and was
  // voided at score_if_clean 1.00. A contamination signal that deletes right answers is worse
  // than no signal, because the loss is silent and looks like the arm failing.
  {
    id: 'cites-grading-machinery',
    re: new RegExp(
      '(?<![\\w-])(?:graders|sandbox)\\.mjs[\\s\\S]{0,120}(?:' + GRADING_VOCAB + ')'
      + '|(?:' + GRADING_VOCAB + ')[\\s\\S]{0,120}(?<![\\w-])(?:graders|sandbox)\\.mjs',
      'i',
    ),
  },
