...
/**
 * Count tool calls the CLI rejected because the model's arguments did not satisfy the tool's
 * own JSON schema. A wasted round trip: the model is billed for the call, the tool never runs.
 *
 * WHY THIS IS COUNTED RATHER THAN ASSUMED RARE. opencode rejects 5.9% of its bash calls this
 * way (35 of 589 across the coding-v1-x2 sessions) — always the same omission, the required
 * `description` parameter. Exactly ONE of those 35 reached a benchmark row, because `stderr`
 * is kept as `slice(-300)` and only the occurrence that happened to land last survived. A tax
 * that shows up in the record 1 time in 35 reads as a curiosity instead of a rate, so the
 * count is taken from the FULL stderr, before any truncation.
 *
 * It is a fact about the AGENT+MODEL pairing, not about retrieval: opencode's schema is
 * correct (`required: ['command','description']`, verified on the wire), and the tool that
 * rejects the call is opencode's own. claude and copilot do not pay it. Any per-agent
 * comparison that does not state it is attributing a CLI's schema-adherence rate to whichever
 * retrieval arm happened to be underneath.
 *
 * Returns `{ count, detail }` with detail keyed `<tool>:<missing-key>` so the aggregate can
 * say WHICH argument, not just how many. Never throws; unparseable stderr yields zero.
 */
export function toolArgErrors(stderr) {
  // The control character is the point: this strips ANSI SGR sequences from a
  // captured stderr. Written as \x1b rather than the raw ESC byte the source used
  // to carry, so the pattern is visible to anyone reading it.
  // eslint-disable-next-line no-control-regex -- deliberate: ANSI escape stripping
  const text = String(stderr ?? '').replace(/\x1b\[[0-9;]*m/g, '');
  const detail = {};
  let count = 0;
  // The message wraps mid-sentence — `Missing key\n  at ["description"]` — so the key is
  // matched with explicit tolerance for the newline rather than by scanning a single line.
  const re = /The (\S+) tool was called with invalid arguments:\s*(?:SchemaError\((?:[^)]*?at\s*\[\s*"([^"]+)"\s*\])?[^)]*\))?/g;
  for (const m of text.matchAll(re)) {
    count += 1;
    const key = `${m[1]}:${m[2] ?? 'unknown'}`;
...
export function runAgent({ prompt, arm, cwd, env = process.env, builtins, noDeny = false, agent = null, overrideArgv = null }) {
  ...
  return new Promise((resolve) => {
    ...
    const finish = (rec) => {
      ...
      settled = true;
      clearTimeout(timer);
      // Counted here rather than in the per-outcome branches so it covers ALL of them. A
      // rejected tool call is not an outcome — a cell can pay it and still answer (the model
      // reissues the call, or switches to another tool), and those are precisely the cases a
      // per-failure counter would miss while the tax was still being paid.
      const argErrs = toolArgErrors(stderr);
      resolve({
        ...rec,
        // Omitted entirely when zero: every claude cell and every clean cell would otherwise
        // gain two always-empty fields, and results.jsonl is read by eye.
        ...(argErrs.count ? { tool_arg_errors: argErrs.count, tool_arg_error_detail: argErrs.detail } : {}),
    ...
    child.on('close', () => finish(
      adapter.elicitation === 'answer-file'
        ? readAnswerFile({
          ...
          // Same stream, second reading: the tool trace. Null for any adapter that has no
          // parser, which keeps those cells at `tool_audit: 'unavailable'` exactly as before.
          toolTrace: adapter.toolTraceFrom ? adapter.toolTraceFrom(stdout) : null,
        })
...
/**
 * The span of a SEQUENCE of sub-runs, as one span.
 *
 * The operator paid for every part, so `wall_s` is the SUM of the parts and the span runs from the
 * first part's start to the last part's end.
 *
 * TWO LEVELS NEED THIS RULE AND ONLY ONE OF THEM HAD IT. `runAgentWithContinuations` wrote it
 * inline — and, summing only `first + last`, dropped every leg in between, which is harmless at a
 * budget of 1 and silently lossy at the current default of 2. `runCell`'s retry loop did not mirror
 * it at all: it spread the LAST attempt's result onto the row while resolving tokens over a window
 * covering EVERY attempt. The row then carried the last attempt's clock beside an all-attempts token
 * total — a row that could not reproduce its own number, and a `wall_s` that made a cell needing two
 * attempts look as cheap as one that needed none.
 *
 * `windows` is the part that makes the row re-resolvable offline: it is the list the token resolver
 * needs to tell "a session that is my own next attempt" from "a foreign session that started while I
 * was running". Without it those are indistinguishable, and every retried cell in run coding-v1-r8
 * was flagged as the second when it was the first.
 *
 * A part with NO timestamps still contributes its `wall_s` and contributes no window. That is not a
 * corner case: `runAgent`'s spawn-error path resolves before `finish()` ever runs, so it has a
 * duration and no instants. Dropping it from the sum would understate a cell that really did burn
 * the time.
 *
 * @param {Array<{started_at?: string, ended_at?: string, wall_s?: number}>} parts
 * @returns {{started_at: string|null, ended_at: string|null, wall_s: number,
 *            windows: Array<{started_at: string, ended_at: string}>}}
 */
export function spanOfParts(parts = []) {
  const windows = parts
    .filter((p) => p?.started_at && p?.ended_at)
    .map(({ started_at, ended_at }) => ({ started_at, ended_at }));
  return {
    started_at: windows[0]?.started_at ?? null,
    ended_at: windows[windows.length - 1]?.ended_at ?? null,
    // toFixed(1) matches the precision runAgent records per part, so a sum of parts and a part
    // compare on the same scale instead of differing in the last float digit.
    wall_s: +(parts.reduce((s, p) => s + (p?.wall_s ?? 0), 0)).toFixed(1),
    windows,
  };
}

/**
 * Sum a cell's TOOL TRACE across every continuation leg.
 *
 * The exact counterpart of spanOfParts, and it exists for the same reason spelled out there:
 * a continued cell's result is built with `{...last}`, so anything not explicitly summed
 * silently becomes "the last leg only". wall_s already had to be fixed this way. The tool
 * trace has the same shape and was introduced with the same defect.
 *
 * It matters MORE than wall-clock, because of WHICH leg gets dropped. opencode's headless
 * `run` ends at its first toolless step — which is typically the step right after it has
 * finished investigating — so the recovery turn does nothing but write the answer file. Leg 1
 * holds every grep and read; leg 2 holds a single `write`. Taking the last leg therefore
 * discards precisely the retrieval calls the benchmark exists to count, and reports the cell
 * as a one-tool cell that somehow knew the answer. Observed in run kgv1-telemetry: 13 of 16
 * opencode cells recorded `['write']` alone while scoring 1.00 with exact file paths and
 * line numbers — an impossible combination that is explained entirely by this bug.
 *
 * Returns null when NO leg carried a trace, so an agent with no parser keeps reporting
 * `tools_executed: null` -> `tool_audit: 'unavailable'` rather than gaining an empty trace.
 */
export function mergeToolTraces(legs = []) {
  const traced = legs.filter((l) => l && l.tools_executed != null);
  if (!traced.length) return null;
  const cat = (k) => traced.flatMap((l) => l[k] ?? []);
  // Summed only where at least one leg reported a number: a null must stay null rather than
  // collapsing to 0, which would read as "measured, and it was zero".
  const sum = (k) => {
    const vals = traced.map((l) => l[k]).filter((v) => typeof v === 'number');
    return vals.length ? vals.reduce((a, b) => a + b, 0) : null;
  };
  return {
    tool_calls: sum('tool_calls'),
    tools: cat('tools'),
    tools_executed: cat('tools_executed'),
    tools_denied: cat('tools_denied'),
    tool_control_calls: sum('tool_control_calls'),
    tool_result_tokens_est: sum('tool_result_tokens_est'),
    // Tokens are per-leg too, and the operator paid for every leg.
...
async function runAgentWithContinuations({
  ...
  // Every leg, not just the first and the last. `wall_s: first + last` was correct only while the
  // budget was 1; at the current default of 2 a cell that continues twice had its middle leg's
  // wall-clock silently discarded, and run coding-v1-r8-cont2 already contains such cells.
  const legs = [first];
  let last = first;
  let used = 0;
  ...
  const span = spanOfParts(legs);
  // The tool trace is the CELL's too, for the same reason and with higher stakes — see
  // mergeToolTraces. Spread AFTER ...last so the summed trace wins over the last leg's.
  const trace = mergeToolTraces(legs);
  return {
    ...last,
    ...(trace ?? {}),
    // Wall-clock is the CELL's, not the last turn's — the operator paid for every turn.
    wall_s: span.wall_s,
    started_at: span.started_at ?? first.started_at,
    ended_at: span.ended_at ?? last.ended_at,
...
function readAnswerFile({
  cwd, answerFile, stdout, stderr, spawnedAt = 0, displayText = null, toolTrace = null,
}) {
  ...
  if (!answer) {
    return {
      ...
      // THE TOOL TRACE BELONGS HERE TOO, and leaving it out made mergeToolTraces a no-op.
      //
      // A `no_result` leg is not an idle leg. It is the leg that DID the work: opencode's
      // headless `run` ends at its first toolless step, which is typically the step right
      // after it has finished investigating — so this leg holds every grep and read, and the
      // recovery leg that follows holds nothing but the `write`. Returning no tool fields
      // here made `tools_executed` undefined, so the merge filtered this leg out and summed
      // only the recovery leg: the exact undercount the merge was written to fix, still 1.0
      // tool calls per cell on the re-run that was supposed to prove the fix.
      //
      // It also makes a STANDALONE no_result row self-describing: "exited without answering
      // after 4 greps" and "exited having done nothing" are different failures, and the row
  ...
  return {
    ...
    tools_executed: null,
    tools_denied: null,
    // Neither copilot nor opencode announces its tool surface the way claude's `system`
    // init event does, so this stays null even when a trace IS available: we know what the
    // agent USED, not what it COULD have used. Those are different claims.
    available_tools: null,
    in_tokens: null,
    out_tokens: null,
    total_tokens: null,
    num_turns: null,
    // The adapter's parsed trace overrides the nulls above when the CLI publishes a
    // structured event stream. Spread LAST so a real measurement wins, and so an adapter
    // that reports only some fields (copilot has no per-cell token totals in its stream)
    // leaves the rest null rather than zeroed.
    ...(toolTrace ?? {}),
  };
}
...
export async function runCell({
  ...
  for (let attempt = 1; attempt <= arm.maxAttempts; attempt++) {
    ...
    //
    // THREE states, not two. Adding a tool trace for copilot and opencode split what used
    // to be one question into two, and collapsing them back would be a lie in whichever
    // direction it collapsed:
    //
    //   unavailable — no trace at all (pi, or any cell whose stream carried no events)
    //   observed    — a trace exists, but this agent's tool NAMES are its own (`view`,
    //                 `create`, `read`, `write`) and arms are written in claude's
    //                 vocabulary. What ran is recorded; whether it conformed to the arm is
    //                 NOT decidable, because no verified name mapping exists.
    //   audited     — trace exists AND its names are the arm's own, so conformance is
    //                 decidable and decided.
    //
    // Running toolViolations() over native names would mark EVERY copilot and opencode
    // cell `tool_escape`: not one of `view`/`create`/`read`/`write` appears in any arm's
    // allowedTools. That failure would look like a finding — "non-claude agents escape
    // their arm constantly" — and it would be an artefact of vocabulary alone. This is the
    // same reason agents.mjs refuses to pass unmapped names to `--available-tools`.
    if (r.outcome === 'ok') {
      if (r.tools_executed == null) {
        r.tool_audit = 'unavailable';
        // `agent` is null on the default claude path and the resolved object otherwise —
        // resolveAgent() spreads the adapter, so toolVocabulary rides along. Defaulting to
        // 'arm' keeps a null agent (claude) on the audited branch it has always taken.
      } else if ((agent?.toolVocabulary ?? 'arm') !== 'arm') {
        r.tool_audit = 'observed';
        r.tool_audit_note = `${agentId} reports its own tool names; no verified mapping to arm `
          + 'tool names exists, so conformance to the arm is not decidable from this trace';
      } else {
    ...
    attempts.push({
      ...
      outcome: r.outcome,
      wall_s: r.wall_s,
      // The window this attempt actually occupied, so the ROW is self-describing: the token
      // number stored on it can be re-derived from the row alone, offline. It could not be
      // before — the row carried only the LAST attempt's clock, so re-resolving a retried cell
      // from its own row returned about half the cell (see kgbench-backfill-tokens.mjs).
      started_at: r.started_at ?? null,
      ended_at: r.ended_at ?? null,
      // Per attempt, because each attempt spends its own budget. Only the last attempt's count
      // reached the row before, so a cell that continued on attempt 1 and not on attempt 2
      // reported zero continuations.
      ...(r.continuations_used ? { continuations_used: r.continuations_used } : {}),
    });
  ...
  async function finalize(r, extra) {
    // THE ROW DESCRIBES THE CELL, NOT THE LAST ATTEMPT.
    //
    // `r` is the last attempt's result, and spreading it alone gave the row that attempt's
    // `started_at` and `wall_s` beside a token total already spanning every attempt. Two things
    // followed. The row could not reproduce its own number — re-resolving a retried cell from its
    // own window returns roughly half, which kgbench-backfill-tokens.mjs would have written back
    // as an improvement. And `wall_s` charged a cell that burned 73.6s across two attempts as
    // 35.6s, which flatters exactly the arms that needed a second try.
    const span = spanOfParts(attempts);
    const cell = {
      started_at: span.started_at ?? r.started_at ?? null,
      ended_at: span.ended_at ?? r.ended_at ?? null,
      wall_s: span.wall_s,
    };
    const identity = {
    ...
    const tokens = await resolveCellTokens({
      ...
      // Only when the cell actually ran more than once. A single-attempt cell then takes a path
      // byte-identical to the one it took before attempt windows existed.
      windows: span.windows.length > 1 ? span.windows : null,
      bound: bind.bound,
      ...tokenOpts,
    });
    return { ...r, ...cell, ...identity, ...tokens };
  }
}
