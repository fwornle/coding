 ...
 */
export function runAgent({ prompt, arm, cwd, env = process.env, builtins, noDeny = false }) {
  env = agentEnv(env);
  ...
  const args = [
    ...
    ...(noDeny ? [] : ['--disallowedTools', denyListFor(arm, builtins ?? DENYABLE_BUILTINS).join(',')]),
 ...
 */
export async function measureBaseline({ arm, cwd, env, reps = 3, builtins }) {
  const prompt = 'Reply with the single word OK. Do not use any tools.';
  const samples = [];
  // The baseline probes already start a real session with this arm's real flags, so the
  // tool surface they report is free evidence of whether isolation actually applied.
  let available = null;
  for (let i = 0; i < reps; i++) {
    const r = await runAgent({ prompt, arm, cwd, env, builtins });
    if (r.outcome === 'ok') { samples.push(r.in_tokens); available ??= r.available_tools; }
  }
  if (!samples.length) return { baseline_in_tokens: null, samples: 0, available_tools: available };
 ...
 */
export async function runCell({ arm, question, rep, cwd, env, builtins }) {
