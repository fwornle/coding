...
/**
 * Environment for a benchmark child.
 *
 * Strips ANTHROPIC_API_KEY / ANTHROPIC_AUTH_TOKEN, matching what
 * configure_proxy_routing() in scripts/launch-agent-common.sh already does for the
 * claude agent. Without this the child inherits whatever pay-as-you-go key happens
 * to be exported from .env, which is a different credential (and a different billing
 * state) from the one interactive sessions use — an exhausted key there turns every
 * cell into an api_error and the whole matrix into noise. ANTHROPIC_BASE_URL is
 * deliberately preserved so proxy routing still applies.
 */
