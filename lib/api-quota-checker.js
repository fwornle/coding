...
async function checkGroqQuota(config, timeout) {
  ...
  if (billingType === 'monthly') {
    ...
    try {
      const repoRoot = process.env.CODING_REPO || path.resolve(path.dirname(new URL(import.meta.url).pathname), '..');
 ...
 * Uses Management API if XAI_MANAGEMENT_KEY + XAI_TEAM_ID are set,
 * otherwise falls back to static prepaidCredits from config.
 */
async function checkXAIQuota(config, timeout) {
  ...
  // If we have Management API credentials, fetch real balance
  if (managementKey && teamId) {
    try {
      const balanceResponse = await makeHttpRequest(
      ...
      if (balanceResponse.statusCode === 200) {
        ...
        // Extract balance — API may return { balance: X } or { remaining_balance: X }
        const remainingCredits = balanceData.balance ?? balanceData.remaining_balance ?? balanceData.amount ?? null;

        if (remainingCredits !== null) {
          const initialCredits = config?.provider_credits?.xai?.prepaidCredits ?? 25;
          const remaining = initialCredits > 0 ? Math.round((remainingCredits / initialCredits) * 100) : null;
          ...
          return {
            ...
            quota: {
              ...
              used: `$${(initialCredits - remainingCredits).toFixed(2)}`,
              limit: `$${initialCredits}`,
              unit: 'USD'
            },
            cost: {
              total: initialCredits - remainingCredits,
              currency: 'USD',
      ...
      } else if (balanceResponse.statusCode === 401 || balanceResponse.statusCode === 403) {
        logger.warn('xAI Management API key invalid or lacks permissions');
      } else {
        logger.warn(`xAI balance API returned ${balanceResponse.statusCode}`, { body: balanceResponse.body });
      }
    } catch (error) {
      logger.error('xAI Management API check failed', { error: error.message });
    }
  }

  // Fallback: Use static prepaid credits from config
  try {
    const prepaidCredits = config?.provider_credits?.xai?.prepaidCredits ?? 25;
    const estimatedUsed = 0;
