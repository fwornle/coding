 ...
 *
 * Phase 69, Plan 69-05, Task 1 — live task_id stamping integration test.
 *
 * Proves the live-tail wiring contract (claude-fs-watch onTokenRow →
 * sub-agent-live-claude supervisor): build rows from a Claude session JSONL,
 * stamp each row's task_id via the SINGLE span reader (resolveLiveTaskIdSafe,
 * D-03), set the distinct adapter user_hash, and insert into token-usage.db.
 *
 * Covers:
 *   1. With LLM_PROXY_DIST_DIR pointed at a stub exporting
 *      `resolveLiveTaskId: () => 'task-live-1'`, every inserted row reads back
 *      with `task_id === 'task-live-1'` and `user_hash === 'cladpt'`.
 *   2. With the stub returning '' (no open span), every inserted row reads back
 *      with `task_id === ''` (out-of-window / no-span resolution rule).
 *   3. An insertTokenRow that THROWS does not propagate out of the
 *      onTokenRow-shaped emission path (the supervisor's hook swallows it,
 *      D-08 failure isolation) — the emit helper resolves, never rejects.
 *
 * The emission path under test mirrors the supervisor's onTokenRow body in
 * scripts/sub-agent-live-claude.mjs verbatim (build → stamp → set hash →
 * insert each, all inside try/catch).
 */
...
function makeTempDb() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'claude-taskid-db-'));
...
function ownedFixtureCopy() {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'claude-taskid-fx-'));
...
function makeStubDist(spanJs) {
  const dir = fs.mkdtempSync(path.join(os.tmpdir(), 'claude-taskid-dist-'));
 ...
 * scripts/sub-agent-live-claude.mjs (build → stamp via resolveLiveTaskIdSafe →
 * set adapter user_hash → insert each), wrapped in try/catch so a thrown
 * insert never propagates.
 */
async function emitTokenRows({
  fullPath,
  tokenDb,
  buildClaudeTokenRows,
  insertTokenRow,
...
test('live rows are stamped with the live task_id from the single span reader', async () => {
  jest.resetModules();
  const distDir = makeStubDist(
    "module.exports = { resolveLiveTaskId: () => 'task-live-1' };\n",
  );
  ...
  try {
    ...
    const rows = tokenDb
      .prepare('SELECT task_id, user_hash, granularity_tier FROM token_usage WHERE user_hash = ?')
      .all(ADAPTER_USER_HASH_CLAUDE);

    // 3 per-turn + 2 per-reasoning-step (two thinking blocks in the fixture).
    expect(rows.length).toBeGreaterThanOrEqual(3);
    for (const r of rows) {
      expect(r.task_id).toBe('task-live-1');
...
test('live rows carry task_id="" when no span is open (reader returns "")', async () => {
  jest.resetModules();
  const distDir = makeStubDist(
...
test('an insertTokenRow that throws is swallowed by the emission path (D-08 isolation)', async () => {
  jest.resetModules();
  const distDir = makeStubDist(
    "module.exports = { resolveLiveTaskId: () => 'task-live-1' };\n",
  );
  ...
  const { ADAPTER_USER_HASH_CLAUDE } = await import(
    '../../lib/lsl/token/token-db.mjs'
  );
  const { resolveLiveTaskIdSafe } = await import('../../lib/lsl/token/task-id.mjs');

  // A poisoned insert that throws synchronously — the emission path must NOT
  // let it propagate (mirrors the supervisor's try/catch, D-08).
  ...
  // The emission helper resolved (did not reject) despite the throwing insert.
