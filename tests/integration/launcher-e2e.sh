...
# Test: --pi flag selects the pi agent and the adapter is valid
test_pi_flag() {
  local coding_root
  coding_root="$(cd "$SCRIPT_DIR/../.." && pwd)"

  # 1. Help text contains --pi
  local help_output
  help_output=$("$CODING_BIN" --help 2>&1) || true
  assert_output_contains "$help_output" "--pi" "help should document --pi" || return 1

  # 2. Agent adapter sets correct AGENT_NAME
  local agent_name
  agent_name=$(bash -c 'source "'"$coding_root"'/config/agents/pi.sh" 2>/dev/null; echo "$AGENT_NAME"')
  if [ "$agent_name" != "pi" ]; then
    echo "Expected AGENT_NAME=pi, got $agent_name"
    return 1
  fi

  # 3. Agent adapter sets correct AGENT_COMMAND
  local agent_cmd
  agent_cmd=$(bash -c 'source "'"$coding_root"'/config/agents/pi.sh" 2>/dev/null; echo "$AGENT_COMMAND"')
  if [ "$agent_cmd" != "pi" ]; then
    echo "Expected AGENT_COMMAND=pi, got $agent_cmd"
    return 1
  fi

  # 4. There is deliberately NO scripts/launch-pi.sh — bin/coding falls back to
  #    launch-generic.sh for config-only agents. Assert the generic path exists
  #    instead of an agent-specific wrapper that should not be written.
  if [ ! -x "$coding_root/scripts/launch-generic.sh" ]; then
    echo "scripts/launch-generic.sh is not executable (pi relies on it)"
    return 1
  fi
  if [ -e "$coding_root/scripts/launch-pi.sh" ]; then
    echo "scripts/launch-pi.sh should NOT exist — pi uses the generic launcher"
    return 1
  fi
}
...
run_test "--pi flag and adapter valid"            test_pi_flag
